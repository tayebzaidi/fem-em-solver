...
import tests.validation.test_birdcage_sar_mass_averaged as _mass_averaged_module  # noqa: E402

# `OPS-63`: the shared builder ``_build_mass_averaged`` calls
# ``build_four_port_sweep`` and prints its `PORT-9` readings (reciprocity,
# sigma(S), column power sums, Z class spreads) but does not return them, and
# the asserts on them live in pytest functions this example never calls.  To
# assert what the log shows, the example wraps the *same* function in the
# builder's module namespace and keeps its return dict — same call, same
# arguments, same result; no builder edited, no computation added.
_CAPTURED_SWEEP: dict = {}
_build_four_port_sweep_original = _mass_averaged_module.build_four_port_sweep


def _capturing_four_port_sweep(*args, **kwargs):
    sweep = _build_four_port_sweep_original(*args, **kwargs)
    _CAPTURED_SWEEP["sweep"] = sweep
    return sweep


_mass_averaged_module.build_four_port_sweep = _capturing_four_port_sweep
...
def _spread(values):
    ...
    scatter on the tracked 0.0025-rung ``metrics.json``.  The AED side of that
    comparison lives only in the private adjudication file (gitignored, never
    quoted here — `OPS-63`).  Whether the spread collapses under h-halving is
    the whole question of step 4.
    """
...
def _write_comparison(m, *, private: bool = False) -> Path:
    ...
    # Raw f-string (`OPS-63`): the Markdown pipe escapes ``\|`` below were an
    # invalid Python escape (SyntaxWarning).  Byte-identical output — ``\|``
    # stays ``\|`` and the one former ``\\|`` is now written ``\|``.
    path.write_text(
        rf"""# ANS-2 — coil-driven SAR, loaded four-leg birdcage, 10 MHz: comparison table
...
def _print_step4_readout(m) -> None:
    ...
    print(
        ...
        + "   — the AED reading is in the private adjudication file only; "
        "PRINTED, NEVER GATED",
        flush=True,
    )
    print(
...
def main() -> None:
    ...
    # -- `OPS-63`: the builders' ASSERTED labels, asserted here too. ----------
    sweep = _CAPTURED_SWEEP["sweep"]
    sigma_max = float(np.max(sweep["sigma"]))
    column_power_max = float(np.max(sweep["column_power"]))
    containment_reach = CENTRE_RADIUS_M + float(built["radii"]["10 g"])
    containment_z = max(
        abs(float(c[2])) + float(built["radii"]["10 g"])
        for c in built["centres"].values()
    )
    if comm.rank == 0:
        print(
            "[ANS-2] LABEL KEY (`OPS-63`): the [PORT-9 step3d] and [MAT-4 step 4] "
            "blocks above are printed by the shared builders, whose own ASSERTED "
            "labels refer to their pytest modules.  THIS EXAMPLE ASSERTS, with "
            "every band imported: the [ANS-2] ANCHORS lines above, and the lines "
            "below.\n"
            f"        PORT-9 (i) ||S - S^T||/||S|| = {sweep['reciprocity']:.3e}, "
            f"ASSERTED <= {RECIPROCITY_BAND:.0e}\n"
            f"        PORT-9 (ii) sigma_max(S) = {sigma_max:.9f}, max column power "
            f"sum = {column_power_max:.9f}, ASSERTED <= 1 + "
            f"{PASSIVITY_SIGMA_TOLERANCE:.0e}\n"
            "        PORT-9 (iii') Z class spreads self "
            f"{sweep['spreads']['self'] * 100:.4f}%  adjacent "
            f"{sweep['spreads']['adjacent'] * 100:.4f}%  opposite "
            f"{sweep['spreads']['opposite'] * 100:.4f}%, ASSERTED <= "
            f"{ADJACENT_SPREAD_BAND * 100:.1f}%\n"
            f"        MAT-4 ball containment r0 + a_10g = "
            f"{containment_reach * 1e3:.4f} mm, ASSERTED < phantom radius "
            f"{PHANTOM_RADIUS * 1e3:.1f} mm; max |z_c| + a_10g = "
            f"{containment_z * 1e3:.4f} mm, ASSERTED < half-height "
            f"{0.5 * PHANTOM_HEIGHT * 1e3:.1f} mm\n"
            "        MAT-4 10 g mis-paired control (sign) "
            + "  ".join(
                f"(c_{(k + 2) % 4}; {k}) {built['control_pairs'][k] * 100:.4f}%"
                for k in range(4)
            )
    ...
    assert sweep["reciprocity"] <= RECIPROCITY_BAND, (
        f"the four-port sweep reads ||S - S^T||/||S|| = {sweep['reciprocity']:.9e} "
        f"against the imported {RECIPROCITY_BAND:.0e} RECIPROCITY_BAND"
    )
    assert sigma_max <= 1.0 + PASSIVITY_SIGMA_TOLERANCE, (
        f"sigma_max(S) = {sigma_max:.9f} exceeds 1 + the imported "
        f"{PASSIVITY_SIGMA_TOLERANCE:.0e} PASSIVITY_SIGMA_TOLERANCE"
    )
    assert column_power_max <= 1.0 + PASSIVITY_SIGMA_TOLERANCE, (
        f"max column power sum of S = {column_power_max:.9f} exceeds 1 + the "
        f"imported {PASSIVITY_SIGMA_TOLERANCE:.0e}"
    )
    for name, value in sweep["spreads"].items():
        assert value <= ADJACENT_SPREAD_BAND, (
            f"the {name} class of Z spreads {value * 100:.4f}% against the "
            f"imported {ADJACENT_SPREAD_BAND * 100:.1f}% ADJACENT_SPREAD_BAND"
        )
    assert containment_reach < PHANTOM_RADIUS, (
        f"the 10 g ball reaches r = {containment_reach:.6f} m, outside the "
        f"phantom's imported {PHANTOM_RADIUS:.6f} m radius"
    )
    assert containment_z < 0.5 * PHANTOM_HEIGHT, (
        f"a 10 g ball reaches |z| = {containment_z:.6f} m, through the phantom's "
        f"flat face at {0.5 * PHANTOM_HEIGHT:.6f} m"
    )
    for k in range(4):
        assert built["control_pairs"][k] > C4_COVARIANCE_BAND, (
            f"the 10 g mis-paired control (c_{(k + 2) % 4}; {k}) reads "
            f"{built['control_pairs'][k] * 100:.4f}%, inside the imported "
            f"{C4_COVARIANCE_BAND * 100:.1f}% band — the 10 g C4 identity is "
            "not measuring covariance"
        )
