#!/usr/bin/env bash
# Scheduled weekly planning review (high effort). Installed in crontab;
...
if [[ -f "$MODEL_ENV" ]]; then
  ...
  if [[ -n "${REVIEW_MODEL_OVERRIDE_SCOPE:-}" && "$REVIEW_MODEL_OVERRIDE_SCOPE" != "weekly" ]]; then
    echo "$(date -u) review-model override scoped to ${REVIEW_MODEL_OVERRIDE_SCOPE}, not weekly; ignored" >> "$LOG"
