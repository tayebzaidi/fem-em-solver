"""`GEO-20` step 2a probe — the ring-gap layout at 16 legs, instrumented.

This is `tests/mesh/test_birdcage_ring_gaps_scaleup.py` as parked on
`attempt/GEO-20-step2-20260828T094500Z`, plus the step 2a instrumentation:
`_port_rank_ownership` (an `allgather`ed count of **owned** cells per
`PORT_LOWER+i` / `PORT_UPPER+i` tag) and the per-port / `DISCRIMINATOR` prints
that compare the broken-sheet set to the set of ports whose two half-boxes are
not owned by one rank. Run it with `-s` — the evidence lives in `print`:

    mpiexec -n 4 python3 -m pytest scripts/probes/geo20_step2a_ownership_scaleup.py -s

It lands here rather than on the attempt branch (the §7 entry's stated
alternative) because at the time of the 2026-08-28 15:00 slot a concurrent
session held `examples/ports/05_*` modified, and that path differs between
`main` and the branch, so `git checkout` of the branch would have refused. The
16-leg gates below are **red by construction** until `GEO-24` lands the ghost
layer; the measurement is the printed table, not the footer. Logs
`20260828T200204Z_GEO-20-step2a-n4.log` / `20260828T200524Z_GEO-20-step2a-n8.log`.
...
def _measure_ring(leg_count):
    ...
    return {
        ...
        "ownership": _port_rank_ownership(mesh, cells, port_cell_tags, comm),
        "comm_size": comm.size,
    }


def _port_rank_ownership(mesh, cells, port_cell_tags, comm):
    """``{cell tag: (owned cells on rank 0, ..., rank n-1)}`` — step 2a's evidence.

    `cells.values` is rank-local and `cells.indices` runs over ghosts as well as
    owned cells, so the count is taken over the owned block only and
    `allgather`ed; the resulting table is identical on every rank. The iteration
    order is fixed by `port_cell_tags`, which every rank builds the same way, so
    the `allgather`s are entered in the same order everywhere.
    """
    n_owned = mesh.topology.index_map(3).size_local
    owned = cells.indices < n_owned
    values = cells.values
    table = {}
    for tags in port_cell_tags.values():
        for tag in tags:
            table[tag] = tuple(
                comm.allgather(int(np.count_nonzero(owned & (values == tag))))
            )
    return table


def _owner_ranks(counts):
    """The ranks holding at least one owned cell of a tag."""
    return tuple(r for r, n in enumerate(counts) if n > 0)
...
def _report(label, m):
    ...
    # `GEO-20` step 2a's discriminator: the set of ports whose sheet does not
    # reconstruct, against the set whose two half-boxes are not owned by one
    # rank. The hypothesis under test predicts these two sets are equal at every
    # rank width, and a single port in one and not the other refutes it.
    broken, straddling = [], []
    for i in m["ring_ports"]:
        a_c = m["areas"][CONDUCTOR_IFACE + i]
        a_a = m["areas"][AIR_IFACE + i]
        a_p = m["areas"][PHANTOM_IFACE + i]
        lo = _owner_ranks(m["ownership"][PORT_LOWER + i])
        hi = _owner_ranks(m["ownership"][PORT_UPPER + i])
        if len(set(lo) | set(hi)) > 1:
            straddling.append(i)
        if abs(m["sheet_area"][i] / sheet_analytic - 1.0) >= EXACT:
            broken.append(i)
        print(
            f"[GEO-20 step 2 {label}] P{i} ownership: lower tag "
            f"{PORT_LOWER + i} {m['ownership'][PORT_LOWER + i]} ranks {lo}  upper "
            f"tag {PORT_UPPER + i} {m['ownership'][PORT_UPPER + i]} ranks {hi}  "
            f"straddles={len(set(lo) | set(hi)) > 1}",
            flush=True,
        )
        print(
    ...
    print(
        f"[GEO-20 step 2 {label}] DISCRIMINATOR at -n {m['comm_size']}: "
        f"broken sheets {broken} ({len(broken)}/{len(m['ring_ports'])})  "
        f"straddling ports {straddling} ({len(straddling)}/{len(m['ring_ports'])})"
        f"  sets agree={sorted(broken) == sorted(straddling)}  "
        f"broken-not-straddling={sorted(set(broken) - set(straddling))}  "
        f"straddling-not-broken={sorted(set(straddling) - set(broken))}",
        flush=True,
    )
