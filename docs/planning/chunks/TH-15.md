...
## Step 3 — executed 2026-09-13, 15:00 slot (✅)

**Module:** `tests/validation/test_th15_birdcage_pec_hole.py`, new. No `src/` change and no gate module edited, so rule (c) had nothing to re-run. One frequency per window (`TH15_STEP3_FREQ_MHZ`). Complex build, `FEM_EM_REQUIRE_COMPLEX=1`, `tests/environment` first, `-s`, `timeout -k 30 590`, `mpiexec -n 2`.

**Plumbing finding.** `run_lumped_sheet_port_case` needs no keyword: it builds its solver from `problem`, and the PEC selection lives on the problem. However, `pec_facet_tags` narrows the constrained set to the listed tags **only**, and `birdcage_port_domain` tags no outer-box facet group. `pec_facet_tags=(401,)` would therefore have dropped the outer box's PEC, which is a different fixture from `PORT-9`'s. On the hole mesh the cavity wall is itself exterior, so the default `None` pins both walls. The module asserts that containment, reduced over ranks: 19 826 owned tag-401 facets, 0 not exterior (`20260913T202954Z_TH-15.log:938`). **Consequence:** the natural-cavity control (401 not pinned) is not constructible without an outer-box tag, and it was not run. It was printed-only and unasserted, so this is a gap, not a gate.

**Fixture:** step 3a's own call, `tests/mesh/test_birdcage_port_sheets._build(True, as_hole=True)`, giving 80 181 cells. The sheet construction is `_four_port_rung`'s (imported helpers). The material map is phantom only, since the conductor cell tag is absent.

| f | `‖S−Sᵀ‖/‖S‖` (≤ 1e-3) | `σ_max` (≤ 1 + 1e-9) | spreads self / adj / opp (≤ 0.5 %) | P_src − P_sheets = Re P_in | ½∫_phantom σ\|E\|² | rel dev (≤ 1e-3) | log |
|---|---|---|---|---|---|---|---|
| 10 MHz | 1.588e-14 | 0.999994234 | 0.0190 / 0.0094 / 0.0059 % | 6.376395215e-08 W | 6.376395218e-08 W | 3.634e-10 | `202954Z:951–954` |
| 64 MHz | 1.474e-15 | 0.999813792 | 0.0483 / 0.0497 / 0.0326 % | 1.844922767e-06 W | 1.844922767e-06 W | 2.223e-12 | `202718Z:951–954` |
| 128 MHz | 9.164e-16 | 0.999502556 | 0.0455 / 0.0734 / 0.0465 % | 4.099567225e-06 W | 4.099567225e-06 W | 1.099e-12 | `202828Z:956–959` |

**Timing.** Each hole window took 61–63 s elapsed, i.e. mesh 21 s, sweep 5.3–5.5 s under `PORT-19` reuse, and the extra P1 field solve 4.0 s. The 10 MHz window with the solid control took 93 s. That is far under the 5–8 min prediction, which had been scaled from `PORT-11`'s pre-reuse 67/179/201 s. No orphaned ranks were found before or after any window (`pgrep -c python3` = 0).

**Negative control (asserted).** `PORT-11`'s own `_four_port_rung` on the solid 116 085-cell route, in the same window as the 10 MHz hole, reproduces `LEG_D_S_MATRIX_10MHZ` to 1.158e-10 against `FREQUENCY_CONTROL_BAND` 1e-6 (`202954Z:2768`; 17 passed, `:2833`). This is the reproduction record `PORT-11`'s 64 and 128 MHz modules both use as their control. The solid route was not re-run at 64 or 128 MHz.

**Power identity, the comparand as first registered and why it moved.** The first window (`20260913T202311Z_TH-15.log`, Status 1, 95.55 s) asserted the terminal form `Σ_i ½Re(V_i Ī_i)` and read 7.700077682e-05 W against 6.376395218e-08 W, a relative deviation of 1.2e+03 (`:958, :2775`). A sheet's terminal `½|I|²R` is not its form-level dissipation (`PORT-16`/`TH-19`), so that sum is not the power entering the field. The asserted `Re P_in` is now `P_src − Σ P_sheet,field`, taken through `TH-19`'s imported `_source_power_w` / `_sheet_field_dissipation_w`. The band (1e-3) did not move. The terminal sums are kept in the print as records: 7.700e-05 / 5.610e-05 / 3.226e-05 W. **Reading:** the exact form closes to about 1e-10 because it is the discrete Galerkin power balance. Its content on this fixture is that no dissipation remains outside the phantom once the coil is a Dirichlet hole. On the solid route the same balance carries a nonzero conductor term (σ = 800 S/m).

**Printed only.** The hole's 4×4 sits beside the σ = 800 solid record in one log: hole `202954Z:946–949`, solid `:2760–2763`. The per-class `max|ΔS|` at 10 MHz, **hand-read from row 1 of those two matrices rather than computed by the module**, is self ≈ 9.74e-2, adjacent ≈ 2.47e-2, opposite ≈ 4.83e-2. The module computes no `ΔS` at 64 or 128 MHz, because the solid route was not re-run there. `TH-14` step 3's bracket should compute this in-module.

**Scope held.** No copper, no Larmor-accuracy claim, no `TH-14` claim, and no band moved. Step 2's two-torus `Re Z = 0` identity is not this step, and the weekly's step-2 ruling has not landed, so the row stays 🟡.

**Step 3c (2026-09-13 19:30 slot) — the hole route's terminal-power excess is attributed.** This is §9 item 3. Tests only, in `tests/validation/test_th15_birdcage_pec_hole.py`.

**Construction.**
- A shared `_power_attribution` computes, for the P1 drive on each route, every scalar MPI-reduced:
  - per port, `½Re(V I*)` and `P_sheet,field`;
  - `fem_em_solver.ports.shares.terminal_form_deficit` (imported), per sheet and pooled;
  - the phantom loss, and `P_(Ω∖phantom)` = whole-domain `½∫σ|E|²` minus the phantom.
- The solid control became a module fixture that also runs one P1 field solve on `_four_port_rung`'s returned problem.
- The sheet dissipation uses `TH-19`'s imported package-builder forms; no `SpatialCoordinate` is involved.

**Window.** `20260914T005452Z_TH-15-step3c.log`: `-n 2`, complex build, `timeout -k 30 300`, `-s`, durable capture. 21 passed in 100.29 s, `[capture] rc=0`, elapsed 102 s. The module was not edited after the window, so it ran as committed (rule (i)).

**Asserted.**
- (a) `P_src − ΣP_sheet,field = P_phantom + P_(Ω∖phantom)`:
  - hole: rel **3.327e-15**, with `P_(Ω∖phantom)` = 0.000000000e+00 W (`:966–967`);
  - solid: rel **7.898e-15**, with conductor 4.482216632e-04 W (`:2787–2788`).
  - Both are identities of the discrete solve and hold by construction.
- (b) The solid's P1 pooled `C/terminal − 1` is **1.059204217e-02** against 2d's 1.059204217e-02, rel 1.490e-10 (`:2794`).
- (c) The hole's gates:
  - `‖S−Sᵀ‖/‖S‖` 1.706501807e-14;
  - σ(S) 0.999994234, 0.979295757, 0.979295715, 0.979167768;
  - spreads 0.0190 / 0.0094 / 0.0059 % (`:951–953`);
  - the tag-401 census is 19 826 facets, 0 not exterior;
  - the solid reproduces `LEG_D_S_MATRIX_10MHZ` to 1.158e-10 (`:2799`).

**Printed.**
- Hole (`:962–970`):
  - `P_src` 2.657078677e-03 W, `ΣP_sheet,field` 2.657014913e-03 W, phantom 6.376395218e-08 W.
  - Per port `½Re(V I*)` / `P_sheet,field` / `½|I|²Re Z_p` / per-sheet `C/terminal − 1`:

    | Port | `½Re(V I*)` (W) | `P_sheet,field` (W) | `½|I|²Re Z_p` (W) | `C/terminal − 1` |
    | --- | --- | --- | --- | --- |
    | P1 | +1.897496397e-03 | 8.172426518e-04 | 5.445424927e-03 | 1.058877362e-02 |
    | P2 | −6.089385529e-04 | 6.153864035e-04 | 6.089385529e-04 | 1.058867193e-02 |
    | P3 | −6.026299859e-04 | 6.090110429e-04 | 6.026299859e-04 | 1.058868152e-02 |
    | P4 | −6.089270811e-04 | 6.153748146e-04 | 6.089270811e-04 | 1.058867917e-02 |

  - Pooled `C/terminal − 1` **1.058874954e-02**.
  - Excess over the volume loss **7.693701287e-05 W = 2.8955e-02 of `P_src`** (predicted 2.8955e-02).
  - Exact split, from `V = V_s − I Z_p`: source part `½Re(V_s Ī₁) − P_src` = 7.342921323e-03 − 2.657078677e-03 = +4.685842647e-03 W; sheet part = −4.608905634e-03 W. They sum to +7.693701287e-05 W, which equals `C_total − terminal_total` = +7.693701287e-05 W.
- Solid (`:2783–2791`):
  - `P_src` 3.143759587e-03 W, sheets 2.695481546e-03 W, phantom 5.637745667e-08 W.
  - Per-sheet `C/terminal − 1`: 1.059184497e-02, 1.059279464e-02, 1.059329835e-02, 1.059187408e-02.
  - Excess 6.716202469e-05 W = 2.1364e-02 of `P_src`, again equal to `C − terminal` (the `PORT-16` 10 MHz gap).
- Hole/solid pooled ratio **0.9997**, against the 5× filing threshold: not above (`:2796`).

**Reading.** The terminal sum is `P_vol + (C_total − terminal_total)`, exactly. The step-3 anchor's "1 208×" is the terminal sheet form's ~1.06 % Cauchy–Schwarz deficit, which `PORT-16` measured on the solid, set against a phantom-only volume loss that is 1/1 200 of it. The hole's readout is indistinguishable from the solid's in `C/terminal − 1`. The known-issues entry is retired. The row's power sentence is left for the review to re-register on this term (rule (h)); nothing is re-registered in-slot. The natural-cavity control was not attempted.
