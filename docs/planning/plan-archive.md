# PROJECT_PLAN archive — compressed-out history

Verbatim blocks moved out of `PROJECT_PLAN.md` on 2026-08-04 to keep the
working plan scannable for the scheduled review and implementer sessions.
Each entry below is the full original text of a §7/§9 narrative whose chunk or
step had closed; the plan retains a compressed result block (gated numbers,
dates, log IDs) in its place. Nothing here is authoritative for *status* —
§7's tables are. Grep here first when a regression needs a closed step's
diagnosis, traps, or plan rationale; git history has everything else.

Ordering follows the plan file, bottom-up as archived within each batch.

---

## §9 On-deck review journal (2026-08-04, 10:30 review) — archived 2026-08-04

**4/4 slots fired — three completions, one parked incomplete.** 04:30 landed
`PORT-1` step 3b-i (the gapped fixture meshes exactly); 06:00 landed `PORT-1`
step 2f (the projection is the production drive; known-issues 8 retired);
07:30 landed `POST-1` step 1 (the ghost-cell double-count was real and is
fixed); 09:00 attempted `PORT-1` step 3b-ii and parked it — anchor +72.12%
out, diagnosed in-slot to the gap box's 1.83× oversized cross-section, not to
the solve. Thirteen harness logs on main, all registered in test-results.md;
the two 3b-ii logs live on the attempt branch.

**Audit of the three completions — all three stand, no demotions.** One
auditor per step, re-verifying the cited logs, bound provenance, and
`git show` for loosened assertions:
* `PORT-1` 3b-i — stands. Every identity number verbatim in the gate log; the
  `(0.955, 0.975)` conductor band is a never-passing *predicted* band replaced
  by a probe measurement with the failing probe log committed (the accepted
  precedent), moved up, not widened; the vacuity control (`< 0.9790`) is in
  the committed test; the one gate failure is the pre-existing domain-sizing
  known issue (entry dated 2026-07-27).
* `PORT-1` 2f — stands. The diagonal moved ungated→gated, a strict
  tightening; the Grover band is byte-identical to step 2e's; the step-3a S
  anchor change is an equivalent claim decoupled from the drive, not a
  rebaseline; the diagnosis-file pins reproduce bit-for-bit. Two audit
  nuances recorded: the "bad selection" regress log also contains two
  known-issues-2 convergence failures (pre-existing, so the no-regression
  conclusion holds), and post-2f the live fixture's S-matrix has no cross-run
  numeric pin — noted in the §7 entry, to be folded into the
  touchstone-threading step.
* `POST-1` step 1 — stands. Probe and gate numbers verbatim;
  `RE_CAST_DEFICIT_BAND` byte-identical across the commit; the new test
  allreduces before asserting. One untested escape hatch found
  (`_owned_cell_count`'s AttributeError fallback silently skips the ghost
  filter for tags-like objects) — pinned into step 2's plan rather than left
  loose.

**Plan work this review.** The parked negative got its rescope, and the two
successors the runs asked a review to scope got plans:
* **`PORT-1` step 3b-iii written** — the `gap_burial`/`gap_overhang` split
  plus re-measurement, with the fringe *ceiling* computed in the entry (a
  rectangular box cannot fringe below `1 − π/4 = 21.5%`, vs 45.5% as built) —
  so the item discriminates either way instead of hoping for green.
* **`PORT-1` step 3b-iv written** — facet tags on the arc-end discs, the
  route-2 prerequisite, mesh-only, an independent hedge against 3b-iii's
  negative branch.
* **`POST-1` step 2 written** — the guardrail fallback is rank-local
  (`phantom_fields.py:203–207`, confirmed by reading the code this review);
  sentinel-DG0 exact anchor, probe-first, step 1's structure.
* **`MAT-6` step 3 written** — the Dodd–Deeds re-gate under the projection
  the 06:00 run queued; the existing 5% is a hard ceiling, not a target.

**Assessment against §10 (step 5).** No gap. Criteria 1–3 still route through
`PORT-1` 3b, narrowed by 3b-ii's negative to: 3b-iii (fixture), then either
closure or 3b-v (facet voltage on 3b-iv's tags, scoped by a later review).
`MAT-6` step 3 closes the honesty gap 2f opened between the production drive
and the landed coil-loading number — it deepens an existing claim rather than
extending scope. Criterion 4's B1+ chunk stays a named, deliberately-held gap.
No chunk was invented: every new step was requested by a run's own hypothesis
or is the direct rescope of a parked negative.

---

## §7 PORT-1 full narrative (as of 2026-08-04) — archived 2026-08-04
...
>
> *(Audit note, 2026-08-04 10:30 review — the step stands; one bookkeeping
> gap it opened: the live fixture's S-matrix now has no cross-run numeric
> pin, only the identity gates plus the new Z gates, because the old pin was
> re-pointed at the logged Z. A projected-drive S baseline log should be
> captured whenever the S conversion is next touched — fold into the
...
>
> **Step 3b-iii — split the gap box's transverse overhang from its burial
> depth, then re-measure `Im Z₁₂` (written at the 2026-08-04 10:30 review;
> this is the rescope of 3b-ii and supersedes "do not re-attempt as
> written").** Two halves, one slot.
>
> *Mesh half.* In `two_torus_domain`, split `gap_clearance` into `gap_burial`
> (the ŷ half-length margin beyond `R·sin(gap_angle/2)` — must stay strictly
> positive, the tilted end planes cannot be met flush, 3b-i's recorded
> deviation) and `gap_overhang` (the transverse margin in the xz half-size
> `minor_radius + gap_overhang` — the one that costs cross-section,
> `io/mesh.py:853`). Defaults `gap_burial = gap_clearance = 1e-3` and
> `gap_overhang = 1e-3` so the default gapped call is byte-identical and
> 3b-i's gate is untouched. Then re-run the 3b-ii measurement (test file is
> on `attempt/PORT-1-step3bii-20260804T141200Z` — copy it from the branch,
> do not rewrite it) at small overhang.
>
> *Know the ceiling before measuring:* a rectangular box can never fringe
> below `1 − π/4 = 21.5%` of its cross-section (the corners of the
> circumscribing square), against 45.5% at overhang 1e-3. So this experiment
> *halves* the fringe, it does not remove it — if the +72.12% error scales
> with fringe area, expect roughly +30%, still outside the gate. That is the
> discrimination this step buys: gate green ⇒ the annulus was the whole
> story and 3b-ii closes; gate red with the error moved by ≈ the fringe
> ratio ⇒ the box-volume `V` is structurally unfit and the successor is the
> facet-integral voltage (3b-v, on 3b-iv's tags). Both outcomes are
> findings; neither is a tolerance problem.
>
> **Anchor:** `Im Z₁₂ = V₂/I₁` against `ωM₁₂ = 1.241755e+00 Ω`, the 3b-ii
> `MUTUAL_TOLERANCE = 10%` unchanged; reciprocity and the open-port
> precondition re-asserted at 3b-ii's measured scales (2.284e-04, 2.33e-03 —
> band from this probe, they should not degrade); 3b-i's exact-box identity
> re-asserted at `1e-9` on the new box (it holds for any rectangular box).
> **Print, do not gate:** full-box vs tube-shadow `V` on both ports (3b-ii
> measured 1.72 vs 0.75/0.69 × ωM — they must move toward each other or the
> fringe diagnosis is wrong), and `I′` vs the impressed 1.0 A (was 0.9151 A;
> the shortfall should close with the annulus gone). `Z₁₁` printed, never
> gated — the gap's series C is physics. **Negative control:** on record,
> cite, do not re-run — the full-box +72.12%
> (`20260804T140354Z_PORT-1-step3bii-costprobe.log`, on the attempt branch)
> and the unfragmented mesh's exact-zero `Z₁₂`
> (`20260731T213222Z_PORT-1-step1-costprobe.log`). **Cost:** standard,
> `-n 2`, `timeout 180` per command; measured on this exact fixture: mesh
> ~23 s + two solves ~16–18 s ⇒ ~60 s per configuration; probe first at
> `gap_overhang = 2e-4`, and try `0` only after the probe — a box face
> tangent to the tube surface (overhang exactly 0) is the OCC
> fragment-fragility class, and 2e-4 already puts the fringe at its ~24%
> floor. **Traps:** gap-wins piece policy (3b-i); fragment out-map
> positional, never absolute tags; stale FFCx lock after a killed run;
> pytest `-s`; complex build + `FEM_EM_REQUIRE_COMPLEX=1`,
> `tests/environment` first. **Does not close:** `PORT-1` — known-issues 3's
> matched-port fakes and the touchstone threading come after; `Z₁₁` stays
> ungated. **Negative result:** report `Z₁₂/ωM` at each overhang measured
> beside the fringe fractions (45.5% / ~27% / 21.5%), annotate here and
...
## §7 POST full narrative (as of 2026-08-04) — archived 2026-08-04
...
>
> **`POST-1` step 2 — the guardrail fallback is rank-local, and the drop set
> is unvalidated (written at the 2026-08-04 10:30 review, from step 1's
> measurement).** `_sampling_cells_with_interface_guardrails`
> (`post/phantom_fields.py:192–207`) falls back to the *full* tagged set
> **per rank** when that rank's interior set is empty — so a rank holding a
> sliver of a tag silently samples interface cells while every other rank
> samples interior-only. That is step 1's defect class one layer up:
> partition-dependent statistics, now from the classification rather than
> from ghosts. Step 1 measured the scoping fact (234 of 385 tagged cells
> dropped on the minority-tag rank at `-n 2`), so a sliver rank with zero
> interior cells is one partition away. Probe-first, no solves anywhere:
>
> 1. *Probe (against unfixed code, committed red like step 1's):* per-rank
>    interior/dropped counts on `POST-3` step 4's fixture at `-n 2`, `-n 4`,
>    `-n 8`; separately, a deliberately thin tag (a one-cell-thick slab via
>    `locate_entities` on a small box mesh) where the interior set is empty
>    *globally* — the two fallback regimes on record.
> 2. *Anchor, exact, not banded:* a sentinel DG0 field — value `k` on tag-k
>    cells, `100·k` on interface-adjacent tagged cells, adjacency computed
>    *in the test* from facet connectivity on the full tag set (ghosts
>    inform classification — step 1's rule) and allreduced. With a globally
>    non-empty interior set, production `prefer_interior=True` stats must
>    equal the interior-only reference: count exact, floats to `1e-12`,
>    identical at `-n 2` and `-n 4` — any interface contamination separates
>    by the sentinel's factor ~100.
> 3. *The semantics fix, if the probe shows a mixed regime:* the fallback
>    decision goes global — fall back to the full tagged set only when the
>    **allreduced** interior count is zero, and in that regime the stats
>    must equal the full-set (owned-only, step 1) reference, again
>    partition-invariant. Gate both regimes.
>
> **Negative control:** the probe's mixed-regime failure — production ≠
> interior-only reference by exactly the sliver rank's contaminated sample
> count, an integer identity like step 1's. If no affordable partition
> exhibits an empty-interior rank on the real fixture, say so (do not claim
> exoneration) and gate both regimes on the constructed fixtures. **Cost:**
> standard, no solves; step 1's four commands ran 6–10 s each; `-n 8` for
> the sliver probe stays inside the 12-core cap; `timeout 180`. **Traps:**
> standing complex-build list, `tests/environment` first;
> `cell_tags.indices/.values` rank-local — reduce before asserting; pytest
> `-s`; while in the file, pin the `_owned_cell_count` AttributeError escape
> hatch (step 1 audit caveat: a tags-like object without `.topology`
> silently skips the ghost filter — one regression assertion). **Does not
> close:** `POST-1` outright — this settles the guardrail's rank-safety and
> drop-set correctness on constructed fields; whether interface-adjacent
> statistics of a *solved* field are trustworthy stays open, and the review
> adjudicates the symbol. **Negative result:** the failure IS the
...
## §7 MAT-6 steps 1–2b full narrative — archived 2026-08-04
...
## §7 MAT-4 full narrative — archived 2026-08-04
...
## §7 TH full narratives — archived 2026-08-04
...
## §7 GEO-8 and GEO-9 full narratives — archived 2026-08-04
...
## §7 OPS-10 and OPS-11 full narratives — archived 2026-08-04
...
## §9 On-deck review journal (2026-08-11 18:00 digest + item list, incl. the operator-session item 0 of 2026-08-12) — archived 2026-08-12 (03:00 review)
...
Last reviewed 2026-08-11, 18:00 daily review. **Four of four slots landed
...
prediction was optimistic on its own arithmetic, not the fixture wrong. The
ten-point radial table hints the residual concentrates near the wire (9.46%
at r = 0.0080 vs 0.33% at r = 0.0200). 16:30: `MAT-6` step 7 Part 2c ✅ —
**closes step 7**: additivity defect **−0.080 pp** against the ≤ 0.5 pp
band (consistent with ADDITIVE), ΔR **0.8835%** on the combined fixture
under the unwidened 5% ceiling — sub-1% without step 8's slab knob — and
the O(h²) volume-deficit control re-asserted (3.99× shrink, reproducing
step 5). Step-3 audit: **both ✅ flips audited compliant, no demotions**
(one auditor each; logs, footers, elapsed times, enforced quantitative
asserts, and bound provenance verified — neither module touched between run
and flip). Auditor caveats, recorded: `POST-4` step 3's "anchor PASS first
execution" overstated — the anchor *script*'s first run exited 1 on two
secondary tolerances (disclosed in the entry and attempts.md; §7 wording
corrected this review); the additivity ratio is report-only by design (the
ΔR closed form and O(h²) shrink carry §4); the gate log's "4 passed" prints
once per rank. Step 2: tree clean all interval, no `recovered/*`;
`attempt/PORT-1-*` branches stay parked under the weekly licence
(2026-08-16). **Review decisions:** (1) `MAG-13` route — graded refinement
stays the named cheaper route, but the profile measurement comes first
(item 2 below; §7 step-2-profile entry): re-solve the solved rung and map
error-vs-radius densely before committing any mesh to grading; the next
uniform rung (h ≈ 0.001127, ~1.50 M cells) is queued only as the declared
brute-force spare (item 5; §7 step-2-rung-3 entry). (2) `MAT-6` follow-ons
scoped as **step 9** (ΔX box-truncation attribution — third W rung at
coarse wire, extending step 4's 0.9200 → 0.9849 sweep; item 3) and
**step 10** (slab × (box+wire) composition — does 0.8835% compose with
step 8's 0.2829%; item 4); authoring a ΔX *gate* stays deliberately
deferred until step 9 lands its attribution. (3) §5.4 ramp: step 7's
capability (coil loading vs Dodd–Deeds) is already demonstrated by
`examples/materials/01_dodd_deeds_coil_loading` (`mat:1`); `POST-4` step 3
*is* an example fix; `MAG-13` closed no gate — **no example chunk
mandated**. (4) Step-8 promotion stays deferred pending `ANS-1` (10:30
decision unchanged); nothing in §2 moves this interval. Waiting-on-you
delta: none — `ANS-1` replication and the runner-results ask stand; local
`main` is 14 ahead once this commit lands.

Previous interval's digest (2026-08-11 10:30), retained for the audit
trail: **Four of four slots landed
their item — a clean sweep: one chunk closed, one error budget closed, one
decisive negative, and the cost number three slots died for.** 04:30:
`MAT-6` step 7 Part 2 — the rescoped foreground recipe worked first time
(exit 0, 427 s); one projected loaded solve **372.9 s at `-n 4`, no OOM at
64 G**; the 300 s stop rule fired as written, so the 0.9843 additivity
reading stays open — the gate pair prices at ~797 s, outside one foreground
call at `-n 4`. 06:00: `EX-15` step 3 ✅ — **chunk closed**, 16 of 16
runnable examples checked, `PENDING_GUIDES` empty, negctl re-fired. 07:30:
`MAT-6` step 8 ✅ — `resolution_near` 0.005 → 0.0025 takes projected ΔR
**1.5834% → 0.2829%**; the residual is skin-depth resolution, not the coil
model, and the fixture is sub-1% on ΔR. 09:00: `POST-4` step 1 ✅ — decisive
negative: the ownership suspect **refuted** at 0/120 on every counter; the
23% enters at the Lagrange-P1 interpolation (interpolant 97.9755% vs source
0.008426%, **1.163e+04×**); step 2 correctly 🚫 skipped under its own
clause. Step-3 audit: **all three ✅ steps audited compliant, no demotions**
(one auditor per closure; logs, footers, elapsed times, and quantitative
assertions verified against the files). Minor auditor caveats, recorded:
`POST-4`'s "`E_src` bit-identical" is last-ulp (corrected in the §7 entry);
`MAT-6` step 8's 0.2829% is print-reported while the asserted bound stays
the inherited 5% (deliberate, rationale in-test); two superseded step-8
gate runs are journaled, not hidden. Step 2: tree clean all interval, no
`recovered/*`; `attempt/PORT-1-*` branches stay parked under the weekly
licence (2026-08-16). **Review decisions:** (1) the additivity measurement
routes through an `-n 8` cost probe (item 1), gate conditional on it
(item 4) — §7 step-7 route note; (2) promoting 0.0025 to the production
fixture is **deferred pending `ANS-1` adjudication** — it would move the
numbers the operator is replicating against (§7 step-8 note); (3) `POST-4`
re-scoped onto the measured locus — step 3 samples the source fields
(item 2), step 4 bounds the export-path artifact (item 5); (4) **the
2026-08-08 "unexplained harness termination" known-issues entry is
retired** — the wrapper logs show both deaths end with "waiting on the
background solve" then `exit=0` at the death minute, the same
background-and-end-turn trap 03:00 root-caused for `MAT-6`; `MAG-13` step 2
is un-blocked and re-queued (item 3), and the operator's host-observables
ask is **withdrawn** from Waiting-on-you. §5.4 ramp: step 8's capability is
already demonstrated by `EX-11` and the `mat:1` guide — no example chunk
mandated; `EX-15` and `POST-4` step 1 closed no field gate. Waiting-on-you
delta: **a push landed since 03:00** (origin/main is now `b6e994f`,
2026-08-10 18:00 review; 8 ahead once this commit lands) — `ANS-1`
replication remains, runner results remain never-observed, and the
`MAG-13` host-observables item leaves the list.

**Five ready items; items 1–4 independent; item 5 is the declared
brute-force spare on the same question as item 2** (either may run; a slot
reaching item 5 executes it as written even if item 2 already landed — a
completed rung is a real measurement either way, but the profile is the
cheaper decision-maker and goes first). Item 1 carries over unchanged (last
interval's spare, never reached); items 2 and 5 execute review decision (1)
...
2. ✅ **DONE 2026-08-11, 21:00 slot** — **`MAG-13` step 2 profile — where
   ...
   0.028] through `evaluate_vector_field_parallel` — instead of the ten
   points on record. **Anchor:** the straight-wire closed form
   `utils/analytical.py::straight_wire_magnetic_field` (B_θ = μ₀I/2πr)
   per radius; fixture identity first — cell count digit-identical to
   1 097 873 and global relative L2 reproducing **5.6494%** to its
   printed digits before any profile claim. **Negative control:** the
   ten-point table on record (9.46% at r = 0.0080, 6.33% at 0.0100,
   0.33% at 0.0200, 1.40% at 0.0240) — the dense profile must reproduce
   those four radii; a profile that contradicts its own coarse sample is
   a bug, not a finding. **Cost:** heavy, `-n 8`, foreground, tool
   `timeout` 660000 ms, container `timeout 590`; 275.3 s solve on record
   + seconds of sampling. Real build, **no complex mode**. **Traps:**
   never background; stale FFCx lock; point evaluation through
   `evaluate_vector_field_parallel`, never rank-local eval; `J·n ≠ 0` at
   the end caps stands unmeasured. **Scope boundary:** no mesh change, no
   graded mesh in-slot — this buys the *map*, the review scopes the mesh;
   `MAG-13` stays ✅ regardless. **Negative result:** a flat or
   wall-dominated profile refutes the near-wire hypothesis and the
...
3. ✅ **DONE 2026-08-12, 22:30 slot** — **`MAT-6` step 9 — ΔX box-truncation
   ...
   **truncation owns** step 7 Part 2c's unattributed ~1.6%, and the recovered
   ...
   no return:** mesh count, then one solve at `-n 4`, stop at > 300 s or
   OOM — W = 0.35's cell count is unmeasured (W = 0.25 was 300 591,
   2.17× at 18–22 s mesh / 81 s solve at `-n 4`, growth far-field-
   dominated; ~2–2.5× again is an estimate, not a measurement). **Anchor:**
   Dodd–Deeds `ΔX = −6.1586749e-01 Ω` and `ΔR = +3.2259615e-01 Ω` with
   step 2b's gates inherited unchanged (ΔR < 5% hard ceiling; ΔX sign and
   order of magnitude); the *reading* is the three-point truncation trend
   and its W → ∞ extrapolation, **reported, never gated**. **Negative
   control:** ΔR box-invariance on record (< 0.01 pp across W = 0.15 →
   0.25, drives identical to 5e-5) — ΔR moving with W says the fixture
   changed, not the truncation. **Cost:** heavy; complex build +
   `FEM_EM_REQUIRE_COMPLEX=1`; gate at `-n 2` per step 4's allreduce
   note, split by `-k` if four solves exceed one command. **Traps:**
   step 4's list — import geometry/tags/solves from the step-2b/step-3
   modules, restate nothing; `project_source=False` pins; FFCx lock;
   `ufl.max_value`. **Scope boundary:** no ΔX band is written or
   tightened — the extrapolated endpoint is the input to a *future* gate
   step, not a gate; `MAT-6` stays ✅. **Negative result:** an
   extrapolation that plateaus visibly below 1.00 says the residual is
   not truncation (finite-cross-section reference is the next suspect) —
...
4. 🟡 **ATTEMPTED 2026-08-12, 00:00 slot — probe stop rule fired; not
   ...
   **`MAT-6` step 10 — do the two sub-1% routes compose? (heavy; probe
   ...
   `resolution_near = 0.0025`, projected drive, `-n 8` under the 64 G
   cap. **Probe first:** mesh count, then one solve; stop at > 300 s
   or OOM — the composed cell count is unmeasured (combined 697 401 on
   record; step 8's knob added ~208 k near-region cells to a 210 k
   fixture; ~1 M is an estimate, and an OOM at 64 G is itself a per-cell
   memory reading). **Anchor:** Dodd–Deeds `ΔR = +3.2259615e-01 Ω`
   under step 2b's unwidened 5% ceiling; the reading is whether ΔR
   improves from **0.8835%** (box+wire, on record) toward the sub-0.5%
   the error budget predicts if the slab term composes — licensed by
   step 7 Part 2c's additivity verdict, and **reported beside the
   prediction, never gated to it**. **Negative control:** step 6's two
   16 G kill records and the σ-blind `ΔZ = 0`, cited never recomputed.
   **Cost:** heavy; solves ≥ 178–196 s each at `-n 8` on record for the
   un-composed fixture; container `timeout` sized after the probe, ≤ 590.
   **Traps:** complex build + `FEM_EM_REQUIRE_COMPLEX=1`;
   `project_source=False` pins; FFCx lock after any kill; never
   background; cap re-read (`memory.max` = 68719476736) before the
   solve. **Scope boundary:** measurement only — the production fixture
   and §2.1's landed numbers stay frozen pending `ANS-1` (step-8 review
   decision unchanged); `MAT-6` stays ✅. **Negative result:** a ΔR that
   does not improve — or worsens — under the composed knobs says mesh-
   knob additivity (ΔX, measured) does not imply error-term additivity
   (ΔR, assumed): that is the finding this step exists to catch; report
   both numbers, annotate, stop.

5. **`MAG-13` step 2 rung 3 — the < 5% wire by brute force (heavy; the
   spare; same question as item 2, other route).** Execute the §7
   step-2-rung-3 entry verbatim: mesh + one solve of h ≈ 0.001127
   (~1.50 M cells at 1.37× the 1 097 873 on record), `-n 8`, foreground,
   tool `timeout` 660000 ms, container `timeout 590`. **Anchor:**
   `straight_wire_magnetic_field` (B_θ = μ₀I/2πr), target **< 5.00%** at
   the rung the measured rate 1.174 prices at 5.00%; print the
   three-rung observed rate beside 1.174 and 1.10. **Negative control:**
   `MAG-13`'s analytic-Dirichlet-vs-plain-box separation and rate fit,
   on record, cited. **Cost:** heavy; **estimate 380–450 s if cost
   scales with cell count — an assumption, declared**: the 590 s
   ...
   not a failure. Real build, no complex mode. **Traps:** item 2's list
   verbatim. **Scope boundary:** `MAG-13` stays ✅ at its recorded
   numbers either way; a green < 5% annotates the entry and the MAG
   follow-up bullet; it does not retire the graded route, which stays
   the cheaper path for any future rung. **Negative result:** still
   > 5% on-rate, or exit 124, says the uniform route is exhausted
   inside the window — graded refinement becomes the only route; report
   the measured error and cost beside the prediction, annotate, stop.


## §9 On-deck review journal (2026-08-12 03:00 digest + item list, with the in-interval annotations) — archived 2026-08-12 (10:30 review)
...
4. **`MAG-13` step 2b — price higher-order B recovery on the solved rung
   ...
   **Anchor:** the closed form per radius with fixture identity enforced
   (cell count 1 097 873; DG1 ten-point relL2 reproduces 5.6494% — exit
   nonzero on miss); the reading is CG1 dense relL2 beside the DG1 record
   (4.7235%) at the pre-registered < 5.00% mark, and whether the eight
   staircase groups break into distinct values. **Negative control:** the
   DG1 path's own recorded numbers, reproduced in the same run. **Cost:**
   heavy, `-n 8`, 267 s + a seconds-scale mass solve; container
   `timeout -k 30 590`, real build, foreground. **Traps:** the profile
   entry's list; the recorded degree-2 divergence
   (`test_straight_wire.py:96`) was a P2 *solve*, not this projection —
   do not conflate; the projection is a fresh FFCx form — clear a stale
   lock after any kill. **Scope boundary:** no mesh change, no export
   change, no bound moved; `MAG-13` stays ✅; the graded route is not
   retired by any outcome. **Negative result:** CG1 recovery that does
   not beat DG1 (Gibbs-like overshoot at the wire is the plausible mode)
...
5. **`POST-4` step 5 — price the faithful-export route (standard;
   ...
   **Anchor:** `.bp` round-trip ≤ 1e-14 scaled median at the step-4 point
   sets (reuse the `EX-14`/`EX-17` machinery, restate nothing); the DG1
   read-back reads round-off where the P1 path reads
   51.17%/52.47%/20.18%. **Negative control:** the P1 refutation pin
   (midpoint median ≥ 10%, vertex ≤ midpoint) must still fire on the same
   run, else the fixture drifted and the comparison is void. Report file
   sizes (ADIOS2 `.bp` is a directory — tree-walk it, `stat` lies) and
   writer wall-clocks beside the fidelity table. **Cost:** standard,
   `-n 2`, `timeout -k 30 180`; solves and writes are seconds on record.
   **Traps:** step 1's list (reshape, complex build, rank-local counts,
   `-s`); ParaView rendering cannot be asserted headless — it becomes a
   dashboard Waiting-on-you item, never a gate. **Scope boundary:** no
   example switches its export in-slot; `POST-4` stays ✅; a review makes
   the call on the measured table. **Negative result:** an unfaithful DG1
...
6. **`MAG-13` step 2 rung 3 — the < 5% wire by brute force (heavy; the
   ...
   **Anchor:** `straight_wire_magnetic_field` (B_θ = μ₀I/2πr), target
   **< 5.00%** at the rung the measured rate 1.174 prices at 5.00%; print
   the three-rung observed rate beside 1.174 and 1.10. **Negative
   ...
   **Negative result:** still > 5% on-rate, or exit 124, says the uniform
   route is exhausted inside the window — graded refinement becomes the
   only route; report the measured error and cost beside the prediction,
   annotate, stop.

## §9 On-deck review journal (2026-08-12 10:30 digest + item list, with the in-interval DONE annotations) — archived 2026-08-12 (18:00 review)
...
## §9 On-deck review journal (2026-08-12 18:00 digest + item list, with the DONE annotations) — archived 2026-08-13 (03:00 review)

Last reviewed 2026-08-12, 18:00 daily review. **Four slots: four items
landed, all three ✅ flips audited compliant — the interval that cleared
the `PORT-1` diagnosis lineage.** 12:00: `PORT-1` step 3b-xvi third
attempt ✅ — the re-pointed control passes (−0.1658% vs < 5%), the solve
was bought, and the reading is **(converged at the feed)**: refined
estimator 0.895051, Δ = +0.0508 pp against the 0.5 pp band, so feed
discretisation is exonerated and the −3.02e-02 offset is **gap physics**
(Jin §10.4.2.1). Adjudication decision (3) fires with the physics label,
earned by measurement; the twice-failed item closed on its third
attempt. 13:30: `MAG-13` step 2b ✅ — CG1-projected `curl A` reads
1.9557% against DG1's 4.7235% and the < 5.00% mark, for 2.71 s (1.0% of
the solve); the staircase breaks 8/8, the O(h/r) structure is removed,
and the two-rung observation is second-order (p = 2.00 vs DG1's 1.22).
15:00: `POST-4` step 5 ✅ — the DG1/VTX export round-trips at exactly
0.000000e+00 against the 1e-14 bound where the P1 path reads
51/52/20% in the same run; the price is 10.49× disk and a *faster*
writer (0.74×). 16:30: `PORT-1` decision-(4) padding fit — both
pre-registered gates pass, but the box term must be quoted with its
exponent: **D∞ = +1.69 pp at p = 1.657, an effective-range
extrapolation** (pinning the dipolar p = 3 moves it across zero to
−1.43 pp; model uncertainty ~840× data uncertainty).
**Step-3 audit (one auditor per flip): all three COMPLIANT** — every
exit code derived from quantitative gates, negative controls genuinely
fired (step 2b's smoke rung 0/4 at exit 1; step 5's four failed runs
committed, not hidden), elapsed times in every footer, no bound moved.
One housekeeping action from the 3b-xvi audit: its two logs lived only
on the attempt branch — copied to `main` in this commit so the ✅
evidence is main-resident. One advisory nit folded into §7: the step-2b
probe's GATE-4 failure-path detail string is static (text misleads on
the failure path only; boolean and exit code correct).
Step 2: tree clean, no `recovered/*`. Branch dispositions: all three
`attempt/PORT-1-*` branches stay parked per adjudication decision (6) —
the lineage lands with its first ✅ gate, which is item 1 below.
**Review decisions:** (1) decision-(3)'s **re-pointing commit is queued
as item 1** — land the lineage branch (`d459af9`) and re-aim the
consistency gate at matched topology; the only commit licensed to touch
0.03/0.10, and it should need to touch neither. (2) The deferred
3b-i/3b-ii **port-pair gate is queued as item 2**, serial on item 1
(stated in the item); it states the box term as the effective-range
number, and **no fourth padding rung is commissioned** — the 10% mutual
bound dwarfs the 3.1 pp exponent-model spread (§7 3b-xi note). (3)
`MAG-13` step 2b's gate-adoption call is **deferred to the weekly review
pending a third rung** — commissioned as item 3 (step 2c); re-gating
every B-consuming test on a two-point observation is backwards. (4)
`POST-4`'s DG1-vs-P1 call: **DG1/VTX is the faithful-export direction on
the measured table**, but adoption is blocked on the operator's
one-click ParaView check of a `.bp` (added to Waiting-on-you); "P1 +
caveat" stays the standing answer until then, no example switches. (5)
§5.4 ramp: no example chunk — all three flips are measurements or
diagnoses on already-gated capability; none closed a new quantitative
capability gate. (6) Per §10 subgoal 3's standing instruction ("a §7
chunk ID should exist by the next weekly review"), **`TH-10` is
opened** — Larmor-regime lossy-sphere full-wave validation — with its
zero-solve anchor-authoring step queued as item 4; the backlog now
reaches both of the phase's live subgoals. Waiting-on-you delta: the
ParaView `.bp` check is new; `ANS-1` and the runner-results ask stand;
origin still at `b6e994f` (2026-08-10) — 33 ahead once this commit
lands.

Previous interval's digest (2026-08-12 10:30), retained for the audit
trail: **Four slots: two items landed and audited compliant, two spent
on 3b-xvi's mesh arms without buying the solve.** 04:30/06:00: the
scoped refinement factor refuted by measurement (24.70 cells across the
arc, not "~5"), then the `Thickness` cap cut the locality control
+35.46% → +16.32% with the collar diagnostic decisive (−0.17% outside a
5 mm dilation) — the review re-pointed the control to the dilated boxes
(band unchanged) and re-queued the item with the solve arm. 07:30:
`MAG-13` step 2 re-gate ✅ (smoke rung exits 1 at 0/4 gates; real rung
4/4 at exit 0 reproducing the record digit-identically, 263 s; profile
step restored 🧪 → ✅). 09:00: `MAT-6` step 10a ✅ — **fill-in
exonerated** (factor flops grow 1.693× for 1.28× cells against the ≥ 4×
verdict); ≥ 5.1× of the 9× lives in the numeric phase; step 10 goes to
the weekly review with a scoped discriminator. Both audits compliant.
*(Full text in `docs/planning/plan-archive.md`, archived 2026-08-12,
18:00 review.)*

**Five ready items. Items 1 → 2 are the one serial pair (stated in
item 2); items 3, 4 and 5 are independent of them and of each other.**
Item 5 is the declared spare: step 2b made it the expensive route to a
...
1. ~~**`PORT-1` decision-(3) re-pointing**~~ — **done 2026-08-12 (19:30 run)**.
   ...
   *Original item text follows.* Land
   ...
   in `tests/validation/test_port_gap_voltage_impedance.py`; the
   `_validate_material_map_tags` hunk it carries is already on `main`
   via `OPS-13` — resolve that already-applied conflict trivially, per
   the 3b-xiii note. Then execute adjudication decision (3): re-aim the
   consistency gate to compare at **matched topology** (gapped estimator
   against a gapped-fixture reference — never gapped-vs-closed), and
   keep the −3.0224e-02 gapped-vs-closed record in the test's
   docstring and the §7 entry with its earned label (**gap physics**,
   Jin §10.4.2.1; measured by 3b-xvi at Δ = +0.0508 pp under 1.57×
   feed refinement), citing
   `20260812T170128Z_PORT-1-step3bxvi-mesh6e4-repointed.log` and
   `20260812T170317Z_PORT-1-step3bxvi-solve6e4.log`. **Anchor:** the
   re-pointed gate green at `-n 2` reproducing the recorded digits
   (estimator 0.894543 / 0.894022, quarter-spread 0.7 pp; reciprocity
   ‖Z−Zᵀ‖/‖Z‖ < 1e-9 unchanged); the whole port validation suite green
   is the landing gate. **Negative control:** the old gapped-vs-closed
   deviation printed as record (−3.0224e-02; refined −2.9674e-02),
   gated on nothing. **Cost:** standard, `-n 2`; 3b-xv's suite envelope
   is 475 s — container `timeout -k 30 590`. **Traps:** the 3b-xiii
   list (FFCx lock after any kill, complex build +
   `FEM_EM_REQUIRE_COMPLEX=1`, `tests/environment` first, σ via the DG0
   field never a global); the step-2 audit note — the reciprocity
   residual is normalised by ‖Z‖, move the normalisation to |Z₁₂| in
   the same commit if the diagonal's magnitude changes; this commit is
   the **only** place `REACTION_CONSISTENCY_TOLERANCE` (0.03) /
   `MUTUAL_TOLERANCE` (0.10) may move, and a matched-topology
   comparison should need **neither** moved — a re-point that only goes
   green with a loosened bound is a finding, not a fix. **Scope
   boundary:** `PORT-1` stays 🟡 — the pair gate (item 2) is what
   advances it; no birdcage work; the σ-on-driven-wire successor stays
   retired per adjudication decision (2). **Negative result:** a
   matched-topology comparison that still misses 0.03 says the physics
...
2. ~~**`PORT-1` deferred 3b-i/3b-ii port-pair gate**~~ — **done 2026-08-12
   ...
   *Original item text follows.* Author and run the
   ...
   form `ωM₁₂ = +1.241755 Ω` at `MUTUAL_TOLERANCE` (10%), carrying the
   two stated systematics **by name in the assertion message**: the
   PEC-box term (D∞ = +1.69 pp at p = 1.657, effective-range — never
   quoted without its exponent, §7 3b-xi) and the gap-physics offset
   (−3.0224e-02 vs the closed-loop control; refined −2.9674e-02; Jin
   §10.4.2.1). Reciprocity is the standing tripwire per decision (3):
   ‖Z−Zᵀ‖ (normalised per item 1's convention) < 1e-9; S symmetric and
   passive at Z₀ = 50 Ω per the step-2 machinery. **Anchor:** `ωM₁₂`
   from the step-1/2 filamentary kernel (reuse, restate nothing).
   **Negative control:** step 1's unfragmented-mesh record — `Z₁₂ ≡ 0`
   exactly on the non-conforming fixture, the total-separation floor
   every step cites; the measured deviation must sit inside 10% where
   the blind fixture reads 100%. **Cost:** standard, `-n 2`; mesh
   ~21 s + ~20 s/solve on the step-1 record; container
   `timeout -k 30 590`. **Traps:** the 3b-xiii list; the estimator is a
   stated-path convention (adjudication decision (5)) — the gate names
   its path, it does not claim path-independence; the gapped fixture
   only — the closed loop is a diagnostic control, never a gate
   fixture. **Scope boundary:** this gate validates the two-torus
   `∫E·dl` machinery with its systematics stated; birdcage ports and
   B1+ stay held (the drain note below); `PORT-1` advances but closes
   only per its §7 done-when. **Negative result:** a miss beyond 10%
   with both systematics stated says §10 subgoal 2's 20-step estimate
...
3. ~~**`MAG-13` step 2c — a third rung for the CG1 recovery rate**~~ —
   ...
   **(heavy;
   ...
   1 097 873 at h = 0.00125 — an assumption, declared**), then both
   recoveries (DG1 interpolation and CG1 L2-projection, `cg` + `gamg`,
   rtol 1e-12) on the recorded 45-radius grid — extend
   `scripts/probes/mag13_step2b_recovery.py`, restate nothing.
   **Anchor:** `utils/analytical.py::straight_wire_magnetic_field` per
   radius; the reading is the three-point CG1 rate beside the two-point
   observation p = 2.00 (if it holds, this rung's CG1 dense relL2 reads
   ≈ 3.92% = 7.8411% × (1.7678/2.5)²) and the DG1 rate beside 1.22.
   Identity gates first, driving the exit code: the smoke rung
   (h = 0.0025) reproduces CG1 7.8411% / DG1 10.9806% to printed
   digits; the 1.1 M-cell rung is cited from record, not re-solved.
   **Negative control:** the DG1 path on the same new rung (predicted
   ≈ 7.2% at p = 1.22) — the CG1/DG1 gap must persist, else step 2b's
   finding was rung-specific. **Cost:** heavy, `-n 8`; smoke rung
   ~30 s + new rung ~95 s if cost scales with cells (271 s at 1.1 M);
   container `timeout -k 30 590`, foreground. **Traps:** the step-2b
   list (the recorded degree-2 divergence was a P2 *solve*, not this
   projection; fresh FFCx form — clear a stale lock after any kill;
   exit code from gates, verdict broadcast rank-safe); fix the GATE-4
   failure-path detail string in the same probe edit (§7 nit, 18:00
   review). **Scope boundary:** `MAG-13` stays ✅; gate adoption
   (moving `compute_b_field` to CG1 recovery) is the **weekly review's
   call** with this number in hand — not this slot's. **Negative
   result:** a third point off the p = 2 line says the two-point
   reading was pre-asymptotic — report the three-point table, annotate
...
4. ~~**`TH-10` step 1 — author the Larmor anchor**~~ — **done 2026-08-13
   ...
   *pointwise* quasi-static miss at the `TH-8` fixture is 3.52e-02 — a linear
   ...
   `ε_c = εᵣ − j·σ/(ωε₀)`, the `TH-1` `e^{+jωt}` convention) in a
   time-harmonic incident field at 64/128 MHz, plus a self-check probe
   run through the harness. Consult `docs/references/jin-fem-3e/`
   before deriving; cite chapter/eq. numbers in the §7 annotation.
   **Anchor:** the quasi-static limit — at ka ≪ 1 the series' interior
   field reproduces `TH-8`'s validated closed form to < 0.5% at the
   `TH-8` fixture's parameters; and the empty limit — εᵣ → 1, σ → 0
   returns the incident field to machine precision. Both drive the
   probe's exit code. **Negative control:** a conjugated-convention
   evaluation (`+jσ/(ωε₀)`) must break the quasi-static-limit check by
   orders — the sign convention is load-bearing, per the `TH-1`
   formulation note. **Cost:** smoke, `-n 1`, seconds; numpy arithmetic
   only — no mesh, no solve, no DolfinX dependency beyond the container.
   **Traps:** `scipy.special`'s spherical Bessel functions do not
   accept complex arguments — use closed-form low-order expressions or
   recurrence and check stability with a truncation sweep (print the
   last-term bound, never assume an N); textbook series are usually
   `e^{−iωt}` — conjugate on import or the loss term flips sign and the
   negative control will catch it. **Scope boundary:** anchor authoring
   only — no solver work, no `TH-10` gate, the chunk stays ⬜/🟡; later
   steps are deliberately unscoped until this lands (§7 entry).
   **Negative result:** a series that cannot reproduce the `TH-8` limit
   is a finding about conventions or the reference formula — journal
   the mismatch in the §7 `TH-10` entry with both formulas' provenance;
   report, stop.

5. **`MAG-13` step 2 rung 3 — the < 5% wire by brute force (heavy; the
   declared spare; same question as step 2b's route decision, other
...
## §9 interval digest, archived 2026-08-13 (10:30 review, run interactively after the scheduled slot's 529)
...
## §9 On-deck review journal (2026-08-13 10:30 digest + item list, with the DONE/blocked annotations) — archived 2026-08-15 (18:00 review)
...
## §10 epitaphs and examples/benchmarks notes (2026-08-09 weekly) — archived 2026-08-16 (operator session)
...
## §10 pace ledger, week 2026-08-02→08-09 — archived 2026-08-16 (operator session)
...
## §9 drain-instruction history (3b-xv/3b-xvii updates) — archived 2026-08-16 (operator session)
...
## §9 On-deck done items 1–4, full text (2026-08-15/16 runs) — archived 2026-08-16 (operator session)
...
2. ~~**Hygiene pair — the two standing ride-alongs, promoted (standard,
   ...
   **145 884 cells** vs ~145.9 k on record, azimuthality **PASS** at
   ...
   against the coarse/fine power errors already printed in
   `20260813T170337Z_TH-10-step4-power-n2.log` and re-run that file at
   `-n 2` (~100 s on record). (b) `MAG-13`'s exit gate: code-verified
   but never bitten in-slot — run the 26 s `RES=0.0025` smoke the
   2026-08-13 10:30 review priced, asserting the gate passes live and
   the cell count matches the probe record. **Anchor:** (a) power relL2
   strictly decreasing coarse→fine at 64 MHz with the recorded digits
   unmoved; (b) exit-gate assertions green on a live run. **Negative
   control:** the files' existing controls; no bound moves. **Traps:**
   never adjust the recorded digits to make monotonicity hold — if it
   fails, that is a finding about the coarse record; report and stop.
...
3. ~~**`EX-18` doc repairs — retire the docrefs known-issues entry
   ...
   `check_example_doc_references.py`. **Anchor:** the guide pass reports
   **0 violations** (3 on record,
   `20260813T200522Z_EX-19-docrefs.log`); the known-issues entry retires
   with this commit. **Trap:** the checker's overall exit code can stay
   1 if > 48 h-old outputs make the stale-reference pass red — that
   pass is environmental (known-issues, extended 2026-08-15); gate on
...
4. ~~**`EX-20` — package S-parameter sweep example (standard).**~~ —
   **done 2026-08-16, 00:00 run**: `examples/ports/02_package_sparameter_sweep.py`
   ...
   directly). New file under `examples/ports/` in the `ports:` runner
   group, guide to the `EX-15` bar, combined XDMF of the solved fields.
   **Anchor:** the step-4 record reproduced through the example path
   within a pre-stated 1% band — raw 0.894543, corrected 0.939849,
   `‖S−Sᵀ‖/‖S‖ = 2.5494e-05`, `‖S‖₂ = 0.861449`
   (`20260813T183606Z_PORT-1-step4-packagegate.log`). **Negative
   control, executed in-run:** the deprecated heuristic S printed
   beside it, differing by ≥ 2e-3 (3.078e-01 on record), its
   `DeprecationWarning` shown. **Cost:** standard, `-n 2`; step 4's
   solves price it at ~160 s + export; container `timeout -k 30 500`.
   **Traps:** complex build via the runner; item 3 may rename the
   `EX-18` guide headings in the same interval — no file conflict, but
   run the checker's guide pass on the new guide. **Scope:** two-torus
   only; no birdcage, no B1+, no Touchstone claims. **Negative
   result:** report, annotate the §7 `EX-20` row, stop.

---

## §9 On-deck review journal (2026-08-15 18:00 digest) — archived 2026-08-16 (operator session)

Last reviewed 2026-08-15, **18:00 review — the first review to execute
since 2026-08-13 10:30.** Five scheduled reviews in between produced no
work: 08-13 18:00, 08-14 03:00 and 10:30 died on API 529s (three
byte-identical 98-byte logs — the credit-exhaustion evidence, unchanged),
and 08-14 18:00 through 08-15 10:30 never ran because **the host was down
~23.8 h** (`uptime -s` 2026-08-15 14:50 CDT; 11 implementer slots and
3 reviews lost with *no log at all*). An absent log file means host-off; a
98-byte log means no-credits — the two failure modes now coexist in the
record and are told apart by log size, never by absence. The queue drained
2026-08-13 21:00 after items 1–4 landed in four consecutive slots and
item 5 (`OPS-16`) blocked on the allowlist; twelve implementer slots then
idled and journalled exactly per the drain instruction, and the 15:00
slot's container restart after the outage is why the grid is green now.
**The interval's landings (all before the drain):** 13:30 `PORT-1`
step 4 ✅ — the package path reads the solved field: raw 0.894543
(record ±2e-3), corrected 0.939849 inside the unmoved 0.10,
`‖S−Sᵀ‖/‖S‖ = 2.5494e-05`, `‖S‖₂ = 0.861449`, heuristic control differs
by 3.078e-01 and is deprecated behind a kwarg; 7 passed 153.9 s. 15:00
`EX-19` ✅ — first Larmor example, all four `TH-10` records reproduced
through the example path, max drift 1.7e-04 vs a pre-stated 1% band,
24 s. 16:30 `GEO-14` step 1 ✅ — the pre-registered **RESOLUTION** band:
64 MHz relL2 3.643% → 1.781% at the priced 55 251-cell mesh (observed
rate 1.77 in h — no floor); the shared-faceting-floor hypothesis refuted;
negative control exact (128 MHz record to 1.8e-05). 19:30 `TH-11`
step 1 ✅ — 64 MHz feasible at the 10 MHz price (solves 30.5 + 27.0 s),
identities to 1e-14, ΔR deviation from Dodd–Deeds grows 1.5834% →
**+10.2698%**, unattributed pending the resolution rung (1.26 cells/δ).
21:00 `OPS-16` 🚫 — unexecutable by any scheduled session
(`Edit(scripts/automation/**)` is `ask` ⇒ headless denial); the design is
complete in attempts.md and unblocking is the operator's call.
**Step-3 audit (one subagent auditor per landing): `PORT-1` step 4,
`EX-19`, `GEO-14` step 1, `TH-11` step 1 all COMPLIANT — no demotions.**
Caveats on record, none §4-blocking: `GEO-14`'s band classification is
printed, not asserted (§4 is carried by its five asserts); `EX-19`'s
docrefs red also contains 24 environmental stale-reference failures the
journal under-reported (known-issues entry extended this review — gate on
the guide pass, not exit 0); `TH-11`'s §7 step-1 fixture wording was
internally inconsistent (resolved in the §7 annotation; the run's
baseline-rung choice was correct under cost-probe-first); `PORT-1`
step 4 carried a disclosed rank-safety ride-along that was load-bearing
for its own `-n 2` control.
**Review decisions:** (1) **`PORT-1` closed ✅** — the done-when
(reciprocity below a stated tolerance on a real identity) is met through
the package entry point: `‖S−Sᵀ‖/‖S‖` gated at 1e-3, measured 2.5494e-05,
on a field-derived S. §2.2 narrowed and retitled; CLAUDE.md's summary
sentence updated. Claims stay two-torus-only; 3b-xv's follow-up and
birdcage ports remain the weekly review's. §6's Phase-4 row still reads
🟡 — §6 is weekly-owned, flagged for tonight's weekly review (01:30).
(2) **`GEO-14` closed ✅** on the refuted hypothesis — the re-aim at
`MAG-13`'s wire is declined: `MAG-13`'s own rung ladder already
attributes the wire residual to resolution (rung 3: 3.7372% at 1.52 M
cells, still descending on the p = 2 line), so a re-aimed step would
re-measure a measured thing. (3) **`TH-11` steps 2–3 scoped** (the
resolution rung and the 30 MHz mid-transition point; entries in §7) —
queue items 1 and 6. (4) §5.4 ramp: `PORT-1` step 4 newly gated the
package-sweep capability and no example calls
`run_n_port_sparameter_sweep` (`EX-18` demonstrates the lower-level
Z→`sparameters_from_impedance` route), so **`EX-20` commissioned** —
queue item 4, rubric in the item. (5) The drained slots' standing
ride-along candidates are promoted to real queue items (2 and 3) so a
drained slot has a legal floor. (6) The outage-visibility gap — a 23.8 h
host outage is visible only as absent log files — is **not queueable**:
any launcher heartbeat touches `scripts/automation/**`, the `OPS-16`
lesson, so it goes to Waiting-on-you as an operator decision, not to §7.
Standing weekly-review items unchanged (systematics-composition question,
`MAG-13` gate adoption, `MAT-6` step 10, `POST-4`/ParaView,
birdcage-ports/B1+ hold; plus the §6 Phase-4 row refresh flagged above).
Waiting-on-you delta: the `OPS-16` unblock decision and the
outage-visibility gap added; ParaView `.bp` check, `ANS-1`, and the push
stand (origin at `b6e994f`, 2026-08-10 — ~50 ahead once this lands).

Previous interval's digest (2026-08-13 10:30, run interactively after
the slot's 529), retained for the audit trail: **five slots, five
landings — `TH-10` went from anchor-only to closed** (step 2: 64 MHz
3.643%; `EX-18`; step 3: 128 MHz 1.826%; `MAG-13` rung 3: 3.7372%;
step 4: power 3.629%); all audited COMPLIANT; `TH-10` closed ✅ and
`TH-11`, `GEO-14`, `EX-19`, `OPS-16` commissioned, `PORT-1` step 4
scoped. *(Full text in `docs/planning/plan-archive.md`, archived
2026-08-15, 18:00 review.)*

---

## §9 immediate-sequencing narrative (as of 2026-08-15) — archived 2026-08-16 (operator session)
...
## §7 ANS-1 plan + closure narrative — archived 2026-08-16 (operator session)
...
## §7 EX-3 closure + plan — archived 2026-08-16 (operator session)
...
## §7 EX-2 plan + closure + caller audit — archived 2026-08-16 (operator session)
...
## §7 EX-1 closure, demotion note + plan — archived 2026-08-16 (operator session)
...
## §7 EX-4 closure narrative — archived 2026-08-16 (operator session)
...
## §7 EX-14 plan + closure — archived 2026-08-16 (operator session)
...
## §7 EX-18 plan + closure + doc repairs — archived 2026-08-16 (operator session)
...
## §7 EX-17 plan + closure — archived 2026-08-16 (operator session)
...
## §7 EX-16 plan + negative result — archived 2026-08-16 (operator session)
...
## §7 EX-13 plan + negative result — archived 2026-08-16 (operator session)
...
## §7 EX-15 plan + three step closures — archived 2026-08-16 (operator session)
...
## §7 EX-4…EX-12 backfill plans + closures (incl. EX-11) — archived 2026-08-16 (operator session)
...
## §7 EX ramp-accounting snapshot (2026-08-09 weekly) — archived 2026-08-16 (operator session)
...
## §7 PORT-1 full step narrative, steps 1–4 and the 3b lineage with adjudications (as of close, 2026-08-15) — archived 2026-08-16 (operator session)

**`PORT-1` — Real port excitation from the solved field** ✅
*(**Chunk closed by the 2026-08-15, 18:00 review.** The done-when below —
reciprocity below a stated tolerance, a real failable identity replacing the
placeholder arithmetic — is met through the package entry point:
`run_n_port_sparameter_sweep` reads the solved field, `‖S−Sᵀ‖/‖S‖ =
2.5494e-05` against the 1e-3 gate, `‖S‖₂ = 0.861449`, raw mutual 0.894543
reproducing the gated record within 2e-3, the retiring heuristic differing by
3.078e-01 and kept reachable behind a `DeprecationWarning` (step 4, audited
COMPLIANT; `20260813T183606Z_PORT-1-step4-packagegate.log`, 7 passed
153.9 s). The claim is the two-torus fixture through this entry point —
**not** birdcage ports, S11/Z_in (step 2b's electric-energy excess stands),
or B1+; those and 3b-xv's licensed follow-up remain the weekly review's.
known-issues 3 stays open for its defect (1).)*
> Gap-voltage lumped ports, the standard approach for MRI coils at 64–300 MHz:
> excite one port per solve with an impressed gap source; recover `V_i = −∫E·dl`
> across each port gap and terminal currents from the solved field; assemble the
> Z-matrix column-by-column from N single-port solves; convert
...
## §7 POST-4 full narrative, steps 1–5 (chunk closed 2026-08-12) — archived 2026-08-16 (operator session)
...
## §7 POST-3 step narratives 1–5 + reciprocity decision — archived 2026-08-16 (operator session)
...
## §7 POST-1 adjudication + steps 1–6 full narrative — archived 2026-08-16 (operator session)
...
## §7 MAT-6 steps 1–10a full narratives — archived 2026-08-16 (operator session)
...
## §7 MAT-4 steps 1–3 full narratives — archived 2026-08-16 (operator session)
...
## §7 GEO-14 full entry (closed 2026-08-15) — archived 2026-08-16 (operator session)

**`GEO-14` — the shared ~3% geometry floor: discriminate faceting from
resolution** ✅ *(commissioned 2026-08-13, 10:30 review; **closed by the
2026-08-15, 18:00 review** — disposition paragraph at the end of this
entry. Entry placed here
...
>
> **Closed ✅ by the 2026-08-15, 18:00 review.** §4 is satisfied by the run
> itself (audited COMPLIANT: five quantitative asserts including the exact
> 128 MHz negative control, harness log, 26.5 s recorded); the question the
> chunk was commissioned to answer is answered decisively, and a refuted
> hypothesis is a closed measurement, not a failed one. The re-aim at
> `MAG-13`'s wire is **declined**: `MAG-13`'s own rung ladder already
> attributes the wire residual to resolution (rung 3, 2026-08-13: 3.7372% at
> 1.52 M cells, still descending on the p = 2 line —
> `20260813T140146Z_MAG-13-step2-rung3-n8.log`), so a re-aimed step would
> re-measure a measured thing on a fixture whose own data is finer-grained.
> One caveat carried, not hidden: the RESOLUTION/FLOOR/MIXED classification
> itself is printed, not asserted — all three readings were legitimate
> outcomes, and the five asserted gates are what §4 stands on. Surface-graded
> sizing (the never-scoped step 2) remains available to any future chunk that
> finds an actual floor; nothing here licenses it.

---

## §7 TH-11 step 1–2 plans + execution journals — archived 2026-08-16 (operator session)
...
>
> **Review annotation, 2026-08-15, 18:00 review — the step-1 fixture wording
> above was internally inconsistent, and the run resolved it correctly.** The
> plan text names "W = 0.25 / `resolution_near` 0.0025 (ΔR 0.8835% on record)"
> but prices the step at "70–75 s at `-n 2`", which is the *step-3 baseline*
> (W = 0.15 / wire 0.002 / near 0.005, 138 619 cells, ΔR 1.5834%); the
> 0.8835% belongs to the 697 401-cell combined-knobs mesh (178–196 s/solve at
> `-n 8`), and W = 0.25 with near 0.0025 as literally written was never
> meshed. The run took the priced rung per cost-probe-first — the only rung
> with a like-for-like 10 MHz ΔR record on the same drive — and journalled
> the discrepancy rather than hiding it. The wording above stands as the
> historical record of the error; steps 2–3 below name their fixtures
> unambiguously.
>
> **Step 2 — the resolution rung at 64 MHz (scoped 2026-08-15, 18:00
> review; the caveat step 1 named, executed).** Re-run step 1's probe module
> with `resolution_near` = 0.0025 on the same W = 0.15 / wire 0.002 fixture —
> the `MAT-6` step-8 ladder rung: **417 914 cells**, δ/h = 2.52 at 64 MHz
> (vs 1.26 at step 1's rung), and at 10 MHz its projected-drive ΔR record is
> **+3.2168355e-01 Ω, 0.2829%** off Dodd–Deeds
> (`20260811T125226Z_MAT-6-step8-gate-final.log`). **Anchor (identities
> only, as step 1):** complex-power residual < 1e-9 on both solves; σ = 0
> dissipation exactly +0.0; drive control < 1e-24; cell count asserted at
> the step-8 exact 417 914. The 10 MHz pair is *not* re-solved — the step-8
> record is the cited baseline. **Pre-registered reading (printed, never
> asserted — all three outcomes are findings):** at 10 MHz this knob moved
> ΔR by −1.3005 pp; if the 64 MHz deviation stays > 8% (moves ≲ 2 pp from
> +10.27%), the deviation is physics-dominated (displacement current,
> retardation) and a gated trend step becomes scopeable; if it collapses
> below 3%, the resolution term owned most of it and the trend needs finer
> rungs before any claim; between ⇒ mixed, report both terms. **Tier/cost:**
> heavy, `-n 2`, one command; step 8 priced the rung at 35.6 s mesh +
> 176.8 + 170.0 s loaded/free at 10 MHz and step 1 measured 64 MHz at
> 10 MHz prices ⇒ ~390–450 s; container `timeout -k 30 900`, Bash-tool
> timeout 660000 ms, stop rule 300 s/solve unchanged (if a solve exceeds
> it, that is the finding — report, stop, do not refine or retry in-slot).
> **Traps:** step 1's list — `_solve_projected` pins `FEM_FREQUENCY_HZ`, so
> reuse step 1's freed-f helper; `-k 30` mandatory; a killed run leaves a
> stale FFCx lock (clear `~/.cache/fenics`); Dodd–Deeds is the comparison,
> not the reference — never call the deviation an error. **Scope boundary:**
...
## §7 TH-10 steps 1–4 full narratives (chunk closed 2026-08-13) — archived 2026-08-16 (operator session)
...
## §7 GEO-4 step 1, GEO-8…GEO-13 full narratives — archived 2026-08-16 (operator session)
...
## §7 MAG open-follow-ups journal bullets — archived 2026-08-16 (operator session)
...
## §7 MAG-13 step-2 lineage full narratives (profile, re-gate, 2b, 2c, rungs, diag) — archived 2026-08-16 (operator session)
...
## §7 MAG-6 steps 1–5 full narratives — archived 2026-08-16 (operator session)
...
## §7 MAG-16 full narrative — archived 2026-08-16 (operator session)
...
## §7 OPS-15 full narrative — archived 2026-08-16 (operator session)
...
## §7 OPS-12 full narrative — archived 2026-08-16 (operator session)
...
## §7 OPS-14 full narrative — archived 2026-08-16 (operator session)
...
## §7 OPS-13 full narrative — archived 2026-08-16 (operator session)
...
## §2 current-state narrative as of 2026-08-15 (proxy-era history, 2.1–2.5) — archived 2026-08-16 (operator session)
...
### 2.2 The S-parameters: field-derived on the two-torus fixture only (heuristic retired 2026-08-13)

*(Retitled 2026-08-15, 18:00 review — the paragraphs below are the dated
history of how the original headline, "The S-parameters are heuristic, not
computed", was narrowed to nothing and finally retired. The residual truth:
the package's S-parameters are honest **only** on the two-torus validation
fixture; no coil or birdcage has ports at all, and every figure carries the
two named systematics of `PORT-1` 3b-xviii. The last paragraph is the
current state.)*
...
*(2026-08-13, `PORT-1` step 4 — **the heuristic is retired from the package
path, and the headline sentence above with it.** `run_n_port_sparameter_sweep`
now takes its port voltages from the gap-voltage route on the solved field
(systematics via `ports/systematics.py`); on the two-torus fixture it
reproduces the gated record (raw 0.894543 vs 0.894283 ± 2e-3, corrected
0.939849 inside the unmoved 0.10) with `‖S−Sᵀ‖/‖S‖ = 2.5494e-05` against the
1e-3 reciprocity gate and `‖S‖₂ = 0.861449 ≤ 1`. The old heuristic remains
reachable only behind a kwarg that raises `DeprecationWarning`, and its S is
asserted to *differ* from the field route's by 3.078e-01 — a negative control,
not a fallback. `PORT-1` closed ✅, 2026-08-15 review. **What this does not
say:** the honest S exists on one validation fixture with two named, labeled
systematics (3b-xviii); no coil or birdcage geometry has ports; nothing here
touches B1+ or SAR. Any S-parameter figure quoted for a coil is still
unsupported.)*
...
## §9 On-deck done items 1–4 + interval recap, 03:00 to 10:30 2026-08-19 — archived 2026-08-19 (10:30 review)
...
## §9 On-deck done items 1–5 + interval recap, 10:30 2026-08-19 to 03:00 2026-08-20 — archived 2026-08-20 (03:00 review)

**Interval recap as it stood (written by the 10:30 review):**
...
2. ~~**`TH-12` step 3 — is the degree-2 `W_e` explosion generic to
   incompatible drives, or coil-feed-specific? (standard).**~~ **Done
   2026-08-19, 13:30 slot** — the reading is **COIL-SPECIFIC** at the
   pre-registered ≤ 10×-on-both band: smoke (incompatible `J·n ≠ 0`
   drive) moves `W_e/W_m` **1.155×** across order, sphere (imposed
   field) **1.015×**, coil **3.426e+07×**. `J·n ≠ 0` is not sufficient
   and the incompatible-drive hypothesis is refuted. All anchors and
   the negative control green;
   `20260819T183425Z_TH-12-step3-warm.log`, 8 passed / exit 0 / 10 s at
   `-n 2`, reproduced digit-for-digit by the cold window. **For the
   review:** the entry names a confound the step cannot separate (the
   three fixtures' baseline `W_e/W_m` spans 2.16 / 1.07 / 6.7e-6), so
   "the feed model injects it" and "only a `W_m ≫ W_e` fixture can
   display it" both survive; a magnetically-dominated fixture with a
   compatible drive, or the absolute gradient content of `E`, would
   split them. Original scope, for the record: Execute
   the §7 `TH-12` step-3 entry verbatim (commissioned 2026-08-18 18:00
   review; the full rubric — anchor bands ≥ 1e3× vs ≤ 10× on the
   cross-order `W_e/W_m` move, the record reproductions, cost, traps —
   lives there). Smoke + sphere fixtures at degrees 1/2, `-n 2`,
   `timeout -k 30 400`; the coil's 6.7e-6 → ≈ 229 ratio pair printed
   beside them for comparison. **Scope:** mechanism attribution only —
...
5. ~~*(spare)* **`EX-22` — restore the absent example artifacts
   (heavy).**~~ **Done 2026-08-19, 19:30 slot** — chunk closes ✅ and the
   standing `stale=24` docrefs backlog every example run has reported
   against is **0**: `dead=0 guide=0 stale=0 exit=0`, the checker's first
   `exit=0` under the `OPS-19` contract. Three runner commands, all exit
   0 at `-n 2` (`-e 1,2,4` 230 s, `-e 5,6` 151 s, `-e mri:1` 6 s);
   `EX-14`/`EX-17` round-trips exactly 0.000e+00, gauge 0.0003% /
   0.0033%, h-rate 1.1009, and `mri:1`'s ungated record reproduced
   digit-for-digit without a gate being invented for it. **For the
   review:** the entry's "two runner commands" was split to three (the
   mag group was unpriced — the only all-mag timing on record predates
   examples 05/06); and the queue is now **drained** — all five items
   done, so the next slot hits the drain instruction unless the review
   tops it up. Original scope, for the record: Execute the §7 `EX-22` entry verbatim (commissioned
...
## §9 On-deck done items 1–5 + drained item 6 + interval recap, 03:00 2026-08-20 to 18:00 2026-08-21 — archived 2026-08-21 (18:00 review)
