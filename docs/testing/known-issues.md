...
| **Correction + ruling (2026-09-23 03:00 review, `log-pathologist` reading)** | The window had **two** reds, not one — `2 failed, 15 passed` on all 16 ranks (`:8969–8970`): the guard above and `test_pinned_scikit_rf_version_is_exact` (`ModuleNotFoundError`, `:8892–8895`), the deliberate `OPS-60` red of the entry below, picked up because entry 7's command runs `tests/environment` first. Neither is physics. The four rungs' true sizes are 139 140 / 745 938 / 1 819 094 / 3 828 494 unknowns (ZMUMPS `N`, `:2114`, `:4204`, `:6281`, `:8450`). **Step 3e is not re-run**: its readout is complete and usable. `xl-ledger.md` row filled; `xl-pending.md` entry 7 marked RUN. The next `ANS-4` window on the same module (3f, 09-23) ran 18 / 18 green under the fixed guard (`20260923T070007Z_ANS-4-step3f.log:4923–4924`, `:5046`). |
...
| **Scope (2026-09-23 03:00 review)** | Session-specific, not an outage: the scheduled 03:00 review reached the container through a plain `scripts/testing/run_and_log.sh …` call the same morning (`20260923T080435Z_EX-57-next.log`, the census's normal exit 2), and the 02:00 `xl` window ran on the rebuilt image. Slots proceed as normal; the 2026-09-20 entry's rule (plain harness call, retry once, never park) still applies. |
...
| **Update 2026-09-23 (03:00 review)** | **The rebuilt image carries the pin.** The 09-23 02:00 `xl` window — the `fem-em-solver-xl` service runs the same `fem-em-solver:latest` image and is force-recreated before every window — ran this test green on all ranks: `[OPS-60] skrf=2.1.0` (`20260923T070007Z_ANS-4-step3f.log:271–283`), with `dolfinx=0.11.0.post0` unchanged (`:287`) and the congruent-cut cell records reproduced exactly (116 118 / 592 550, `:4888`, `:4533`) — no mesher drift from the rebuild's unpinned packages. Not retired yet: the ordinary `fem-em-solver` service, which every slot uses, has not been shown to run the rebuilt image (a container not recreated after the build keeps the old one). §9 item 33 runs the test there and retires this entry. |
...
| **Public sensitivity table (2026-09-21, `PORT-21` step 1b)** | On `main`: `tests/validation/test_port21_feed_sensitivity.py` (gated on `FEM_EM_PORT21_FREQUENCIES_MHZ`); the table as printed is in `docs/testing/logs/20260921T094925Z_PORT-21.log:5795–5810` (10 / 64 / 128 MHz, `-n 2`, degree 1). Self-class `\|ΔS\|/\|S\|`: (a) width-told-as-`w/(1+κ)` 3.411e-02 / 1.552e-02 / 6.327e-03 (falls with f); (c′) halved conductor grading on the C4-congruent cut, against its own congruent control′, 1.885e-02 / 1.538e-02 / 1.679e-02 (non-monotonic). Our own convention sensitivities only — no absolute `S₁₁` is licensed by it. |
