...
### 🟡 OPEN 2026-09-07 (`TH-15` step 2d, 12:00 implementer slot) — `tests/validation/test_port_package_sparameters.py` fails **three digit-reproduction records at `-n 4`** and passes all 17 at `-n 2`: the records are `-n 2` records and the module is rank-width-sensitive at 1e-4, four decades above its 1e-6 reproduction bands
