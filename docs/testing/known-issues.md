...
### 🟡 OPEN 2026-09-10 (`OPS-43` (d) gate, 06:00 implementer slot) — the complex MUMPS smoke solve is **not bit-reproducible across processes at `-n 2`**: 2 of 22 identical child solves drifted by 1 ULP, so the (d) gate's pre-registered `unset == set` bit-identity is intermittently red for a reason that is **not** `FEM_EM_SOLVER_PROGRESS`

| | |
|---|---|
| **Test** | `tests/solver/test_solver_progress_inert.py::test_solver_progress_variable_is_inert_on_result_and_visible_only_when_set` — **not on `main`** (parked with the slot's tree). Fixture = `tests/solver/test_time_harmonic_smoke.py`'s cylinder at h = 0.03 (1 405 cells, 2 004 dofs), axial drive, degree 1, default MUMPS options; each solve a separate child `mpiexec -n 2` launched from rank 0 with a PMI-scrubbed env. |
...
| **Update 2026-09-10 — the cause is identified and the entry is ready to retire** | `ANS-4` step 2d ran the order-matched comparison (`20260910T180911Z_ANS-4-step2d-capture.log`, 14 passed / 1 failed, 7225 s, `-n 16`, peak 290.2 GiB). At **degree 2** — which is HFSS First Order by `ANS-5` — on a mesh of comparable unknown count to AED's own converged run, the disagreement falls to **≈ 1 %** from the 4–7 % the degree-1 fixture showed, and the degree-2 sequence converges (successive change 1.19 % → 0.62 %, ratio 1.90). **The 2026-09-06 reasoning was wrong**: it excluded element order because AED's own order shift was small, but AED's answer was order-insensitive because *its* run had converged and ours had not — the asymmetry was the signal, not an exclusion. Numeric ruling in gitignored `docs/private/ans4-step2d-2026-09-10.md`. **Retire this entry when the 2026-09-13 weekly banks the verdict** — the reading the evidence supports is AGREE at 64/128 MHz, and that is a review's to bank, not a run's. |
