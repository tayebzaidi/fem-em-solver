...
### `ANS-1` / `ANS-3` example scripts raise `ModuleNotFoundError` at import since the 2026-08-28 artifact rename (found 2026-08-30 by the `ANS-4` slot, verified by the 18:00 review)

| | |
|---|---|
| **Scripts** | `examples/ansys_benchmarks/loop_over_lossy_slab_10MHz/01_loop_over_lossy_slab_10MHz.py:96` — `__import__("01_materials_01_dodd_deeds_coil_loading")`; `examples/ansys_benchmarks/two_torus_gap_ports_10MHz/03_two_torus_gap_ports_10MHz.py:88` — `__import__("02_ports_02_package_sparameter_sweep")`. |
| **Symptom** | `ModuleNotFoundError` on `./run_examples.sh -e ans:1` / `ans:3` (not yet observed in a harness log — `EX-37`'s negative control records it); the files on disk are `examples/materials/01_dodd_deeds_coil_loading.py` and `examples/ports/02_package_sparameter_sweep.py`. |
| **Verified at** | `18dcd5a`, by reading: `git log -S'01_materials_01_dodd_deeds_coil_loading' -- examples` returns only `67e4c1c` (2026-08-28, "prefix every example's output artifacts with group and number"), whose diff on the two ANS scripts is the `BASENAME`/docstring rename — the global substitution also rewrote the import string. `EX-30` leg (ports) had both cases green on 2026-08-26 (`ans` census 2 → 0). No `ans:1`/`ans:3` run since. |
| **Cause** | A text substitution that did not distinguish the artifact stem from the module stem. `ANS-4` (2026-08-30) imports by path (`importlib.util.spec_from_file_location`) and is unaffected. |
| **What it does and does not invalidate** | The ✅ runnable-half records (`ANS-1` 08-09, `ANS-3` 08-16 / re-run 08-26) were produced before the break and stand; the *example path* is what is broken. `EX-36` leg (ports + ans) would red on both, so that leg is serial on the fix. |
| **Resolves with** | `EX-37` (§9 item 1, 2026-08-30 18:00 review): restore the two strings, negative control on unpatched `main`, re-run both, census. |
