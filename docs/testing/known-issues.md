...
### The `MAT-6` step-3 baseline coil mesh no longer builds inside a scheduled slot: `test_coil_loading_degree2.py` spends its whole 570 s ceiling meshing, where the full module ran in 543 s on 2026-08-18 (`TH-13` step 3a, 2026-08-31)
...
| **Symptom** | The run is killed by the container-side `timeout -k 30 570` with the log showing collection, the instant `SKIPPED` of `test_the_memory_exponent_measured_on_the_fine_rung`, and then `test_the_mesh_is_the_mat6_step3_baseline` still in progress on all eight ranks. No assertion was reached and no number moved; `-k 30` worked — the container was healthy afterwards, zero stray `python3`, `memory.max` 137438953472. |
| **Cause** | **Not diagnosed.** The comparison that makes it a regression: on 2026-08-18 (`20260818T200059Z_TH-12-step2-full.log`) the *entire* module — this mesh, the 138 619-cell degree-1 and degree-2 solves, and the 61.94 GiB factorization — completed in **543 s** at the same `-n 8` on the same ceiling. Today the mesh alone exceeds ~560 s. The obvious suspect is `OPS-18` (2026-08-22), which moved the image to gmsh **4.15.2-git-657c8e9** from the 0.7.2 image's gmsh; that upgrade is known to have moved mesh *counts* on several fixtures (the two-torus 184 919 → 184 176, the straight wire 145 900 → 147 235), so a mesh *time* change on the most expensive generator in the tree is plausible but unmeasured. Machine load is the other candidate and was not sampled. |
...
| **Fix** | Not attempted. Cheapest first: (a) time the generator alone in a smoke-tier probe (`MeshGenerator.loop_over_half_space_domain` at the step-3 parameters, no solve) on this image, which prices everything else; (b) if it is the generator, a review decides whether the module gets a cached mesh or a coarser baseline rung — both move a *record*, so neither is an implementer's call in-slot; (c) if it is load, re-run the owed regression in a quiet slot at the unchanged ceiling. |
