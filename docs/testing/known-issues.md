...
### `mag:1`'s `EX-14` VTX read-back check is not exercised on the current image — `adios2` 2.12.1 has no top-level `ADIOS` (2026-09-14, 07:30 implementer slot)

| | |
| --- | --- |
| **Symptom** | `examples/magnetostatics/01_straight_wire.py` prints `⚠ VTX round-trip read-back unavailable: AttributeError: module 'adios2' has no attribute 'ADIOS'`, then `Note: XDMF files were still created …`, and exits 0. `_check_vtx_roundtrip` catches the error and returns `False` instead of raising, so the "written `.bp` reproduces the in-memory field" check never runs. The `.bp` directories are still written. |
| **Verified at** | `8f5f656`: `20260914T125223Z_EX-57-straight-wire.log:285` (flagged) and `20260914T125244Z_EX-57-straight-wire-control.log:265` (unflagged), both Status 0 at `-n 2`. Container probe `20260914T125326Z_EX-57-adios2-probe.log:34` reads `adios2 2.12.1 has ADIOS: False` (`/usr/local/lib/adios2/__init__.py`). |
...
| **Scope to check** | `mag:2` (`02_circular_loop.py`) carries the same `EX-14` read-back pattern (entry at line ~4612 above). Not re-run this slot. |
