...
| **Ruled 2026-09-09 18:00 review, ruling (3) — the pin stays a pinned path set, and the declaration is written late rather than replaced by a pattern** | Matching `ans:*/metrics.json` by glob was considered and **rejected**: it trades one line of maintenance per benchmark case for a weaker guarantee, and the whole point of `EX-29`'s pin is that widening the freshness exemption has to be *declared by the chunk that widens it*. `ANS-4` and `ANS-2` step 1 each committed a `metrics.json` without making that declaration; the fix is that the declaration is now written, late, with its provenance. **Disposition: `OPS-44`** (§7, §9 item 2) adds both paths and extends the provenance comment to name both chunks and both dates, with the anchor being the module's own set-equality identity against an *independent* re-derivation from `git ls-files` (five members, not a count) and the negative control being that an untracked artifact still does not gain the exemption and that dropping any pinned path still turns the identity red. `OPS-42`'s deselection was correct — a chunk should not edit a record it does not own — and this row **retires in `OPS-44`'s commit**, not before. One citation corrected in passing: the `AssertionError` this row cites at `:210` is the `short test summary info` restatement; the assertion itself is at `20260909T213349Z_OPS-42.log:194–195`. No band, no exit-code contract and no artifact on disk moves. |
