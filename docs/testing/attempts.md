...
## 2026-09-23 11:40 UTC — EX-57 mesh:11 setup figure — complete (§9 item 38, 06:00 CDT implementer slot)

- **Executor:** interactive session acting as `example-runner` (foreground, no sub-agents spawned).
- **What landed:** `write_setup_figure` call added to `examples/meshing/11_birdcage_sixteen_ring_sheet_longitudinal.py`, right after `_measure_ring(SCALED_LEG_COUNT, orientation="longitudinal")` builds the mesh and before `_report_safely`/any printed timer. Region names: `1`/`101`-`116` conductor (bulk + 16 uncut leg boxes, "leg L{i} conductor (uncut)"), `3` phantom, `PORT_LOWER+i`/`PORT_UPPER+i` for `i` in `m["ring_ports"]` named "ring port P{i} lower (inner, u < R)" / "upper (outer, u > R)" — the `port` `CLASS_COLOURS` keyword. Air (`2`) hidden, phantom translucent. Slice `normal=(0,0,1)`, `origin=(0,0,0.5*COIL_LENGTH-1e-3)` — the same 1 mm off-symmetry-plane offset `mesh:9` measured necessary on this same both-rings-cut, 16-leg construction (`COIL_LENGTH` imported from `tests.mesh.test_birdcage_port_tags`, already the module `RING_RADIUS` came from — no gate edit). Title 98 chars, under the 130 limit.
- **Figure:** `examples/meshing/figures/meshing_11_birdcage_sixteen_ring_sheet_longitudinal_setup.png`, 466 KiB (≤ 600 KiB). Viewed before committing: 3-D panel shows all 16 copper legs, the blue translucent phantom, and red port-box halves on both rings — the 3-D panel carries the informative load, as the guide caption says; the z-slice panel shows only the 16 leg cross-sections and the phantom (the thin port boxes do not resolve at that cut height), consistent with the caption.
- **Guide:** `## Setup figure` section added to `examples/meshing/11_birdcage_sixteen_ring_sheet_longitudinal.md` right after "### Scope" (same position `mesh:9`'s guide uses), full PNG filename, caption states which tags each colour class covers (conductor: `1`, `101`-`116`; phantom: `3`; port: `117`-`148`/`217`-`248`) before describing the geometry.
- **Runs (all through `run_and_log.sh`, chunk `EX-57`):**
  - pre-census (setup figures): `20260923T110918Z_EX-57.log`, exit 2, `examples=56 ok=25 missing=31 broken=0`.
  - flagged run (`FEM_EM_SETUP_FIGURES=1`, `-n 2`, `timeout -k 30 300`): `20260923T110927Z_EX-57.log`, Status 0, 96 s. Reproduced `270728` cells (record `270728`, ratio `1.000000`), terminal-area state census 2 states, low state `10 of 32` (record `10 of 32`), all imported identity/band assertions green.
  - post-census (setup figures): `20260923T111136Z_EX-57.log`, exit 2, `examples=56 ok=26 missing=30 broken=0` — matches the predicted delta made before the flagged run.
  - docrefs census: `20260923T111150Z_EX-57.log`, exit 2 (age-driven staleness only, unrelated to this item), `dead=0 guide=0 stale=13`; guide pass (56/56 runnable examples carry all required headings).
  - unflagged re-run (default path, no `FEM_EM_SETUP_FIGURES`): `20260923T111200Z_EX-57.log`, Status 0, 86 s, same digits (`270728` cells, `10 of 32` low state); PNG mtime unchanged, confirming the default path never touches the committed figure.
- **Deviation from the item text:** the pre-census docrefs reading was not captured separately before the source edit (only the post-run docrefs census was taken); the post reading is clean (`dead=0 guide=0`), so no reference regression is possible from this item's own change, but a true before/after docrefs pair is not on record — noted for the auditor.
- **Commit:** one commit — example script, guide, PNG, `test-results.md`/harness logs, §9 item 38 flip, this entry.

## 2026-09-23 11:15-11:43 UTC — EX-57 mesh:12 setup figure — complete (drained-queue fallback, §9 item 38 append)

- **Executor:** interactive session acting as `example-runner` (foreground, no sub-agents spawned). Docker's socket was denied to a bare `docker compose ps` in this sandbox; the harness-wrapped `docker compose exec` calls (the supported path) worked throughout.
- **Census (before):** `docker exec check_example_setup_figures.py --next` (run by the coordinator's parent session, `20260923T111523Z_EX-57.log`) read `ok=26 missing=30 broken=0` and pointed at `examples/meshing/12_birdcage_f_human_rung.py` — used as the "before" reading per the coordinator's instruction, not re-run.
- **What landed:** `write_setup_figure` call added to `examples/meshing/12_birdcage_f_human_rung.py`, right after `_build_rung(F_HUMAN_RING_RADIUS, scale_sizing=False, keep_mesh=True)` builds branch B (the fixture) and before branch A is built or anything is printed. Region names: `1`/`101`-`116` conductor (bulk + 16 uncut leg boxes, "leg L{i} conductor (uncut)"), `3` phantom, `PORT_LOWER+i`/`PORT_UPPER+i` for `i` in `fixture["ring_ports"]` named "ring port P{i} lower/upper" — the `port` `CLASS_COLOURS` keyword. Air (`2`) hidden, phantom translucent. Slice `normal=(0,0,1)`, `origin=(0,0,0.5*coil_length-1e-3)` where `coil_length = COIL_LENGTH * (F_HUMAN_RING_RADIUS/RING_RADIUS)` (`COIL_LENGTH`/`RING_RADIUS` imported from `tests.mesh.test_birdcage_port_tags`, the same module `mesh:11` used) — the same 1 mm off-mid-plane offset `mesh:9`/`mesh:11` measured necessary on this same both-rings-cut construction. Title 90 chars, under the 130 limit.
- **Figure:** `examples/meshing/figures/meshing_12_birdcage_f_human_rung_setup.png`, 573 KiB (≤ 600 KiB). Viewed before committing: 3-D panel shows all 16 copper legs, both end rings split into red port-box halves, and the blue translucent phantom; the z-slice panel shows the graded triangulation around the leg conductors inside the tetrahedral fill — matches the guide caption.
- **Guide:** `## Setup figure` section added to `examples/meshing/12_birdcage_f_human_rung.md` right after "### Scope" (same position as `mesh:9`/`mesh:11`), full PNG filename, caption states which tags each colour class covers (conductor: `1`, `101`-`116`; phantom: `3`; port: `117`-`148`/`217`-`248`) before describing the geometry.
- **Runs (all through `run_and_log.sh`, chunk `EX-57`):**
  - flagged run (`FEM_EM_SETUP_FIGURES=1`, `-n 2`, `timeout -k 30 400`): `20260923T111718Z_EX-57.log`, Status 0, 217.6 s (real build, 39108 gmsh chatter lines filtered). Reproduced branch-B `504642` cells (record `504642`, ratio `1.000000`) and meshed/CAD `0.965414` (gate `0.95`); branch-A `204977` cells (record `204977`) and meshed/CAD `0.893028`; negative-control assertion `control["cad_ratio"] < CAD_MASS_GATE <= fixture["cad_ratio"]` held; all imported bands green.
  - post-census (setup figures): `20260923T112127Z_EX-57.log`, exit 2, `examples=56 ok=27 missing=29 broken=0` — matches the predicted delta (`ok` 26→27, `missing` 30→29) made before this item started.
  - docrefs before: `20260923T112146Z_EX-57.log`, exit 2, `dead=0 guide=0 stale=12` — read on the **pre-edit tree** (`git stash push` on the two changed files, census run, `git stash pop` to restore, per operator standing rule (b)/§9 template "docrefs census captured BEFORE and AFTER the edit").
  - docrefs after: `20260923T112155Z_EX-57.log`, exit 2, `dead=0 guide=0 stale=12` — unchanged, as predicted (adding a guide section and a committed PNG does not move dead/stale reference counts).
  - PNG `git add`ed between the post-census and the unflagged re-run, per the OPS-59 (b) identity convention.
  - unflagged re-run (default path, no `FEM_EM_SETUP_FIGURES`): `20260923T112205Z_EX-57.log`, Status 0, 214.3 s, same digits (branch B `504642`/`0.965414`, branch A `204977`/`0.893028`); PNG md5 unchanged (`0a8a6b1d74bd19225f0adfe589f2bdf0` before and after), confirming the default path never touches the committed figure.
- **Deviations from instructions:** none. The docker-socket denial reported in the prior turn (bare `docker compose ps`) was a sandbox restriction on that specific unwrapped command, not on the harness path; all compute in this item ran through `run_and_log.sh` as instructed.
- **Commit:** one commit — example script, guide, PNG, `test-results.md`/harness logs, §9 item 38 append, §7 `EX-57` row addendum (`mesh:11` + `mesh:12`), this entry.

## 2026-09-23 12:30-13:05 UTC — EX-57 mat:1 setup figure — complete (drained-queue fallback, 07:30 CDT implementer slot)

- **Queue:** §9 On deck items 33-38 all DONE at slot start; took the standing drained-queue fallback. `check_example_setup_figures.py --next` → `examples/materials/01_dodd_deeds_coil_loading.py` (`20260923T123233Z_EX-57.log`: `examples=56 ok=27 missing=29 broken=0`; docrefs `dead=0 guide=0 stale=12 exit=2`, pre-edit tree).
- **Executor protocol breach (for the review):** the `example-runner` spawned in the foreground did **not** execute. It spawned a *nested* `example-runner` in the background and returned at once. That nested agent then **backgrounded its flagged harness window** and ended its own turn. This breaks implementer-run.md step 3 (foreground executors; never background a harness window). No harm this time: the backgrounded window returned a footer, and only one executor ever ran (checked with ListAgents; no stray python3/mpiexec in the container at 12:51). The slot owner waited in the foreground, then took over from the censuses onward. The nested runner also left ~40 short iteration logs (12:31-12:41, mostly renderer probes and `timeout` 124s). They are committed as-is because they are harness records. **Hypothesis:** the example-runner definition needs an explicit "never spawn agents; never `run_in_background`" line. Spawn-prompt wording alone did not bind the nested agent.
- **What landed:** `write_setup_figure` call in `examples/materials/01_dodd_deeds_coil_loading.py`, placed after the mesh timer print and before the two solves. Tags: `1` wire (conductor), `2` air (hidden), `3` slab (translucent). Slice normal `x`, through the loop's symmetry plane. `## Setup figure` guide section; the slot corrected the caption colour (the slab renders purple, not blue) and noted that the wire cross-sections are pixel-scale in the slice panel (guide-only edit, module untouched after its windows). PNG `examples/materials/figures/materials_01_dodd_deeds_coil_loading_setup.png`, 445 KiB (≤ 600).
- **Runs (all `run_and_log.sh`, chunk `EX-57`):** flagged `-n 8`, `timeout -k 30 280`: `20260923T124124Z_EX-57.log`, exit 0, 281 s, 418888 cells, ΔR 0.2747 % vs filament / 0.3895 % vs finite wire. Unflagged `-n 8`, `timeout -k 30 430` (recorded width, guide §2): `20260923T125310Z_EX-57.log`, exit 0, 256 s. The four printed result lines are identical to the flagged run (`diff` empty) and the PNG md5 is unchanged. Post setup-figure census `20260923T125238Z_EX-57.log`: `ok=28 missing=28 broken=0`, as predicted. Docrefs `20260923T125244Z_EX-57.log`: `dead=0 guide=0 stale=11 exit=2`. `stale` fell 12 → 11 because the flagged run refreshed `mat:1`'s stale ParaView output. `OPS-59` (b) `tests/unit/test_doc_reference_exit_codes.py`: 20 passed (`20260923T125744Z_EX-57.log`).
- **Docker socket:** the bare harness command worked every time. Every harness call wrapped in a pipe or redirect (`| grep`, `> /dev/null`) was denied within about 1 s (`125224Z`, `125225Z`, `125234Z`, `125257Z`, `125303Z`: Status 1, footers only). This looks like the sandbox exclusion matching only the bare command form. Next slots should call the harness bare and read the log afterwards.
- **Next:** census `--next` on `main` for the following figure.
- **Addendum 12:59 UTC:** after `a60d87a` landed, the nested `example-runner` (a9aa095a…) showed as `running` again: it had resumed after its backgrounded window finished. The slot stopped it with TaskStop before it touched the tree (clean tree, HEAD `a60d87a`, no python3/mpiexec in the container). So a backgrounded executor can come back and act *after* the slot owner has committed. That makes the nesting breach a correctness risk, not just a timing one.

## 2026-09-23 14:00-14:10 UTC — EX-57 mat:2 setup figure — complete (drained-queue fallback, 09:00 CDT implementer slot)

- **Queue:** §9 On deck items 33-38 all DONE at slot start; took the standing drained-queue fallback. `check_example_setup_figures.py --next` → `examples/materials/02_lossy_sphere_sar.py` (`20260923T140022Z_EX-57.log`: `examples=56 ok=28 missing=28 broken=0`). The bare `docker compose ps` preflight was denied on the socket; the bare harness call worked first time (known-issues 2026-09-20 pattern).
- **Executor:** one foreground `example-runner`. No nested agent this time (checked with ListAgents after return: no subagent running). It committed `cabb2f3` by itself, including the §7 `EX-57` row addendum. The slot checked the commit against the logs: all three windows exit 0, elapsed 56 / 52 / 9 s as the row states, `[setup-figure]` line only in the flagged log.
- **What landed:** `write_setup_figure` call in `examples/materials/02_lossy_sphere_sar.py`, placed after `solve_seconds` is captured. Tags: `1` sphere (translucent), `2` air (hidden). Slice at `z = 0`, the equatorial plane of the interior `E_z` comparison. `## Setup figure` guide section. PNG `examples/materials/figures/materials_02_lossy_sphere_sar_setup.png`, 468 KiB (≤ 600).
- **Runs (all `run_and_log.sh`, chunk `EX-57`, `-n 2`, `timeout -k 30 300`):** pre-census `20260923T140312Z_EX-57.log` (`ok=28 missing=28 broken=0`) and docrefs `20260923T140318Z_EX-57.log` (`dead=0 guide=0 stale=11 exit=2`). Flagged `20260923T140323Z_EX-57.log`: exit 0, 56 s, 74020 cells, mean-SAR error 3.422 % / 3.536 % (σ = 0.05 / 0.57), separation 4.850x, σ = 0 control exactly 0.0 W. Post-census `20260923T140441Z_EX-57.log`: `ok=29 missing=27 broken=0`, as predicted. Docrefs `20260923T140448Z_EX-57.log`: `stale` 11 → 10, exit 2. Unflagged `20260923T140454Z_EX-57.log`: exit 0, 52 s, same printed records. `OPS-59` (b) `20260923T140551Z_EX-57.log`: 20 passed.
- **Stopped after one item.** The §9 drain rule says take one figure, then stop and journal.
- **Next:** census `--next` on `main` for the following figure (27 missing).

## 2026-09-25 09:30-09:50 UTC — WF-7 step 3 (§9 item 39, chain H5) — complete (04:30 CDT implementer slot)

- **What landed:** new `tests/validation/test_wf7_f_human_sar10g_hotspot.py` (`FEM_EM_WF7_STEP3=1`) and an additive `phantom_resolution=None` keyword on `_build_f_human` (`None` adds no generator key). No `src/`. SAR through `post.sar` only (`mass_averaged_sar`, `mean_sar`, `build_density_field`, `QUADRATURE_DEGREE` 16 imported). STOP-rule timings and build time are MAX-reduced so the reduced-lattice decision is identical on every rank.
- **Window:** `20260925T093315Z_WF-7-step3.log`, heavy, `-n 8`, `timeout -k 30 1150`, durable capture, Status 0, 352 s, 4 passed. 537 546 cells / 22 594 phantom cells at 0.0075 m, build 133.33 s, 19.052 GiB summed `ru_maxrss`. c* = r 45 mm, z = z_c on the az_ref (11.25°) ray. (i) coverage 9.592e-14 power / 1.297e-13 mass; (ii) C16 worst 0.4904 % (R_7); (iii) mirror 0.0502 %; (iv) all 30 balls contained. P17 control 96.40 % spread (predicted ≫ 5 %, printed). Step 2's `|B₁⁺|` on the refined mesh: 0.0513 % / 0.0456 %, 215/215 points. Per-ball 2.5–4.7 s; the 25 s STOP rule did not fire.
- **Disclosure:** c* sits on the mirror plane (the az_ref ray), so the mirror point equals c*. Identical ball mass 10.001116 g confirms this. The mirror identity read here is therefore cw SAR vs ccw SAR at the same point. That is the pre-registered identity, but not an off-plane reflection test.
- **Rule (a):** `20260925T093918Z_WF-7-step3-ruleA-step2.log`, `FEM_EM_WF7_STEP2=1`, `-n 8`, `timeout -k 30 590`, Status 0, 210 s, 5 passed. 507 266 cells and 0.1495 % reproduced.
- **Next hypothesis:** an off-plane mirror check would need a lattice off the az_ref ray (e.g. az_ref + 5.625°). That is a review decision, not this slot's.

## 2026-09-25 09:45-09:55 UTC — GEO-34 step 1b (§9 item 40) — parked, negative result (04:30 CDT implementer slot, take-next after item 39)

- **Branch:** `attempt/GEO-34-step1b-20260925T094541Z` (`92d03fa`, parent `8643f53`). It holds the additive `built=None` keyword on `build_four_port_sweep` (replaces only the `_build(True, …)` call) and the new `tests/mesh/test_step_import_four_port.py` (`FEM_EM_GEO34_STEP1B=1`). The module reuses step 1a's `_build(True, step_export_path=…)` export, `import_step` and `CONFIG_PATH`. No `src/`.
- **Window:** `20260925T094541Z_GEO-34-step1b.log`, standard, `-n 2`, complex, `timeout -k 30 300`, `-s`. Status 1, 114 s; 4 passed, 1 failed. (`20260925T094534Z_GEO-34-step1b.log` is a 0 s docker-socket denial, retried once.) Generator rung + export 26.24 s; import 10.63 s + mesh 24.91 s; sweep 11.72 s; swapped-config import 36.09 s. 116 662 cells against the primitive 116 085 (printed). Two `BOPAlgo` fragment warning lines, one per import (printed, harmless as in 1a).
- **Anchors:** (iv) pre-gate PASS: azimuths 360/90/180/270° (0/90/180/270), err 0.000e+00. (i) PASS: ‖S − Sᵀ‖/‖S‖ 1.897416903e-14 (primitive 1.044e-14). (ii) PASS: σ_max 0.999992218 (primitive 0.999992805); column sums 0.763377628 / 0.786536312 / 0.762671451 / 0.787062035 (primitive max 0.7938). **(iii′) MISS:** self 3.3151 %, adjacent 1.1102 %, opposite 2.2713 % against the 0.5 % band (primitive 0.0553 / 0.0353 / 0.0214 %). Pooled 10.1158 % ⇒ separation 3.0514× against the 10× floor (primitive 166.6766×). Class means: self 23.947, adjacent 17.005, opposite 16.067 Ω.
- **Negative control (asserted, binary):** PASS. With the P2/P3 groups swapped the azimuths read P1 = 360, P2 = 180, P3 = 90, P4 = 270, and pre-gate misses = {2, 3}. The printed permuted-Z control (2 ↔ 3), predicted ≈ 5.9 %: self 3.3151 %, adjacent 10.2114 %, opposite 1.0854 %. Control (1), printed only because the mesh differs: rel. dev 4.17e-2 / 4.97e-3 / 1.09e-2 / 5.30e-3.
- **Rule (a):** `20260925T094757Z_GEO-34-step1b-ruleA.log`, `-n 2`, `timeout -k 30 180`, Status 0, 36 s, 5 passed. The run is byte-identical to the record: 116 085 cells, 1.044255156e-14, σ_max 0.999992805, 0.0553 / 0.0353 / 0.0214 %, 166.6766×. `built=None` changes nothing.
- **Reading:** the miss is structured, not noise. Z₁₁ ≈ Z₃₃ (23.13 / 23.15 Ω) and Z₂₂ ≈ Z₄₄ (22.38 / 22.36 Ω), so the x-axis legs and the y-axis legs differ. The narrowed sheets differ too: 26/26/29/26 facets, `w` 7.264/7.294/7.508/7.294 mm. ‖Z − Zᵀ‖/‖Z‖ is 6.78e-3 (primitive 8.8e-5).
- **Next hypothesis:** the STEP import loses the C4 congruence of the port-box triangulation that `GEO-19` step B's snapped local-frame construction gives the primitive mesh. The cheapest discriminating probe (smoke/standard, no solve) is per-port sheet facet counts and full-sheet triangle-size statistics on the import vs the primitive. If the hypothesis holds, the fix candidates are per-port-box sizing in the config or a `GEO-32`-style congruent sheet cut through `import_step`. Both are review decisions. Bands unmoved; known-issues 2026-09-25 entry.

## 2026-09-25 11:00-11:10 UTC — OPS-63 (§9 item 41) — complete (06:00 CDT implementer slot)

- **What landed:** `examples/ansys_benchmarks/ans2_birdcage_coil_driven_sar_10MHz/02_birdcage_coil_driven_sar_10MHz.py` asserts the builders' ASSERTED-labelled checks with imported bands (`RECIPROCITY_BAND`, `PASSIVITY_SIGMA_TOLERANCE`, `ADJACENT_SPREAD_BAND`, `PHANTOM_RADIUS` / `PHANTOM_HEIGHT`, `C4_COVARIANCE_BAND`). The `PORT-9` readings are captured by wrapping `build_four_port_sweep` in the mass-averaged module's namespace (the builder discards the sweep dict; no builder edited). New `[ANS-2] LABEL KEY` line; AED figure replaced by a pointer to the private adjudication file (no number); comparison template now `rf"""` (byte-identical output). One guide sentence added.
- **Window:** `20260925T110348Z_OPS-63.log`, heavy, `-n 4`, complex, `timeout -k 30 600`, 195 s, example exit 0, ALL ANCHORS GREEN. Reciprocity 7.489e-15; σ_max 0.999993824; max column sum 0.793794824; Z spreads 0.0551 / 0.0364 / 0.0211 %; containment 28.3650 mm < 30 mm, z reach 13.3650 mm < 40 mm; 10 g control 85.2523 / 85.2921 / 85.2750 / 85.2857 % > 5 %. Status 1 is the chained comparison looking in the wrong directory (`metrics_h0.0025.json` is written beside `metrics.json`, not in `paraview_output/`).
- **Leaf identity:** `20260925T110725Z_OPS-63-leaf-identity.log`, Status 0: 145 leaves / 123 numeric primary, worst 9.646e-11 at `.pointwise.0.sar_w_per_kg[1]` ≤ imported `EXACT_IDENTITY_RTOL` 1e-10 (step 4a: 8.19e-11). `20260925T110714Z_OPS-63-leaf-identity.log` is the same PASS plus an in-container `git status` that exited 128 (`dubious ownership`). Scratch comparator `logs/ops63_cmp.py` (gitignored). Run outputs deleted; `metrics.json` / `COMPARISON.md` never modified.
- **Census / audit:** phrase census 2 → 0. `check_private_leak.py --audit` `clean:` before (`20260925T110045Z_OPS-63-audit-before.log`) and after (`20260925T110729Z_OPS-63-audit-after.log`) — it did not flag the AED figure on the pre-change tree (its ≥ 8-digit limit); recorded in the §7 row, checker unchanged.
- **Next hypothesis:** the knob-route worst primary deviation sits at 0.96× the band (9.6e-11 vs 1e-10); a future identity run at a different rank count may cross it on round-off alone — compare at the same `-n` as the tracked record, and treat a 1e-10-scale miss as reduction-order noise to investigate, not a band to widen. The shared builder `test_port_birdcage_four_port.py:1` still emits a `'\*'` SyntaxWarning (out of scope here).

## 2026-09-25 11:11-11:13 UTC — EX-57 th:1 setup figure (§9 item 42) — complete (06:00 CDT implementer slot, take-next after item 41)

- **What landed:** `examples/time_harmonic/01_lossy_plane_wave.py` imports `write_setup_figure` and calls it right after `fine_seconds` is captured (outside both printed `[solve]` lines), on the fine 24³ solve's mesh with `cell_tags=None` — this fixture is a plain box domain with no cell-tag regions (one homogeneous lossy medium, the analytic wave imposed as Dirichlet data on every face), so `region_names={0: "box (homogeneous lossy medium, no cell-tag regions)"}` and the caption says plainly there is nothing to colour by region. The helper has no field-colouring mode, so the mesh is shown instead, named as the 24³ fine rung. Sliced normal to y (the x–z plane), the plane that shows both the propagation axis (+x) and the polarisation axis (z). Title 94 chars, well under the 130 ceiling. Guide gets a `## Setup figure` section between "How to run it" and "How to analyze it, step by step".
- **Pre-census (before any file written):** setup-figures `20260925T111108Z_EX-57.log` (`examples=56 ok=29 missing=27 broken=0`, Status 2), docrefs `20260925T111114Z_EX-57.log` (`dead=0 guide=0 stale=10 exit=2`).
- **Flagged run:** `./scripts/run_examples.sh -e th:1 -n 2 -t 180 --dry-run` emitted the command, run verbatim through the harness with `-e FEM_EM_SETUP_FIGURES=1` inserted (mirroring `cabb2f3`): `20260925T111144Z_EX-57.log`, Status 0, 17 s. `[setup-figure] wrote .../figures/time_harmonic_01_lossy_plane_wave_setup.png (60 KiB; regions 0=box (homogeneous lossy medium, no cell-tag regions))`. Records: rel L2 coarse 7.217852e-02 / fine 3.609441e-02, rate 0.9998, α 13.069299 Np/m vs closed-form 13.067043 (0.0173% vs 1% ceiling), β 27.031347 rad/m vs 27.015150 (0.0600%), σ=0 control α ≡ 0.0 exactly, `|E|` span 3.701× vs closed-form 3.694×. All assertions held.
- **PNG read and checked:** `examples/time_harmonic/figures/time_harmonic_01_lossy_plane_wave_setup.png`, 61 338 bytes (60 KiB, well under 600 KiB). 3-D panel shows one solid-colour cube (single untagged region, orange fallback palette — "box" matches no `CLASS_COLOURS` keyword, correctly not mislabelled "air"); slice panel shows the fine mesh on the x–z plane through the centre. Every caption/comment sentence checked against the render: no cell-tag regions (true — grid carries only tag 0), 24³ fine rung named (true — the call sits on `msh` from the fine solve), slice shows propagation + polarisation axes (true — normal (0,1,0), i.e. x–z plane). No labelling defect found. `git add`ed before the OPS-59 (b) run.
- **Unflagged re-run:** `20260925T111230Z_EX-57.log`, Status 0, 15 s, no `[setup-figure]` line. Every printed record identical to the flagged run digit for digit (rel L2, rate, α, β, span all match).
- **OPS-59 (b) identity:** `20260925T111251Z_EX-57.log`, `pytest tests/unit/test_doc_reference_exit_codes.py -v --tb=short -s`, Status 0, 20 passed.
- **Post-census:** setup-figures `20260925T111304Z_EX-57.log` (`examples=56 ok=30 missing=26 broken=0`, matching the predicted delta 27→26), docrefs `20260925T111309Z_EX-57.log` (`dead=0 guide=0 stale=10 exit=2`, unchanged — this example writes no ParaView `stale` window artifact of its own kind tracked by the checker beyond what was already fresh).
- **Committed:** example edit, guide `## Setup figure` section, PNG, seven harness logs, PROJECT_PLAN §7 `EX-57` row + §9 item 42 DONE, this entry — one commit on `main`.
...
## 2026-09-25 11:15-11:17 UTC — EX-57 th:2 setup figure (§9 item 43) — complete (06:00 CDT implementer slot, drained-queue fallback, census's own `--next`)

- **What landed:** `examples/time_harmonic/02_pec_cavity_resonances.py` imports `write_setup_figure` and calls it right after `solve_seconds` is captured (outside the printed `[solve]` line), on `spectrum.mesh` from `solve_pec_cavity_modes` with `cell_tags=None` — this fixture is a plain box domain with no cell-tag regions (a lossless, source-free 1.0×0.8×0.6 m PEC box, `n × E = 0` on every face), so `region_names={0: "box (lossless PEC cavity, no cell-tag regions)"}` and the caption says plainly there is nothing to colour by region (and no field either — the helper draws the mesh, not a solved eigenfunction). Drawn on the gate's own (6, 5, 4) mesh, the only mesh the example builds. Sliced normal to y (the x–z plane through the box centre (0.5, 0.4, 0.3)), the plane that shows the fundamental (TE₁₀₁) mode's half-wave structure along x and z and none along y (the guide's own step 5). Title 87 chars, well under the 130 ceiling. Guide gets a `## Setup figure` section between "How to run it" and "How to analyze it, step by step".
- **Pre-census (before any file written, run by the orchestrating session prior to this task):** `20260925T111503Z_EX-57-next.log` (`--next` prints `examples/time_harmonic/02_pec_cavity_resonances.py`, `examples=56 ok=30 missing=26 broken=0`, Status 2).
- **Flagged run:** `./scripts/run_examples.sh -e th:2 -n 2 -t 180 --dry-run` emitted the command, run verbatim through the harness with `-e FEM_EM_SETUP_FIGURES=1` inserted: `20260925T111617Z_EX-57.log`, Status 0, 3 s. `[setup-figure] wrote .../figures/time_harmonic_02_pec_cavity_mode_setup.png (87 KiB; regions 0=box (lossless PEC cavity, no cell-tag regions))`. Records: 720 cells, 5330 dofs, 8 eigenpairs converged; modes 239.9805/291.3904/312.3465/346.5469 MHz vs closed form 239.9510/291.3459/312.2838/346.3958 MHz, errors 0.0123%/0.0153%/0.0201%/0.0436% (all < 0.5% ceiling); `null_mode_count=0`; Rayleigh quotient 5.34e-15 relative (≪ 1e-6 gate); exported `|E|` span 2.156391e-17…1.0. All assertions held.
- **PNG read and checked:** `examples/time_harmonic/figures/time_harmonic_02_pec_cavity_mode_setup.png`, 88 594 bytes (87 KiB, well under 600 KiB). 3-D panel shows one solid-colour box (single untagged region, orange fallback palette — "box" matches no `CLASS_COLOURS` keyword); slice panel shows the (6, 5, 4) mesh on the x–z plane through the centre, edges visible. Every caption/comment sentence checked against the render: no cell-tag regions (true — grid carries only tag 0), gate's own (6,5,4) mesh (true — the call sits on `spectrum.mesh`), slice through (0.5, 0.4, 0.3) shows the x–z plane (true — normal (0,1,0)). No labelling defect found. `git add`ed before the OPS-59 (b) run — the first pass of the identity test failed with "admitted by the rule but not tracked" because the PNG was not yet staged; re-run green after staging.
- **Unflagged re-run:** `20260925T111732Z_EX-57.log`, Status 0, 2 s, no `[setup-figure]` line. Every printed record identical to the flagged run digit for digit (all four mode frequencies/errors, `null_mode_count`, exported `|E|` span match; Rayleigh relative differs in its last digits — 3.48e-15 vs 5.34e-15 — both far under the 1e-6 gate, expected floating-point-order noise, not a printed anchor record).
- **OPS-59 (b) identity:** `20260925T111705Z_EX-57.log`, `pytest tests/unit/test_doc_reference_exit_codes.py -v --tb=short -s`, Status 0, 20 passed (after `git add`ing the PNG; the pre-add attempt `20260925T111647Z_EX-57.log` failed 3 tests as expected).
- **Post-censuses:** setup-figures `20260925T111635Z_EX-57.log` (`examples=56 ok=31 missing=25 broken=0`, matching the predicted delta 26→25), docrefs `20260925T111725Z_EX-57.log` (`dead=0 guide=0 stale=10 exit=2`, unchanged).
- **Committed:** example edit, guide `## Setup figure` section, PNG, harness logs, PROJECT_PLAN §7 `EX-57` row + §9 item 43 DONE, this entry — one commit on `main`.
- **Next hypothesis:** none open; the item closes clean. The census's `--next` now points at the next missing example for a future slot.

## 2026-09-25 12:31-12:33 UTC — EX-57 th:3 setup figure (§9 item 44) — complete (07:30 CDT implementer slot, drained-queue fallback, census's own `--next`)

- **What landed:** `examples/time_harmonic/03_dielectric_sphere_in_uniform_field.py` imports `write_setup_figure` and calls it right after `solve_seconds` is captured (outside the printed `[solve]` timing), on `msh`/`cell_tags` from `_solve` — the `MeshGenerator.sphere_in_box_domain` fixture the `TH-8` gate closed on, the same generator `mri:2` and `mat:2` already picture. `region_names={SPHERE_TAG: "sphere (dielectric, eps_r=78)", 2: "air"}`, `hide_tags=(2,)` (air hidden), `translucent_tags=(SPHERE_TAG,)` (the sphere translucent — nothing else inside it to reveal, one-object-class convention). Sliced at `z = 0`, the mesh centre and the plane the interior probe cloud and both exterior pole/equator probes sit close to. Title 78 chars, well under the 130 ceiling. Guide gets a `## Setup figure` section between "How to run it" and "How to analyze it, step by step" (matching the `th:1`/`th:2` placement convention within the same example group).
- **Pre-census (before any file written, run by the orchestrating session prior to this task):** `20260925T123026Z_EX-57-next.log` (`--next` prints `examples/time_harmonic/03_dielectric_sphere_in_uniform_field.py`, `examples=56 ok=31 missing=25 broken=0`, Status 2).
- **Service check:** `docker compose -f docker/docker-compose.yml ps` showed `fem-em-solver` Up before any harness call.
- **Flagged run:** `./scripts/run_examples.sh -e th:3 --dry-run` emitted the command (verified it maps to `03_dielectric_sphere_in_uniform_field.py`), run verbatim through the harness with `-e FEM_EM_SETUP_FIGURES=1` inserted: `20260925T123139Z_EX-57.log`, Status 0, 11 s. `[setup-figure] wrote .../figures/time_harmonic_03_dielectric_sphere_setup.png (425 KiB; regions 1=sphere (dielectric, eps_r=78), 2=air)`. Records: 39684 cells; interior `E_z` = 0.038416 V/m vs closed form 0.037500 V/m (2.442%, ceiling 5%, 2.443% on the `TH-8` record); spread 0.080%, transverse/`E_z` 0.084%, `|Im|/|Re|` 0.000e+00; volume-integral cross-check 0.038410 V/m, 0.014% from the probe average; tagged sphere volume 5.206270e-04 m³ vs 4/3πR³ = 5.235988e-04 m³ (0.568%); jump over the pole 59.20× vs closed-form 56.27× (7.782%), at the equator 11.46× vs 11.83× (0.756%); negative control cited (0.918143 V/m, 2348.4% off, RECORD_BLIND_ERROR 23.48 > 1 > measured error). All assertions held.
- **PNG read and checked:** `examples/time_harmonic/figures/time_harmonic_03_dielectric_sphere_setup.png`, 435 056 bytes (425 KiB, well under 600 KiB). 3-D panel shows the translucent blue sphere alone (air hidden, legend reads "sphere (dielectric, eps_r=78)" / "air (hidden)"); slice panel shows the `z = 0` cross-section, grey square air-box outline around a centred blue sphere disc. Every caption/comment sentence checked against the render: sphere translucent with nothing else inside to reveal (true — no second conductor), air hidden in 3-D but visible in the slice (true), slice at the mesh centre near the probe planes (true — probes sit at `r ≤ 0.55R` interior, `r = 1.2R` exterior, both near `z = 0` in this fixture's geometry). No labelling defect found. `git add`ed before the OPS-59 (b) run.
- **Unflagged re-run:** `20260925T123155Z_EX-57.log`, Status 0, 7 s, no `[setup-figure]` line. Every printed record identical to the flagged run digit for digit (interior `E_z`, spread, transverse, `|Im|/|Re|`, volume cross-check, tagged volume, both jump ratios, control separation all match).
- **Setup-figure post-census:** `20260925T123230Z_EX-57.log` (`examples=56 ok=32 missing=24 broken=0`, matching the predicted delta 25→24, Status 2 — staleness info, not failure).
- **Docrefs census:** `20260925T123244Z_EX-57.log` (`dead=0 guide=0 stale=10 exit=2`, unchanged from the pre-item state).
...
## 2026-09-25 14:00-14:05 UTC — EX-57 th:4 setup figure (§9 item 45) — complete (09:00 slot, drained-queue fallback, census's own `--next`)

- **What landed:** `examples/time_harmonic/04_evanescent_waveguide_decay.py` imports `write_setup_figure` and calls it right after `solve_seconds` is captured (outside the printed `[solve]` timing), on `msh`/`None` from `_solve` — the `TH-7` evanescent-TE10-below-cutoff fixture (`dmesh.create_box`, geometry/frequency/exact-field imported from `tests/validation/test_waveguide_cutoff.py`). The fixture carries **no cell tags at all** (one lossless homogeneous medium, `eps_r=mu_r=1`, `sigma=0`), so `region_names={0: "guide interior (no cell tags -- single lossless medium, eps_r=mu_r=1, sigma=0)"}` and no `hide_tags`/`translucent_tags` — there is nothing to hide or make translucent. Sliced at `y = B_M/2` (`slice_normal=(0,1,0)`), the mesh centre, the plane carrying both axes the physics is read off: z (decay) and x (the TE10 half-arch). Title 87 chars, well under the 130 ceiling. Guide gets a `## Setup figure` section between "How to run it" and "How to analyze it, step by step" (matching the `th:1`/`th:2`/`th:3` placement convention within the same example group).
- **Pre-census (before any file written):** setup-figures `20260925T140144Z_EX-57-pre.log` (`examples=56 ok=32 missing=24 broken=0`, Status 2, matching the queued `20260925T140026Z_EX-57-next.log` this slot started from), docrefs `20260925T140201Z_EX-57-pre-docrefs.log` (`dead=0 guide=0 stale=10 exit=2`).
- **Service check:** `docker compose -f docker/docker-compose.yml ps` showed `fem-em-solver` Up before any harness call.
- **Flagged run:** `./scripts/run_examples.sh -e th:4 --dry-run` emitted the command (verified it maps to `04_evanescent_waveguide_decay.py`, complex build, `-n 2`), run verbatim through the harness with `-e FEM_EM_SETUP_FIGURES=1` inserted and `timeout -k 30 180` (standard tier): `20260925T140403Z_EX-57.log`, Status 0, 10 s. `[setup-figure] wrote .../figures/time_harmonic_04_evanescent_waveguide_setup.png (58 KiB; regions 0=guide interior (no cell tags -- single lossless medium, eps_r=mu_r=1, sigma=0))`. Records: 41472 cells; gamma = 37.650399 Np/m vs closed form 37.652670 Np/m (0.006%, ceiling 5%, 0.006% on the `TH-7` record); whole-domain rel L2 4.406648e-02 (record 4.406648e-02); residual `|Im E_y|/|Re E_y|` 0.000e+00; CG1 cross-check gamma = 37.640293 Np/m, 0.027% from the N1curl fit (well inside the 0.5% bound); transverse profile RMS 0.124% vs sin(pi x/a) (2% bound); control ratio 2.6373 vs 2.6383 cited from the `TH-7` gate log, not re-run; gamma 1.67x below k_c. All assertions held ("All assertions hold. Total elapsed 6.6 s").
- **PNG read and checked:** `examples/time_harmonic/figures/time_harmonic_04_evanescent_waveguide_setup.png`, 59 894 bytes (58 KiB, well under 600 KiB). 3-D panel shows one solid orange block (the whole guide, one region, nothing hidden or translucent) with the single legend entry. Slice panel shows the `y = B_M/2` cross-section — the mesh grid over the full `x` (0.05 m) by `z` (0.05 m) extent, axis triad confirming `x` vertical / `z` horizontal in that panel, slice annotation reads `normal (0.0, 1.0, 0.0) through (0.025, 0.0125, 0.025) m`, matching `B_M/2 = 0.0125`. Every caption/comment sentence checked against the render: no cell tags / nothing to hide or make translucent (true — one uniform orange block), slice carries both the decay axis (z, horizontal) and the mode axis (x, vertical) (true), no second geometry pictured as a control (true — the fixture builds exactly one mesh). No labelling defect found. `git add`ed before the OPS-59 (b) run.
- **Unflagged re-run:** `20260925T140500Z_EX-57-unflagged.log`, Status 0, 6 s, no `[setup-figure]` line (`grep -c setup-figure` = 0). Every printed record identical to the flagged run digit for digit (gamma 37.650399, rel L2 4.406648e-02, CG1 cross-check 37.640293/0.027%, control ratio 2.6373/2.6383, "Total elapsed 3.5 s" for the in-example clock, which is expected to differ from the harness-wall figure since it excludes container start-up — the *physics* records are what must match, and do).
- **Setup-figure post-census:** `20260925T140447Z_EX-57-post.log` (`examples=56 ok=33 missing=23 broken=0`, matching the predicted delta 24→23, Status 2 — staleness info, not failure).
- **Docrefs census:** `20260925T140453Z_EX-57-post-docrefs.log` (`dead=0 guide=0 stale=10 exit=2`, unchanged from the pre-item state).
- **OPS-59 (b) identity:** `20260925T140512Z_EX-57-ops59b.log`, `pytest tests/unit/test_doc_reference_exit_codes.py -v --tb=short -s`, Status 0, 20 passed (run after `git add`ing the PNG).
