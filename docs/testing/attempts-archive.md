# Attempt journal archive — entries moved out of attempts.md

Verbatim entries moved out of `docs/testing/attempts.md` by the weekly
planning review (first batch 2026-08-16) to keep the working journal
readable for the daily review: entries older than 14 days at archival time
move here unchanged, in their original order — never summarized, never
edited. Nothing here is authoritative for *status* — PROJECT_PLAN.md §7 is.
Grep here when a parked branch, an old anomaly, or a rescope needs its
original journal entry; git history has everything else.
