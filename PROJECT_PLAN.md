...
| `ANS-2` | Coil-driven SAR in the loaded four-leg birdcage at 10 MHz: pointwise SAR, whole-phantom power, mass-averaged SAR | 🟡 Open: step 1's runnable half is green with its control asserted as a floor, and nothing absolute is licensed. Step 2 (SPEC rows 4–6) remains open. **Step 3 — the AED numbers landed 2026-09-18** (operator, HFSS 2026 R1, Zero and First Order; Linux half re-run `20260918T235648Z_ANS-2-aed-private.log`, 216 s, anchors green, `COMPARISON_private.md` filled). **Preliminary verdict INCONCLUSIVE, owed to the 2026-09-19 weekly:** the two AED orders agree with each other, and the gap to ours on the shape-free rows 1–3 is a near-constant factor that hinges on a **factor-of-two question in our own stated incident power** (the example's "V_src²/(4Z₀)" line vs the peak-amplitude phasor convention the SAR formula uses; our reported accepted power matches 1 − \|S₁₁\|² only under the halved figure). Settle it by reading the drive code in `src/`, never by choosing the factor that fits: one answer makes rows 1–3 AGREE at the few-percent level, the other a real disagreement for a `PORT` systematics chunk. Row 6 peaks are boundary artifacts of HFSS's cube averaging (flagged in the script); the AED \|E\| column needs a harvest-only rerun. Analysis: `docs/private/ans2-operator-notes-2026-09-18.md`. *History: `docs/planning/chunks/ANS-2.md`.* | heavy |
