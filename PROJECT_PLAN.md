...
>     still unmeasured) in a window of its own. ~3 more slots. **The two review
>     decisions attempt 7 asked for are still owed and unchanged:** formally
>     defer `degree2` (14) and re-base the reachable denominator to **216 of
>     232**, against which the leg now stands at **155 of 216**.
...
6. **ATTEMPTED AGAIN 2026-08-21 (22:30 slot)** — `OPS-17` step 3 leg (b2)
   ...
   **one** — `TH11_STEP4_RUNG` selects the mesh, not the test set, and the
   ...
   (re-measure warm before deferring a group as expensive). Both big
   families are closed; the tail is **75 in ~25 files** with no big block,
   so the next leg returns to **shortest-first batching**, keeping
   `test_port_systematics_composition.py` (killed batch C, still unmeasured)
   in a window of its own — ~3 more slots. **The two decisions attempts 7–8
   asked for are still owed:** formally defer `degree2` (14) and re-base the
   reachable denominator to **216 of 232** (the leg stands at **155 of
   216**).
