...
10. **BLOCKED 2026-09-20 (04:30 slot) — infrastructure, not physics: the
   logging harness cannot reach the docker socket from a scheduled session**
   (`20260920T093410Z_TH-17.log`, Status 1, 1 s, no test ran; known-issues
   2026-09-20, first entry). **Unblock condition:** a sandbox/allowlist change
   that lets `scripts/testing/run_and_log.sh`'s own `docker compose` children
   reach `/var/run/docker.sock` (operator's; an implementer may not edit
   `.claude/settings.json`) — until then **no** item in this queue that needs
   compute can be executed, and the next slot should check that entry before
   anything else. Parts (i) and (ii) are written and unverified on
   `attempt/TH-17-20260920T094500Z` (`4f2785d`) and land by `git checkout` of
   the one test file once a window can run; part (iii) is unwritten and
   additionally needs a padding rung (§7 row: `PORT-1`'s own `L` is 9.1 % off
   Grover at its 0.08 m padding, so the item's 2 % pre-condition is not
   reachable on that fixture as it stands). Item text below, unchanged:
...
11. **`PORT-20` steps 1–2 — the current-drive route reports `S = z_to_s(Z)`**
...
12. **`PORT-22` step 1 — the tuned F-small resonance by a driven sweep,
