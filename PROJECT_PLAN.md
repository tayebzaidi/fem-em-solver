...
- **The birdcage's port S-matrix at 10 / 64 / 128 MHz is a gated
  ...
  **adjudicated 2026-09-06: AGREE at 10 MHz, INCONCLUSIVE at 64/128 MHz
  ...
  no homogeneity, absolute, tuning or SAR claim**). Its closed-form gate
  (step 4, 2026-09-07/08) is still parked: against the boxed image-lattice
  filament form truncated at `N = 3` the unloaded coil reads 2.33% at the
  centre and 7.09% at `r = 0.5R` along `+ŷ` (ten of eleven points inside
  5%, `20260908T004020Z_WF-6.log:1889–1899`), but the lattice's shell
  terms alternate with the converged sum **below** `S_3` (step 4d,
  2026-09-08, `…093332Z:41–46`), so a converged comparand *widens* every
  miss — ≈ 3% at the centre, ≈ 5–8% at `r ≥ 0.4R` along `+ŷ`, predicted
  — and shell averaging breaks the PEC wall identity (3.292e-02 vs 1e-2).
  **No closed-form B₁⁺ claim exists**; the residual is the FEM's to
  explain (`h` never refined on this fixture) and the 10:30 review's
  ruling on the `WF-6` row says how. Its step 3
...
review's to queue. **2026-09-07 03:00 review:** `auditor` PASS on all
eight checks at `ff50ce0` — headers `5a7fe19` = the closer's parent; 13 of
13 digits traced (`20260907T003548Z_MAT-4-step4.log:1932–1937,
1943–1946`); the gate an executed assert at
`test_birdcage_sar_mass_averaged.py:563`, the control at `:587`, the
identity at `:511, 519`; `C4_COVARIANCE_BAND` imported from
`test_birdcage_b1_plus_map.py:120` and untouched; the `sar.py` diff
exactly the `<= 0.0` → `< 0.0` permissiveness, `default_rho` untouched.
Re-traced by this review: `:1943–1946` read 0.3303 / 0.0756 / 0.0574 /
0.3132% with the 1 g column and the ~86% controls on the same lines. One
correction: the elapsed is **132 s** by the harness footer (`:2260`),
129.71 s by pytest — the "130 s" above is the rounded pytest figure. The
slot's `QUADRATURE_DEGREE` provenance slip (the item named the wrong
module for the 16; the value imported is the step-3 one) is noted and
harmless. The owed example is **`EX-53`** (`mri:3`, §9 item 4). The 1 g
column's `h` route is a phantom rung below `PHANTOM_RESOLUTION_FINE` —
≈ 8× the cells, uncosted, above the standard tier by construction — named
for the weekly, not queued.

**Step 5 — the 1 g column on the `GEO-27` rung (scoped 2026-09-08 03:00
review, §9 item 4; the row's ✅ is unchanged by it — a step inside a
closed row, its own gate is what a review ratifies from the number).**
`GEO-27` priced the rung: `phantom_resolution = 0.0025` m gives 4.96 cells
across the 1 g ball at **199 920 cells** (1.66× the 0.0075 rung's 120 499,
21.4× its phantom cells), 36.27 s to mesh, a single reading
(`20260908T033218Z_GEO-27.log`). New module
`tests/validation/test_birdcage_sar_1g_rung.py` importing step 4's helpers
(the four single-drive solves, the mass-averaging operator, the C4 pairing,
`C4_COVARIANCE_BAND`) and running them once at `phantom_resolution =
0.0025`, `-n 4`, complex build. **Anchors (asserted):** (i) the **10 g** C4
identity at the imported, unmoved 5% band on this rung — the same assert
step 4 runs at 0.0075 (0.3303 / 0.0756 / 0.0574 / 0.3132%,
`20260907T003548Z_MAT-4-step4.log:1943–1947`), so its label is *asserted*:
an operator that is C4-covariant at `h` is C4-covariant at `h/3`, and a
miss here is a finding about the mesh, not a band; (ii) the whole-phantom
ball identity (`½∫σ|E|²` tagged vs ball) at the imported 1e-10, as step 4.
**Printed, predicted (rule (e) — no prior measurement at this rung):**
(1) the four 1 g C4 pairs — predicted **inside 5%** on every pair (the
worst pair 6.8383% at 1.65 cells across the ball falls with `h`; the 10 g
column's 0.33% at 3.57 cells is the comparison class); (2) the four
per-drive 10 g peaks against step 4's 6.348 / 6.327 / 6.332 / 6.328e-07
W/kg and the per-drive 1 g peaks against step 4's — the rung-to-rung
move, predicted a few % on 10 g and larger on 1 g; (3) `size_global` and
phantom-tag cells beside `GEO-27`'s 199 920 / 58 866 — the **repeat that
rung lacks**; equal to the integer means a later review may pin it, a
drift is an `OPS-18`-class record, neither is asserted here. **Tier /
ranks / cost:** step 4 measured **132 s** at `-n 4` on 120 499 cells (four
solves + the operator, `…-step4.log:2260`); MUMPS on 1.66× the cells with
the refinement confined to the phantom ≈ 2–2.5× ⇒ ≈ 300–350 s + 36 s mesh
⇒ one window `-n 4`, `timeout -k 30 900`, heavy; collect-only smoke first.
**Traps already paid for:** step 4's (the density map's `0.0` phantom
exclusion, `QUADRATURE_DEGREE` imported from the step-3 module, the
averaging radii from `averaging_ball_radius`); the cells-across figures
are `GEO-27`'s `2a/h`, not a mesh measurement — print them as labels only;
`-k a or b`; complex build + `FEM_EM_REQUIRE_COMPLEX=1`, `tests/environment`
first; no `run_in_background`. **Scope:** no band, no `src/`, no §2
change; the 1 g column stays a printed record until a review gates it
from this number; the row stays ✅ on its step-4 scope. **Negative
result:** the 1 g worst pair still above 5% at 4.96 cells across is the
finding that 5 cells is not enough for a 6.2 mm ball — record it in this
paragraph, no known-issues (nothing red), stop; the 10 g identity failing
...
*gate* at this rung is a review's ruling from these numbers. *Audited
PASS, 2026-09-08 10:30 review, at `c5835d3`:* four footers on header
`127f9ef` (the parent), 27 of 27 digits traced, the 10 g anchor an
executed `assert reading <= C4_COVARIANCE_BAND`
(`test_birdcage_sar_1g_rung.py:353`) with the ball identity at `:324,
:333`, the 1 g test asserting finiteness only, `git diff 127f9ef c5835d3
-- src/` empty; re-traced by this review: `…140457Z:1985–1993`.

**Ruled 2026-09-08 10:30 review — the 1 g column is promoted to a gate
at the `GEO-27` rung and the rung's mesh is pinned: step 5b (§9 item
3).** Two windows agree on every 1 g pair (0.0957 / 0.1199 / 0.1305 /
0.1065%, `…140457Z:1990–1993` and `…141147Z:1990–1993`) with a factor 38
of headroom against the imported band, and three independent windows
agree on the mesh (199 920 / 58 866: `20260908T033218Z_GEO-27.log:7155`,
`…140457Z:1981–1982`, `…141147Z:1981–1982`) — the repeat `GEO-27`'s
caveat asked for. Implementer, complex build, test module only,
`tests/validation/test_birdcage_sar_1g_rung.py`; step 4's module
untouched, so no rule-(c) re-run. **The change:** (a)
`test_the_one_gram_column_is_printed_with_its_pre_registered_verdict`
becomes the 1 g gate — the four cyclic **1 g** C4 pairs asserted
`≤ C4_COVARIANCE_BAND` (imported, unmoved 5%), the print labels flipped
from "PRINTED NOT GATED" to "ASSERTED" so the log claims exactly what the
module runs; the mis-paired 1 g control **printed, predicted ~85%** (the
85.25–85.29% at `:1969–1972` is the 10 g column — rule (e): no prior 1 g
measurement of that comparison, so predicted, never asserted); (b) two
version-tagged records under the (1\*) licence,
`ONE_GRAM_RUNG_CELL_RECORD = 199_920` and
`ONE_GRAM_RUNG_PHANTOM_CELL_RECORD = 58_866`, asserted at the imported
`CELL_COUNT_BAND` = 0.01 (`tests/mesh/test_birdcage_port_sheet_prerequisite.py:57`),
**never at equality** (`OPS-27`'s stale-exact class; the `GEO-25` step-2
pattern); (c) nothing else — the 10 g anchors, the ball identity and the
rung-to-rung prints unchanged. **Anchors (asserted):** the 1 g gate,
backed twice as above; the two records at the band, backed three times;
the existing (i) and (ii). **Tier / ranks / cost:** 251 / 245 s at `-n 4`
(`…140457Z:2289–2290`, `…141147Z:2289–2290`) ⇒ one window `-n 4`,
`timeout -k 30 560` — **not** the 900 step 5 priced, which cannot fit a
foreground 660 s host window (the 09:00 slot sized it down; the right
call); collect-only smoke first. **Traps:** step 5's —
`phantom_resolution=0.0025` through `_build_mass_averaged`,
`build_density_field` with `0.0`, `QUADRATURE_DEGREE` from the step-3
module; `-k a or b`; complex build + `FEM_EM_REQUIRE_COMPLEX=1`,
`tests/environment` first, `-s`; no `run_in_background`. **Scope:**
`MAT-4`'s ✅ gains "1 g and 10 g C4-gated on the 0.0025 rung"; §2's two
SAR clauses (the §2.1 SAR bullet and the §2.2 absolute-SAR bullet)
replace "the 1 g column misses … printed record" with the gate **in the
landing commit**; still no absolute SAR, no compliance, no homogeneity,
no Larmor, and two rungs of one quantity are not a convergence rate.
**Negative result:** the 1 g assert red on the same mesh cannot happen
without the mesh or module changing (two windows agree to the digit) —
...
| `WF-6` | B1+ field mapping and homogeneity (CV) | 🟡 Open: the step-4 family is frozen under the four-attempt rule, and the flag-on ×0.0095 power residual is banked as the terminal form's Cauchy–Schwarz deficit with its mechanism not explained. What would move the row is a numbered step 5 scoped by the weekly: the homogeneity / CV subgoal on the two-rung ladder, or ×0.0095 back on the ladder under rule (f). *History: `docs/planning/chunks/WF-6.md`.* | heavy (step 1 standard, complex; steps 3h 194 s / 3f′ 117 s / 3i 96 s standard; step 4a 4 s smoke, `-n 1`, real; **step 4g 204 s at `-n 4`**; **step 4h 199 s at `-n 4`** + 52 s flag-off control; **step 4i 198 s + 104 s at `-n 4`**; **step 4j 199 s + 101 s at `-n 4`**; step 4k 29 + 32 + 39 + 38 s smoke, `-n 1`, no solve) |
