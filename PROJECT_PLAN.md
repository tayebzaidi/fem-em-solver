...
| `OPS-60` | **`scikit-rf` in the image, pinned** — the circuit engine `PORT-23` gates against the raw layer (operator directive 2026-09-20: use a vetted circuit solver, netlist prepared in our code) | 🟡 **2026-09-21 (04:30 slot): Dockerfile pin `scikit-rf==2.1.0` (newest on PyPI, read in-container by `pip index versions`, `20260921T093027Z_OPS-60-version-probe.log:34`) and the test landed; the negative control fails as predicted on the current image — `ModuleNotFoundError: No module named 'skrf'` (`20260921T093041Z_OPS-60.log:48`, 1 s, `-n 1`). ✅ owed to the operator's rebuild: the same test green on the rebuilt image (known-issues 2026-09-21).** Step 1: add `scikit-rf==<latest release at execution, pinned>` to `docker/Dockerfile`'s pip block beside `pyvista`, and a `tests/environment/test_scikit_rf_version.py` that imports `skrf`, prints `skrf.__version__` and asserts it equals the pin (the `OPS-25` pattern). **The image rebuild is the operator's** (`docker compose build`, outside the sandbox); the slot edits the Dockerfile and the test, runs the test to show it *fails* on the current image (import error = the negative control), journals, and marks `PORT-23` PENDING PREREQUISITE → READY only after the operator's rebuilt image passes it. No other dependency moves; version-tagged constants are unaffected (nothing in `src/` imports `skrf`). | smoke |
...
Last reviewed **2026-09-21, 03:00 review** (Monday; `claude-fable-5-1`, no
override). Full journal: `docs/planning/reviews/2026-09-21-daily.md`.
Rulings that bind the next slots:

- Audits: `PORT-22` ✅ and `PORT-20` ✅ both **PASS**; nothing demoted. `PORT-20`'s ✅ covers the code and the module gates — `ans:3`'s tracked `metrics.json` / `COMPARISON.md` still show the old S table (known-issues 2026-09-20; an interactive-session job, **no slot runs `ans:3`**).
- `xl` 09-21 (`WF-7` 0c, chain H1) priced the F-human degree-1 32-drive set at one factorisation + 31 held solves, 224 s, 13.8 GiB ⇒ **H3 is a heavy-tier item (21), not an `xl` job.**
- `PORT-21` (c): the 0.5 % C4 band is **not moved**; the variant moves onto the congruent cut with its own control′ (item 22). `OPS-50`: the E ordering guard becomes a two-sided separation record at the unchanged 0.02 — **this is the licence** (item 26). `OPS-59` (a): `ans6_…/metrics.json` is declared (item 25). `TH-17` (iii): `L` comes from the hole fixture's own driven route, Grover printed (item 24, rule (f)).
- **Standing from today:** an `ans:` runnable-half item declares its `metrics.json` in `LISTED_EXAMPLE_ARTIFACTS` in the commit that adds it.
- `TH-17` row gate (a) is **not** re-scoped by any slot: the eigen route loads all four ports with `C_tuned`, the driven route feeds P1, so 129.6 MHz and 63.9986 MHz answer different equations — the 09-26 weekly rules.
- `ANS-6` adjudication (AED numbers landed 09-20) is the 09-26 weekly's; items 22, 23 and 30 **never open `docs/private/` or `aed_results/`**.
- No absolute `S₁₁` / `Z_in` claim at 10 or 64 MHz from any item below (known-issues 2026-09-19, `PORT-21`).
- Docker-socket denial: intermittent, caused by piping or subshell-wrapping the harness call (known-issues 2026-09-20) — plain `scripts/testing/run_and_log.sh …`, retry once, never park on it.
- Residual `main` reds at `-n 2`: **3 deliberate/known + the artifact pin (item 25 closes it)**, plus the padding module at `-n 4`.
- Parked: `attempt/PORT-21-…` and `attempt/OPS-50-…` are consumed by items 22 / 26 (delete after landing); `attempt/TH-17-20260920T094500Z` deleted by this review (superseded, its one file landed as `4fa99e3`); the four older branches kept on the 2026-09-09 ruling. No `recovered/*`; tree clean at review start.
- XL: 5 of 6 windows used, **4 `xl` ahead (09-22 … 09-25), 1 `xxl` ahead (09-26)** — floors met exactly; the next review writes the `ANS-6` hole-mesh cost probe from item 23's price.
...
*(The 09-20 queue's items 10–18 are collapsed to one line each below
(daily-review.md step 6c); their execution text is in git (`1ba7302`), their
readouts in the §7 rows and `docs/testing/attempts.md`. The 09-19 queue's
lines 1–9 are dropped — archive 2026-09-20.)*

10. PARTLY DONE 2026-09-20 (04:30) — `TH-17` step 1b: (i) sheet mass present (1.569e+03 × the volume mass on the sheet dofs), (ii) loaded floor 1.2959e+08 Hz, nothing near 64 MHz; (iii) blocked on its 2 % pre-condition → item 24; `4fa99e3`; `20260920T093810Z_TH-17.log`.
11. DONE 2026-09-20 (04:30) — `PORT-20` steps 1–2: current route reports `S = z_to_s(Z)`, closed form exact, control 6.614e-02, `PORT-9` gate digit-identical; `37ede7d`; §7 row.
12. DONE 2026-09-20 (06:00) — `PORT-22` ✅: circuit-vs-field residual ≤ 8.256e-05 at all 11 frequencies 44–84 MHz, one `Im Z_in` sign change in [60, 64] MHz, control 1117× the band; `44c0ee9`; `20260920T110503Z_PORT-22.log`.
13. PARKED 2026-09-20 (06:00) — `PORT-21` step 1: control and (a) green, table complete, variant (c) red on the imported C4 band (259 509-cell re-mesh) → ruled, item 22; `14c86fe`; `attempt/PORT-21-20260920T114000Z`.
14. DONE 2026-09-20 (07:30) — `ANS-6` runnable half 🟡: Cu and PEC columns at 10/64/128 MHz, PEC cross-check 3.331e-13, `P_coil/P_in` read off the live gate call; `75082a0`; `20260920T124620Z_ANS-6.log`.
15. PARKED 2026-09-20 (07:30) — `OPS-50` step 3: four licensed pins re-record green (drifts ≤ 9.71e-06), a fifth guard (PIN E ordering) keeps `PROBE_RESULT FAIL` → ruled, item 26; `e23631a`; `attempt/OPS-50-20260920T125900Z`.
16. HALF DONE 2026-09-20 (07:30) — `OPS-59`: (b) call-site literal → three-count identity (21 = 21 = 21), entry retired; (a) red on one undeclared non-setup artifact → ruled, item 25; `5fd3d81`; `20260920T130232Z_OPS-59-postchange.log`.
17. DONE 2026-09-20 (09:00) — `PORT-20` ✅: four S-derived records re-recorded, `Z` records digit-identical (≤ 2.10e-10), xfail removed; `ans:3` leg stopped on its private-file trap (known-issues 2026-09-20); `b7c0d6a`; `20260920T141141Z_PORT-20.log`.
18. DONE 2026-09-20 (09:00) — `EX-57` `mesh:8` figure on the 16-leg rung, `EX-33` digits reproduced, census `missing` 34 → 33; `5e52e8c`; `20260920T142137Z_EX-57.log`.

**Predicted slot-minutes (rubric element 3 + 15 min fixed), running total
over the *open, unblocked* items:** item 19 → 20 · item 21 → 65 · item 22 →
110 · item 23 → 165 · item 24 → 215 · item 25 → 235 · item 26 → 255 · item
27 → 285 · item 28 → 305 · item 29 → 335 · item 30 → 360 · item 31 → 405 ·
item 32 → **425**. Floor 240 and ≥ 5 items: **met** (2026-09-21 03:00
review). **Item 20 is not counted** — it waits on the operator's image
rebuild. Every counted item is independent of the others; take them in
order. Items 21–24 are the weekly's chain steps H3, F3, T9b and T5 (§10,
2026-09-19), written under rule (4); 25–26 close yesterday's two parked
chunks on this review's rulings; 27–30 are the external-review rows the
operator opened on 09-19 "for a review to queue".

19. DONE 2026-09-21 (04:30) — `OPS-60` ⬜ → 🟡: pin `scikit-rf==2.1.0` + test landed, negative control `ModuleNotFoundError` as predicted; rebuild owed (dashboard item 2); `20260921T093041Z_OPS-60.log`. **`OPS-60` — `scikit-rf` in the image, pinned** (implementer;
...
21. DONE 2026-09-21 (09:30) — `WF-7` step 1 🧪 → 🟡: F-human 32×32 at 64 MHz, degree 1, `-n 8`: reciprocity 9.201e-15, σ_max 0.999914648, 18 classes worst 3.6997 % (other ring / 2 steps), exact identity ≤ 2.835e-15 on drives 1/9/17/25 (first identity 17.31 s > the 15 s stop rule), default-orientation extent 2.816e-17 m; 372 s; `20260921T093429Z_WF-7-step1.log`. 21. **`WF-7` step 1 — the F-human 32×32 at 64 MHz, degree 1, gated on
   `PORT-13`'s identities and `PORT-16`'s exact power identity** (implementer;
   new `tests/validation/test_wf7_f_human_ring_matrix.py`; **no `src/`**;
   **heavy**, `-n 8`; complex build; `main`; independent; **45 slot-min**:
   one ≈ 7 min window + 15 + reading). **Chain step H3** (§10, 2026-09-19),
   rule (4) — serial on H1's price, which landed 2026-09-21
   (`xl-ledger.md`: one factorisation 24.73 s + 31 held back-substitutions
   0.40–0.57 s, 607 039 unknowns, 13.8 GiB summed at `-n 16`; at `-n 8` on
   the ordinary service `20260916T093548Z_WF-7-step0c.log:10419–10431`:
   build 120.98 s, drive 1 27.71 s, held 0.59 s, 11.08 GiB). That price is
   why this is heavy and not `xl`.
   **The change:** build the fixture exactly as
   `scripts/probes/wf7_step0_f_human_cost.py:229–293` does (`_params`,
   `_ring_ports`, `F_HUMAN_RING_RADIUS`, `LEG_COUNT`,
   `F_HUMAN_BRANCH_B_CELL_RECORD` from `test_birdcage_f_human_rung`;
   `ring_sheet_orientation="longitudinal"`; `CELL_COUNT_BAND` from
   `tests/mesh/test_birdcage_port_sheet_prerequisite.py`), drive all 32 ring
   ports through `_solve_one_drive` with factor reuse
   (`test_port_birdcage_ring_column`), and **per drive, before the fields are
   dropped**, evaluate `terminal_form_deficit`
   (`src/fem_em_solver/ports/shares.py:60`) and the exact identity residual
   the way `test_birdcage_power_identity.py:540–554` does — import its
   helpers, restate nothing.
   **Anchors (asserted, every band imported):** (i) the cell record inside
   `CELL_COUNT_BAND`; (ii) 32×32 reciprocity ≤ `RECIPROCITY_BAND` 1e-3
   (0c measured 1.084e-14 on this fixture); (iii) `σ_max` ≤
   `MATRIX_PASSIVITY_CEILING` from `test_port_birdcage_ring_matrix.py:150` —
   **that constant is exactly 1.0; the chain text's "1 + 1e-9" is not what
   `PORT-13` gates on; import, do not restate** (0c printed 0.999915);
   (iv) the 18 C16 × mirror classes via `_azimuth_class`
   (`…ring_matrix.py:206`), exactly `N_SYMMETRY_CLASSES`, azimuth residual
   < `AZIMUTH_MATCH_DEG`, worst class spread ≤ `OPPOSITE_SPREAD_BAND` 0.05 —
   **never measured on this fixture at 64 MHz** (0c printed only the
   `|S_jj|` spread, 1.7891e-03); (v) the exact discrete power identity ≤
   `DISCRETE_IDENTITY_RTOL` 1e-6 on **all 32** driven columns.
   **Negative controls:** *(asserted, the chain's own — made assertable at
   mesh level)* a second build of the same fixture with
   `ring_sheet_orientation` left at its default has every ring sheet's extent
   along the drive direction < `DEGENERATE_EXTENT_M` 1e-12 m while the
   longitudinal build's gap chord is > 0 — the mis-paired orientation cannot
   carry a gap voltage (import the constants and the extent helper from
   `tests/mesh/test_birdcage_ring_sheet_orientation.py`; **no solve on the
   degenerate build**); *(asserted, arithmetic)* one cached column ×
   `CONTROL_COLUMN_SCALE` 1.01 raises the reciprocity ratio by > 1e6× the
   measured one; *(predicted, printed — rule (e))* the same control against
   `MATRIX_CONTROL_MARGIN × RECIPROCITY_BAND` = 2e-3: this review's
   arithmetic from 0c's column sums puts it at ≈ 1.5–2.1e-3, **so the imported
   2× margin is at or past this fixture's ceiling — print it, never assert
   it**.
   **Printed, never gated:** the per-drive terminal residual (0c:
   4.061e-03–4.624e-03) beside the in-run pooled κ; full-column Σ|S_ij|²;
   `S_driven(P17)` beside the step-0 record `0.407423+0.344417j`.
   **Tier / cost:** ≈ 121 s build + 28 s + 31 × (0.6 s + the identity's facet
   assembly, **unmeasured** — print the first one; if it exceeds 15 s, run the
   identity on drives 1, 9, 17, 25 only and say so) + ≈ 125 s for the
   control's second build ≈ 6–8 min; one window, `timeout -k 30 1100`,
   durable capture with the trailing `; exit $rc`, `-s`.
   **Traps:** 32 columns of F-human fields is the memory trap — evaluate,
   then `del fields` per drive (probe `:324–329`); the facet integrals carry
   `metadata={"quadrature_degree": …}` as the imported helpers do; rank-safety
   on every reduced scalar.
   **Scope:** self-consistency identities on one fixture at degree 1. No
   field, no B₁⁺, no SAR (H4 / H5), no absolute S — **the row carries the
   weekly's 5.50 % order caveat verbatim**.
   **Status it can move:** `WF-7` 🧪 → 🟡 — the row has no Done-when for this
   step; **this review adds it (rule (j))**: "step 1 = anchors (i)–(v) and the
   mesh-level orientation control asserted green in one logged window". The
   dashboard's Human-scale row changes from "priced" to "identities gated".
   **Negative result:** a reciprocity, passivity or class-spread miss at human
   scale is a finding for a `PORT` chunk, not a band change (`xl-pending.md`
   entry 6's rule) — known-issues entry with the class table, park on
   `attempt/*`, mark this item BLOCKED (rule (d)); an identity miss at 1e-6
   goes to known-issues against `PORT-16` with the per-drive residuals.

22. DONE 2026-09-21 (09:30) — `PORT-21` step 1 closed on `main` (still 🟡, step 2 the weekly's): (c′) on the C4-congruent cut spreads ≤ 0.0837 % (band 0.5 % unmoved, negative branch did not fire), self move 1.885e-02 / 1.538e-02 / 1.679e-02 vs control′; control′ reproduces the 64/128 MHz spread records; (a) residual 5.359e-05; `20260921T094335Z_PORT-21.log`, `20260921T094925Z_PORT-21.log`. 22. **`PORT-21` step 1b — land the parked table, with variant (c) measured on
   the C4-congruent cut** (implementer;
   `tests/validation/test_port21_feed_sensitivity.py` taken **by path** from
   `attempt/PORT-21-20260920T114000Z` (a new file — safe; record rows are
   appended by hand, never path-checked-out); **no `src/`**; heavy by count;
   `-n 2`; complex build; independent; **45 slot-min**: two windows ≈ 370 s +
   ≈ 180 s + 15). **Chain step F3**, second attempt of the step-1 family
   (2 of 4).
   **Ruling (2026-09-21 review):** on the 259 509-cell re-mesh variant (c)'s
   class spread (self 2.3607 % at 10 MHz, `20260920T112548Z_PORT-21.log:7442`)
   is **four times the move it is supposed to measure** (5.98e-03), so that row
   is mesh asymmetry, not a feed sensitivity — the `GEO-30`/`-31`/`-32`
   mechanism, and a record for the `PORT-21` row, not a defect. **The 0.5 %
   band is not moved and stays asserted on every run.** What changes is the
   mesh (c) is built on, and both sides of its difference move together:
   **control′** = `build_four_port_sweep(frequency_hz=f,
   c4_congruent_sheets=True)` (kwarg, `test_port_birdcage_four_port.py:227`;
   116 118 cells on record) and **(c′)** = the same plus
   `conductor_resolution=0.5 * CONDUCTOR_RESOLUTION`; `|ΔS|/|S|` for (c′) is
   read against control′, never against the gate-record control. The
   gate-record control (1e-9) and variant (a) stay exactly as parked; (b)
   stays skipped-and-named. The uncongruent (c) numbers stay in the table as a
   printed record with the three logs cited; its assert is removed *because
   the variant is replaced*, and the docstring says so.
   **Anchors (asserted, imported):** the parked module's, unchanged — control
   at `LEG_D0_REPRODUCTION_BAND` 1e-9, (a) at `REDUCTION_BAND`, reciprocity /
   passivity / `ADJACENT_SPREAD_BAND` 0.005 on **every** run including
   control′ and (c′); control′'s spreads reproduce the records
   0.0475 / 0.0529 / 0.0366 % at 64 MHz
   (`20260920T070008Z_ANS-4-step3d.log:2548`) and 0.0739 / 0.0822 / 0.0506 %
   at 128 MHz (`20260917T070008Z_ANS-4-step3b.log:2548`) — **printed beside,
   not asserted** (different rank width); **no congruent-cut record exists at
   10 MHz** — that reading is new and is printed.
   **Negative control (asserted):** as parked — every variant, (c′) included,
   moves the self class by > `KNOB_REACHES_THE_SOLVE` 1e-6 relative.
   **Tier / cost:** the parked windows ran 264 / 127 / 263 s; control′ adds
   ≈ 45 s and (c′) ≈ 75 s mesh + solve per frequency ⇒ window 1
   (`FEM_EM_PORT21_FREQUENCIES_MHZ=10,64`) ≈ 370 s, window 2 (`=128`) ≈ 180 s;
   each `timeout -k 30 590`, durable capture. If window 1 overruns, split
   10 and 64 — never raise the timeout.
   **Traps:** **do not open `docs/private/` or any `aed_results/`**; rule (i);
   `-s`.
   **Status it can move:** `PORT-21`'s step 1 from *parked* to **closed on
   `main`** (the row is already 🟡; its Done-when's first half — identities
   asserted, table logged — is met by this item; step 2 is the 09-26
   weekly's, which can only read a table that is on `main`) and the 2026-09-19 `ANS-4` self-class known-issues entry
   gains the table's location.
   **Negative result (pre-registered):** (c′) still exceeds 0.5 % on the cut ⇒
   the generator cannot hold C4 at that grading; (c′)'s gate test is marked
   `xfail(strict=True)` citing a new known-issues entry with the spreads,
   the step closes on control + (a) + the two printed (c) tables, `PORT-21`
   stays 🟡 with step 1 on `main`, and the weekly reads (c) as *not measured*.

23. DONE 2026-09-21 (09:30) — `ANS-6` step 2 knob ✅ + degree-2 price at h = 0.015: unset control at `-n 4` reproduces all 96 tracked S leaves to 1.395e-14 (band 1e-9, `20260921T095749Z_ANS-6-step2-control-check.log:34`; run `20260921T095508Z_ANS-6-step2-control.log`, 134.8 s); degree 2 at 64 MHz, `-n 8`: all gates green both columns, identity 7.812e-13, class moves 4.06e-02…7.40e-02 (predicted 3–7e-2), PEC sweep 61.9 s / Cu sweep 53.9 s / P1 44.9 s, 183.9 s total, summed ru_maxrss 13.78 GiB (`20260921T100022Z_ANS-6-step2-degree2-64MHz.log:924–942`); unknown count not printed. Gate modules re-run green (`20260921T095800Z_ANS-6-step2-gates.log:1476` TH-14 11 passed; `20260921T095937Z_ANS-6-step2-gate-th15.log:995` TH-15 at 64 MHz 6 passed). Original text:
   **`ANS-6` step 2 (chain T9b, slot half) — a degree knob on `ans:6`, unset
   digit-identical, and the first degree-2 price on the hole mesh**
   (implementer; the `ans:6` script + guide, additive `degree: int = 1`
   keyword on `_build_ladder()` (`tests/validation/test_th14_birdcage_copper.py:256`)
   and `_hole_rung()` (`tests/validation/test_th15_birdcage_pec_hole.py:244`)
   forwarded to the solves at `:285, :295, :309` / `:316, :324` — **rule (a):
   both gate modules re-run green in-slot and disclosed**; **no `src/`**
   (`degree=` already exists on `run_n_port_sparameter_sweep` and
   `run_lumped_sheet_port_case`); heavy; complex build; independent;
   **55 slot-min**: control 140 s + two gate re-runs ≈ 100 s + ≈ 100 s + one
   degree-2 window + 15). Serial on T9 ✅ (`75082a0`).
   **The change:** `FEM_EM_ANS6_DEGREE` (the name is the contract), the
   `ANS-2` step 4a pattern (`a2e4a04`): unset ⇒ byte-identical behaviour; set
   ⇒ degree-tagged `metrics` / XDMF names, **`COMPARISON.md` and the tracked
   `metrics.json` never rewritten**, a printed `[ANS-6 step 2] degree knob`
   line. With the knob at 2 the script also requires one frequency
   (`FEM_EM_ANS6_FREQ_MHZ`) and runs the Cu and PEC columns at it only — two
   factorisations, not the σ-ladder.
   **Anchors (asserted):** *control* — the unset run at **`-n 4`** (the width
   the tracked `metrics.json` was written at, `bec0965`,
   `20260920T175038Z_ANS-6-aed-compare.log`, 140 s) reproduces every S leaf of
   the tracked file within the imported `LEG_D0_REPRODUCTION_BAND` 1e-9
   (backing: the same-width F-small reproduction read 1.07e-10…2.57e-10
   yesterday; `EXACT_IDENTITY_RTOL` 1e-10 would have ≈ no headroom — do not
   import that one); *degree 2* — the imported reciprocity / passivity / C4
   gates and the copper surface-loss identity at `DISCRETE_IDENTITY_RTOL`
   hold at 64 MHz on both columns (identities do not depend on order;
   `TH-19` step 3 measured them green at degree 2 on F-small).
   **Negative control (asserted floor, predicted size — rule (e)):** the knob
   reaches the solve — every class moves by > 1e-6 relative between degree 1
   and 2; predicted ≈ 3–7e-2 from F-small's one-mesh order move (6.09 / 5.38 /
   6.70 % at 128 MHz, `ANS-4` step 2b), printed beside; ceiling 2.
   **Tier / cost (degree-2 window is a cost probe — the readout is the
   price):** hole mesh 80 181 cells; `TH-19` step 3 priced F-small's 116 085
   cells at 745 938 unknowns, 68.0 s per factorisation, 17.86 GiB summed at
   `-n 8` (`20260913T183446Z_TH-19-step3-10MHz.log:1950–1954`) ⇒ predicted
   ≈ 515 k unknowns, ≈ 45 s, ≈ 12 GiB; `-n 8`, `timeout -k 30 1100`, durable
   capture. **STOP rule:** first factorisation > 300 s or summed `ru_maxrss`
   > 60 GiB ⇒ kill, report, no second column.
   **Traps:** the unset run rewrites `COMPARISON.md` / `metrics.json` — the
   operator's 09-20 session edited both; after the control,
   `git status --porcelain` must show **no** change to either beyond what the
   band allows, otherwise `git checkout --` them and report (never commit a
   regenerated `COMPARISON.md`); **never open `docs/private/`,
   `aed_results/`, or create `COMPARISON_private.md`**; the script's
   `BASIS_ORDER` string becomes degree-aware; the degree-tagged `metrics` /
   XDMF outputs are **not committed** (a committed one would have to be
   declared in `LISTED_EXAMPLE_ARTIFACTS` — item 25's pin); setup-figure and docrefs
   censuses unchanged (`exit != 1`).
   **Scope:** the knob and one priced degree-2 point at h = 0.015. Not the
   h = 0.005 `xl` cost probe (the next review writes that pending entry from
   this item's price — it also needs a resolution knob, which is *not* this
   item), no adjudication, no absolute S.
   **Status it can move:** `ANS-6` row gains "step 2 (T9b) knob ✅ + degree-2
   price at h = 0.015"; unblocks the `xl` pending entry T9b names.
   **Negative result:** control outside 1e-9 ⇒ the example is not
   reproducible at fixed width — report the worst leaf, known-issues, stop
   before the degree-2 window.

24. DONE 2026-09-21 (11:00 UTC) — `TH-17` step 1c green: LC loop on the two-torus hole (sheet 211, `L_fem(64 MHz)` 1.073064785e-07 H, Grover ratio 0.9888 printed), (a) `Re f` 62.686 MHz vs `f_LC` 64 MHz = 2.053 % ≤ 5 %, (b) 2·C ratio 0.70905 vs 1/√2 = 0.275 % ≤ 2 %, control 0 in window; 394 s `-n 4`; `20260921T110359Z_TH-17.log`. 24. **`TH-17` step 1c — the closed-form LC control, on a fixture that exists
   and an `L` the fixture itself measures** (implementer;
   `tests/validation/test_th17_birdcage_eigenmodes.py` (+ a small loop-fixture
   builder in the same module); **no `src/`**; heavy (the row's tier), `-n 4`;
   complex build; independent; **50 slot-min**: one ≈ 5 min window + 15 +
   reading). **Chain step T5, part (iii)** — third attempt of the step-1
   family (1, 1b, 1c; no gate landed yet; **a fourth freezes it**).
   **Ruling (2026-09-21 review, rule (f), made here and not in-slot):** 1b's
   (iii) could not run because (1) its 2 % pre-condition compared the
   `PORT-1` *impressed-current* fixture's energy-route `L` to Grover, which
   reads 9.1 % at padding 0.08 m and is not parameterised
   (`test_port_self_impedance_energy.py:152`), and (2) **no single-loop
   generator exists** — `two_torus_domain` always gaps both tori
   (`io/mesh.py:1040–1046`). The closed form the chain names is unchanged:
   `f_LC = 1/(2π√(L·C))`, eigenvalue within **5 %**. What is re-registered is
   where `L` comes from: **the same hole fixture's own driven route** —
   `L_fem = Im Z_in/ω` of torus 1's sheet from `run_lumped_sheet_port_case`
   at 10 MHz **and** at the target (both printed; the target's is used) — so
   both sides of the 5 % are this fixture's numbers and the box's effect on
   `L` cancels. Grover (`grover_loop_inductance(0.04, 0.005)` = 1.085e-7 H,
   imported, never restated) is **printed beside, never gated**.
   **The change:** fixture `MeshGenerator.two_torus_domain(port_gap=True,
   emit_port_sheet=True, as_hole=True, …)` with `test_port_reaction_impedance`'s
   constants imported (`MAJOR_RADIUS`, `MINOR_RADIUS`, `SEPARATION`,
   `AIR_PADDING`, `H_FAR`, `H_WIRE`); torus 1's sheet (facet tag 201) carries
   the capacitor mass form exactly as `_capacitor_mass_forms` builds it
   (`h`, `w = area/h`); **torus 2's gap is left open in both solves** (no
   sheet form in the pencil; ≥ 1e9 Ω in the driven solve) — an open loop
   carries no current, and the residual coupling is k²-class (print the
   driven `Z₂₁/Z₁₁`). `C` is computed in-run as `1/((2π·64e6)²·L_fem)`
   (≈ 50 pF for ≈ 1.1–1.2e-7 H — printed, not pinned). `solve_general_mesh_modes`
   with `target_hz` at `f_LC`, `nev = 6`.
   **Anchors (asserted):** (a) the eigenvalue nearest `f_LC` has
   `|Re f − f_LC|/f_LC ≤ 0.05`; (b) at `2·C` the same mode moves by `1/√2`
   within 2 % (the row's gate (d) in miniature — a closed form of the same LC
   circuit). **Negative control (asserted):** with the capacitor form removed
   (`live=()`), no eigenvalue with `Re f` inside `f_LC · [0.8, 1.2]` — the
   mode is the sheet's doing; print the first eigenvalue found (predicted:
   the box's first cavity-like mode, ≳ 0.5 GHz for a ≈ 0.25 m box — printed,
   rule (e)).
   **Printed, never gated:** `L_fem` at 10 and 64 MHz, Grover and the ratio,
   `h/w`, the eigenvalue's imaginary part, dofs, solve seconds.
   **Tier / cost:** the two-torus package window is ≈ 150–190 s at `-n 2`
   (`20260920T141141Z_PORT-20.log`); the pencil on F-small's 111 121 dofs
   ran eight targets in 232 s at `-n 4` (`20260920T093810Z_TH-17.log`) ⇒ one
   window ≈ 4–6 min, `TH17_STEP1C=1` env gate like its siblings,
   `timeout -k 30 1100`, durable capture, `-s`.
   **Traps:** known-issues 2026-09-20 — a ~1 s docker-socket denial is
   intermittent: **retry once, never park on it**, and never pipe or
   subshell-wrap the harness call; `as_hole` requires `port_gap` and
   `emit_port_sheet` (`io/mesh.py:1226–1235`); the form is `(h/w)·jωC`, not
   `w/h`.
   **Scope:** one LC loop. It gates the pencil-with-sheet-mass machinery,
   not the birdcage's modes.
   **Status it can move:** under green — `TH-17`'s row records the pencil as
   **validated against a closed form**, and the 129.6 MHz floor (1b (ii))
   stands as the fixture's spectrum, handed to the 09-26 weekly with this
   review's reading (journal §4.3: the eigenproblem loads **all four** ports
   with `C_tuned`, the driven sweep feeds P1 — `Im Z_in = 0` and the eigen
   condition `Im Z_in = 1/(ωC)` are different equations, so `PORT-22`'s
   63.9986 MHz and 129.6 MHz are not in conflict and row gate (a) may be
   mis-posed; **the weekly rules, no slot re-scopes gate (a)**). Under red —
   a known-issues entry on `general_mesh_pencil` with the three numbers
   (`f_LC`, nearest `Re f`, `L_fem`), and `TH-17` drops to a mode-spectrum
   record per the weekly's watch condition (`PORT-22` ✅ already carries
   "tuned").
   **Negative result:** report and stop; widen nothing; BLOCKED per rule (d).

25. DONE 2026-09-21 (06:00) — `OPS-59` 🟡 → ✅: `ans6_…/metrics.json` declared, 27 passed at `-n 2` (8.2 s), `pinned = checker = git_ls_files = 28`, extras none, known-issues 2026-09-19 retired; `20260921T111239Z_OPS-59.log`. **`OPS-59` (a) — declare the one undeclared artifact and close the chunk**
   (implementer; `tests/unit/test_doc_reference_exit_codes.py` only; smoke;
   `-n 2`; real build; independent; **20 slot-min**: one ≈ 8 s window + 15).
   **Ruling (2026-09-21 review):**
   `examples/ansys_benchmarks/ans6_copper_birdcage_four_port_10_64_128MHz/metrics.json`
   **is declared** — it is the `ANS-1` rule's own case ("an `ans:` case
   commits its own `metrics.json`"; `ans1`…`ans4` are listed at `:459–465`),
   committed by `75082a0` without the declaration; `OPS-44` is the precedent
   for a late one. **The change:** that one path added to
   `LISTED_EXAMPLE_ARTIFACTS`, with the dated comment style of its
   neighbours. Nothing else.
   **Anchors (asserted — the module's own):** module green at `-n 2` (27
   passed expected; 8.2 s on record); the printed identity reads
   `pinned = checker = git_ls_files` (expect 27 each unless a figure landed
   since — count, do not copy). **Negative control (asserted, already in the
   module):** the synthetic non-setup PNG and the orphan `*_setup.png` still
   turn the identity red (`20260920T130232Z_OPS-59-postchange.log:420`).
   **Status it can move:** `OPS-59` 🟡 → ✅; the 2026-09-19 known-issues entry
   retired in the same commit; residual `main` reds 4 → 3.
   **Negative result:** another extra appears ⇒ name it, leave it red, stop —
   same rule as before. *(Standing, from today: an `ans:` runnable-half item
   declares its `metrics.json` in the commit that adds it.)*

26. DONE 2026-09-21 (11:00 UTC) — `OPS-50` 🟡 → ✅: `STEP4_SEP` 0.8198 / 0.8520 / 1.2281 two-sided at the unchanged 0.02, `PROBE_RESULT PASS` Status 0 (sep drifts ≤ 5.12e-06, REPRO ≤ 1.12e-05, round-trip 0), control `e2dd9ef` `FAIL PIN E` Status 1; 4 s + 4 s `-n 2`; `20260921T111436Z_OPS-50-step3b-probe.log`, `20260921T111404Z_OPS-50-step3b-prechange.log`. **`OPS-50` step 3b — land the parked re-record with the ordering guard
   replaced by a two-sided version-tagged separation record** (implementer;
   `scripts/probes/post4_step5_probe.py` only, starting from
   `attempt/OPS-50-20260920T125900Z` (`e2dd9ef`) **by path** — a probe
   script, not an append-only record; standard; `-n 2`; complex build;
   independent; **20 slot-min**: two ≈ 5 s windows + 15).
   **Ruling (2026-09-21 review — this is the licence):** the `vtx_s > mid_s`
   guard (blob `:555–570`) is, by the probe's own docstring (`:23–25`), a
   **fixture-drift tripwire** ("if the P1 numbers moved, the fixture drifted")
   frozen from v0.7.2's measured separations 0.4185× / 0.4818× / 0.6835×. On
   0.11 the fixture *did* move, by the licensed 30-cell cause, and E reads
   **1.2281× in all four 0.11 windows on both instruments**
   (`20260920T125627Z_OPS-50-step3-probe.log:411–413`: 0.8198× / 0.8520× /
   1.2281×). The tripwire is re-expressed so that it is **stricter, not
   looser**: `STEP4_SEP = {A: 0.8198, B: 0.8520, E: 1.2281}` (0.11 image,
   dated, both logs; v0.7.2's three values kept in-comment), each reproduced
   within the **unchanged** `PIN_REPRO_RTOL` 0.02 — the old guard admitted any
   separation in (0, 1], the new one a ±2 % window and fires on drift in
   either direction. Step 4's `REFUTED` verdict needs "not O(50×)", which
   1.23× satisfies on every field; the comment says so. `PIN_MID_MEDIAN_MIN`,
   `ROUNDTRIP_MAX`, `DG1_VS_SOURCE_MAX`, `PIN_REPRO_RTOL` **untouched**;
   `post4_step4_probe.py` untouched (historical instrument).
   **Anchors (asserted — the probe's own):** `PROBE_RESULT PASS`, Status 0,
   round-trip at 1e-14, every `REPRO` drift ≤ 0.02 (measured ≤ 9.71e-06),
   every `PIN_SEP` inside its record at 0.02.
   **Negative control (asserted, binary):** the parked blob at the pinned sha
   `e2dd9ef` (`git show e2dd9ef:scripts/probes/post4_step5_probe.py` into
   `/logs/`, never `HEAD:`) still ends `FAIL PIN E … PROBE_RESULT FAIL`,
   Status 1 **by design** — window name `OPS-50-step3b-prechange`.
   **Traps:** do not pipe; rule (i).
   **Status it can move:** `OPS-50` 🟡 → ✅ (Done-when (b) as amended
   2026-09-20, now also naming this guard — **row amended by this review,
   rule (j)**); the 2026-09-18 probe known-issues entry retired; dashboard's
   "ignore that FAIL line" note goes. The B-nondeterminism entry **stays
   open**.
   **Negative result:** `PASS` still does not print ⇒ a sixth pin — report
   it, revert, stop; the family is then at three and the next review banks
   the negative.

27. DONE 2026-09-21 (11:00 UTC) — `OPS-55` ⬜ → ✅: blake2b digest on the existing allgather; shifted equal-count lists raise on every rank at `-n 2` (flags `[True, True]`, message rank 1 row 0 dev 1.000e-02), `-n 1` does not; pre-change `14a0d71` blob all-valid with 7/8 rows off `(x, 2y, 3z)` by exactly 3.000e-02 (= 0.03·rank); identical-list < 1e-12 green; caller subset 54 passed, only the 2 known-issues #3 reds; `20260921T111743Z_OPS-55.log`, `…111745Z_OPS-55-n1.log`, `…111730Z_OPS-55-prechange.log`, `…111802Z_OPS-55-callers-complex.log`, `…112042Z_OPS-55-callers-real.log`. **`OPS-55` — `evaluate_vector_field_parallel` refuses equal-count,
   different-coordinate point lists** (implementer; **the §7 row is the item**
   — plan, anchor (closed form `(x, 2y, 3z)`), pre-change control at a pinned
   sha into `/logs/`, and negative control (`-n 1` does not raise) are all
   there; `src/fem_em_solver/post/evaluation.py` +
   `tests/post/test_evaluation_collective_guard.py`; smoke + standard; `-n 1`
   and `-n 2`; independent; **30 slot-min**: unit seconds + the rule-(c)
   caller subset (`tests/post`, `tests/ports`, one SAR test) ≈ 3 min + 15).
   **Traps:** the raise must be collective (every rank, allgathered flags) or
   `-n 2` hangs instead of failing — wrap the window in `timeout -k 30 180`;
   pin `<sha>^`, never `HEAD:`. **Status it can move:** `OPS-55` ⬜ → ✅.
   **Negative result:** a caller in the subset goes red ⇒ that caller was
   passing rank-local points — name it in known-issues, do not relax the
   guard, park.

28. DONE 2026-09-21 (11:24 UTC) — `OPS-56` ⬜ → ✅: step-relative endpoint inclusion (`1e-9·step`) with on-grid `stop_hz` pinned bitwise, finite-input `ValueError`s, step-relative merge replacing `np.unique`; pre-change `c1d167d` blob fails 3/3 reproduced cases (ends 127.7005 / 127.7010 MHz, single 64 MHz point); committed module 19 passed at `-n 1`, 2 s, existing 3 planner tests unchanged; `20260921T112323Z_OPS-56.log` (control), `…112330Z_OPS-56.log` (green). 28. **`OPS-56` — sweep-planner endpoint inclusion independent of the carrier**
   (implementer; the §7 row is the item; `src/fem_em_solver/ports/sweep.py` +
   `tests/ports/test_frequency_sweep_planner.py`; smoke, pure numpy, `-n 1`;
   independent; **20 slot-min**). Anchors: the row's exact grids — the three
   reproduced cases, the nondivisible span, `(0.1, 0.3, 0.1)` → 3 points,
   bitwise `stop_hz`; pre-change control at the pinned sha fails the three
   cases. **Status it can move:** `OPS-56` ⬜ → ✅. **Negative result:** an
   existing planner test changes its grid ⇒ stop and report which sweep
   record depended on the old rounding.

29. DONE 2026-09-21 (11:30 UTC) — `OPS-57` ⬜ → ✅: det tests → `LinAlgError` → same-message `ValueError`, non-finite checks, `sparameters_from_impedance` delegates to `z_to_s`; 16-port z0 = 1e-21 round trip 1.877e-16 (det underflows, cond 3.94), delegation 0.0; package module 18 passed / 194 s, `PORT-9` gate 5 passed / 37 s, records inside bands, nothing widened; `20260921T112537Z_OPS-57.log`, `…112548Z_OPS-57.log`, `…112912Z_OPS-57.log`. 29. **`OPS-57` — S ↔ Z conversions: let the solve report singularity; one
   impedance-to-S implementation** (implementer; the §7 row is the item;
   `ports/circuit.py`, `ports/sparameters.py`; smoke + standard; `-n 2` for
   the gate re-run; independent of 27–28; **30 slot-min**: unit seconds +
   `test_port_package_sparameters.py` 190 s + `PORT-9` gate 66 s + 15).
   **Note since the row was written:** `PORT-20` landed — the current route
   now returns `sparameters_from_impedance(Z)` and step 2 gates it against
   `z_to_s` at 1e-12 *because the two are independent implementations*. When
   this item makes one delegate to the other that anchor becomes a tautology:
   **keep an independent check** — the unit module's closed-form T-network
   (`tests/unit/test_port20_current_route_s.py`) already is one; say in the
   `PORT-20` test's docstring that the closed form, not the cross-
   implementation residual, is the anchor from here on. No tolerance touched.
   **Status it can move:** `OPS-57` ⬜ → ✅. **Negative result:** any PORT
   record moves beyond its band ⇒ revert, known-issues, stop.

30. DONE 2026-09-22 (04:00 UTC) — `OPS-58` ⬜ → ✅. The 2026-09-21 block is
   lifted: the operator ruled the single `--audit` match a **false positive**
   — a shared *input* (the phantom cylinder's volume, reproducible from the
   geometry `SPEC.md` already publishes in plain text), not an AED result, so
   **no history rewrite**. The parked branch merged and a narrow shared-input
   allowlist landed: entries key on (tracked path, JSON key on the line, exact
   value), the value is *computed* from its published closed form rather than
   written out, and every suppression is counted and named in the output.
   Controls 24/24 (`20260922T040113Z_OPS-58.log`, 3 s), `--audit` on this clone
   `clean:` with `1 shared-input match suppressed`
   (`20260922T040116Z_OPS-58-audit.log`, 8 s). Waiting-on-you keeps only the
   `install_git_hooks.sh` re-run. See the §7 row. 30. **`OPS-58` — private-leak check: commit messages, fail-closed, "skipped"
   ...
   `run_and_log.sh`, no container**; independent; **25 slot-min**).
   **Traps:** the planted figure in the test is **synthetic** — never read a
   real value out of `docs/private/` or `aed_results/` to build a fixture, and
   `--audit` prints a count, never a figure. **Status it can move:** `OPS-58`
   ⬜ → ✅ and a Waiting-on-you line: re-run `scripts/testing/install_git_hooks.sh`.
   **Negative result:** `--audit` on this clone reports a hit ⇒ **stop, print
   nothing but the commit hash and file, journal, dashboard** — that is the
   operator's to handle.

31. DONE 2026-09-21 (12:30 UTC) — `EX-61` ⬜ → ✅: all three imported anchors
   green at record width, `f0`/`R_in(f0)`/`Q` reproduce `PORT-22`'s own grid
   readings to the digit, both censuses matched predicted delta;
   `20260921T124138Z_EX-61.log`, 203 s. See the §7 row.
   **`EX-61` — the tuned birdcage's resonance curve** (`example-runner`,
   spawned **foreground**: "you are the executor, do not spawn agents, never
   return with a window running"; new
   `examples/ports/19_birdcage_tuned_resonance_curve.py` + same-stem guide;
   **no `src/`**; heavy; `-n 2`; complex build; independent; **45 slot-min**).
   The §7 `EX-61` row (added today) is the item: six frequencies 52–72 MHz in
   4 MHz steps on the gate mesh, every fixture, `C_tuned`, band and helper
   **imported** from `tests/validation/test_port22_driven_sweep_resonance.py`;
   asserted: the circuit-vs-field residual ≤ `REDUCTION_BAND` at each
   frequency, exactly one `Im Z_in` sign change, in the bracket holding
   64 MHz, and `PORT-22`'s `0.5 × C_tuned` control; written: `R_in` / `X_in`
   vs f as CSV + PNG, the interpolated `f₀`, `R_in(f₀)` and loaded Q
   (printed, never gated), combined XDMF of the tuned field at 64 MHz, setup
   figure per the `EX-57` template. **Cost:** `PORT-22` measured ≈ 32 s per
   frequency (358 s / 11) ⇒ ≈ 200 s + export; `timeout -k 30 590`. **Traps:**
   no absolute `S₁₁` / `Z_in` claim (known-issues 2026-09-19); censuses
   before and after by their real filenames; docrefs `exit != 1`. **Status
   it can move:** `EX-61` ⬜ → ✅; runnable examples 55 → 56.

32. **DONE 2026-09-21 (12:30 UTC slot)** — `EX-57` figure for
   `examples/meshing/09_birdcage_sixteen_ring_gaps.py` (`mesh:9`), drawn on
   the 16-leg both-end-rings-cut rung, `EX-35` digits reproduced digit-for-
   digit flagged vs unflagged, census `missing` 33 → 32 (`ok` 23 → 24,
   `broken` 0), docrefs `dead=0 guide=0 exit=2` both sides, `OPS-59` (b)
   re-run green (20 passed) after `git add`ing the new PNG. Two defects
   found and fixed before committing (§7 `EX-57` row has the detail): the
   leg region names first matched no `CLASS_COLOURS` keyword (six-colour
   fallback cycle instead of one conductor class) and the first slice sat
   exactly on the top ring's axial symmetry plane (ring band unresolved).
   Logs: `20260921T130303Z_EX-57.log` (flagged), `20260921T130608Z_EX-57.log`
   (unflagged control), `20260921T130543Z_EX-57.log` /
   `20260921T130813Z_EX-57.log` (pre/post census), `20260921T130859Z_EX-57.log`
   (`OPS-59` re-run). ~~**`EX-57` — setup figure for
   `examples/meshing/09_birdcage_sixteen_ring_gaps.py`** (`example-runner`,
   foreground, same prompt rules as item 31; the example's own tier and
   width — read its recorded window from its `EX-*` row; real build;
   independent; **20 slot-min**). The census's `--next` at review time
   (`20260921T080305Z_EX-57-next.log`: 55 examples, 22 ok, 33 missing,
   0 broken). Template as amended 2026-09-19: hide the region that is not
   the subject; `write_setup_figure` outside any printed timer; title ≤ 130;
   facet tags captioned, not invented as cell regions; both censuses by their
   real filenames; full artifact filenames in the guide; **read the PNG and
   check every caption and source-comment sentence against the geometry
   before committing**; the call-site / PNG / census identity (`OPS-59` (b))
   must still read equal. Done-when is the §7 `EX-57` per-item list.
   **Status it can move:** `EX-57` census `missing` 33 → 32.~~
