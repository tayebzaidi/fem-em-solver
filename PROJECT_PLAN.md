...
| `ANS-2` | Coil-driven SAR in the loaded four-leg birdcage at 10 MHz: pointwise SAR, whole-phantom power, mass-averaged SAR | 🟡 Open: step 1's runnable half is green with its control asserted as a floor, and nothing absolute is licensed. Step 2 (SPEC rows 4–6) remains open. **Step 3 — the AED numbers landed 2026-09-18** (operator, HFSS 2026 R1, Zero and First Order; Linux half re-run `20260918T235648Z_ANS-2-aed-private.log`, 216 s, anchors green, `COMPARISON_private.md` filled). **Preliminary verdict AGREE on the shape-free rows 1–3, owed to the 2026-09-19 weekly to ratify:** the first reading found a near-constant factor of two between AED and us; the interactive session traced it to the example's own incident-power line (`V²/(4Z₀)`, the RMS form) against the package's peak-phasor convention (`ports/superposition.py`: `|V_src|²/(8Z₀)`), confirmed by our reported accepted power matching `1 − \|S₁₁\|²` only of the halved figure. Example, guide and SPEC corrected (ratio to HFSS's 1 W is 400, not 200; `src/` untouched); re-run `20260919T001707Z_ANS-2-normalisation-fix.log` (201 s, anchors green). Under the corrected figure the whole-phantom power agrees at the 5 % level, identically for all four drives and both AED orders, and the driven-point SAR within ±10 % with the scatter on our side (point sampling on the 2.5 mm phantom mesh). The two AED orders agree with each other to 1–3 %. Row 6 peaks are boundary artifacts of HFSS's cube averaging (flagged in the script); the AED \|E\| column needs a harvest-only rerun. The weekly writes the ruling, the band and what it promotes (SPEC §"What an agreement would license"). Analysis: `docs/private/ans2-operator-notes-2026-09-18.md`. *History: `docs/planning/chunks/ANS-2.md`.* | heavy |
...
*(The 09-16 queue's four items are all done: `WF-7` step 0c (`a267c5a`),
`OPS-48` (`fff1673`), `OPS-49` (`788cc2c`), the `mag:5` figure (`9b35280`);
the item texts are archived verbatim in `docs/planning/plan-archive.md`
(2026-09-18) and journaled in `docs/testing/attempts.md`.)*

**Predicted slot-minutes (rubric element 3 + 15 min fixed), running total:**
item 1 → 22 · item 2 → 42 · item 3 → **62**. Floor 240 and ≥ 5 items:
**shortfall 178 min and two items**, stated, not filled (the restock
paragraph above says what exists and why it is not queued). The three items
are mutually independent; take them in order. A slot that finds all three
