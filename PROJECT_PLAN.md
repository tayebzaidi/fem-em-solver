...
1. **✅ DONE 2026-08-27, 19:30 implementer slot — landed on `main` exactly as
   ruled** (six names, five files, all exact equalities, no band, `src/`
   untouched; four anchors green in 604 s — geomfloor `12 passed`, richardson
   `25 passed`, probe/30 MHz `23 passed`, `mesh_cache` real `12 passed, 4
   skipped`, all Status 0; three known-issues entries retired or partially
   retired; findings 38–40 in the §7 entry). **`OPS-27` step 1 —
   re-record the cheap half of the stale 0.7.2-era
...
2. **✅ DONE 2026-08-27, 21:00 implementer slot — landed on `main` as ruled**
   (four edits in four files, not five — `third_rung` imports its constant,
   finding 41; all exact equalities, no band, `src/` untouched; three anchors
   green in 1 523 s — slab `16 passed`, knobs `15 passed`, wire-projected `4
   passed`, all Status 0; the leg (f) and leg (g) known-issues entries retired,
   the leg (e) `third_rung` entry re-headed 🟡 re-run-owed; findings 41–43 in
   the §7 entry). **`OPS-27` step 2 — re-record the expensive half by mesh value: the
...
3. **✅ DONE 2026-08-28, 22:30 implementer slot — landed on `main` exactly as
   ruled** (the double only, one `staticmethod`, `src/` untouched; red
   baseline and green-anchor gate on the identical `tests/ports` command,
   3 s and 2 s; the sign-flip anchor green at `±5.000000e-02 V`,
   `rel=1e-12`; the S-matrix name red at its own assertion ⇒ **entry 3
   re-dated, not retired**, with the measured correction that its zero is
   the *off-diagonal* on this fixture, not the diagonal; the leg (b)
   `allgather` known-issues entry retired whole; the other three
   `tests/ports` modules unchanged at 14 green; finding 44 in the §7
   entry). **`OPS-28` — give `test_port_orientation_sensitivity.py`'s `_DummyComm`
   ...
   `main`; independent; commissioned this review — full rubric in the §7
   entry).** In brief: `tests/ports/test_port_orientation_sensitivity.py:16-21`
   gets `allgather(value) -> [value]` beside its `allreduce`; the reduction
   at `src/fem_em_solver/ports/excitation.py:262-268` is **correct and
   stays** (`git show -- src/` empty). **Anchor:**
   `…::test_port_orientation_flip_changes_induced_voltage_sign` green — a
   sign-flip identity, `V(flipped) = −V(original)` inside the test's own
   band; and the *recorded outcome* of
   `…::test_port_orientation_flip_changes_off_diagonal_sparameter_sign`
   once it reaches its S-matrix assertion — green retires entry 3's line
   for it, red for entry 3's zero-diagonal reason re-dates entry 3 and
   stays filed. **Negative control:** the other three `tests/ports`
   modules unchanged at 14/17 green in the same root command (census:
   every one ≤ 2 s). **Cost:** `tests/ports` whole root, `-k 30 120`,
   ~10 s. **Traps:** under `-n 2` each rank runs the dummy independently —
   `[value]` is the rank-1 semantic and is what a mock comm means;
   `DeprecationWarning` noise from the placeholder route is expected.
   **Scope:** the double only; not a fix of entry 3, no `sparameters.py`
...
5. **`GEO-20` step 2 — the ring-gap port layout at 16 legs: 32 ring
...
6. **`EX-34` — birdcage 4-port S-matrix across the 10 / 64 / 128 MHz
   ...
   **Negative result:** a red imported band through the example path is
   ...
   stop.
7. **`GEO-22` — bisect the `straight_wire_domain` coarse-resolution floor
...
8. **`GEO-23` step 1 — classify the three 0.11 "overlapping facets" sites
